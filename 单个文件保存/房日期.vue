<template>
    <div class="statusmanage">
        ...
        <div v-if="authzData['F:BH_BOOK_STATUS_EDIT_BATCH'] && isHaveData" class="batchmbtn"><el-button @click="batchModifyStatus">批量修改</el-button></div>
        <br/>
        <table class="tablestyle">
            ...
            <tr v-for="item in statusDataList" :key="item.id">
                ...
                <td v-for="subitem in item.bookRoomStateDTOS" :key="subitem.id" @click="modifyStatus(item,subitem)">
        ...
        <el-dialog :visible.sync="dislogVisibleModify" width="30%">
            <span slot="title">修改<span class="mstyle">{{mDate}}</span><span class="mstyle">周{{mWeek}}</span></span>
            <el-form :model="modifyForm" :rules="modifyRules" ref="modifyForm" class="mform">
                ...
                <el-form-item prop="roomInfo">
                    <el-checkbox-group v-model="modifyForm.roomInfo">
                        ...
                        <el-checkbox label="1">设为满房</el-checkbox><br/>
                        <el-checkbox label="2">设置房间数：设为&nbsp;<i class="el-icon-thirdfangkuai1" @click.prevent="jianR"></i>&nbsp;&nbsp;<label v-html="mroomNum"></label>&nbsp;&nbsp;<i class="el-icon-thirdfangkuai2" @click.prevent="jiaR"></i>&nbsp;间&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当前预订{{mroomNumed}}间</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
            </el-form>
            <div slot="footer">
                <el-button @click="dislogVisibleModify = false">取 消</el-button>
                <el-button v-if="authzData['F:BH_BOOK_STATUS_EDIT']" type="primary" @click="modifyEnsure('modifyForm')">确 定</el-button>
            </div>
        </el-dialog>
        <el-dialog title="批量修改" :visible.sync="dislogVisibleBatchModify" width="35%">
            <el-form :model="batchMForm" :rules="batchMRules" ref="batchMForm" label-width="120px" class="batchmform">
                <el-form-item prop="resourceId">
                    <span slot="label"><label class="required-icon">*</label> 选择房源：</span>
                ...
                <el-form-item label="设置：" prop="roomInfo">
                    <el-checkbox-group v-model="batchMForm.roomInfo">
                        ...
                        <el-checkbox label="1">设为满房</el-checkbox><br/>
                        <el-checkbox label="2">设置房间数：设为&nbsp;<i class="el-icon-thirdfangkuai1" @click.prevent="jianR"></i>&nbsp;&nbsp;<label v-html="mroomNum"></label>&nbsp;&nbsp;<i class="el-icon-thirdfangkuai2" @click.prevent="jiaR"></i>&nbsp;间</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
            </el-form>
            <div slot="footer">
...
export default {
    ...
    data(){
        return{
            ...
            modifyRules: {
                roomInfo: [
                    { type: 'array', required: true, message: '请至少选择一个要修改的房间信息', trigger: 'change' }
                ],
            },
            //批量修改
            isHaveData: false,
    ...
    methods: {
        ...
        modifyEnsure(modifyForm){
            this.$refs[modifyForm].validate((valid) => {
                if(valid){
                    ...
                    // console.log(params);
                    // return;
                    this.$api.bookStatusModify(params)
                        .then(response => {
                            // console.log(response);
                            const result = response.data;
                            if(result.code == '0'){
        ...
        resourceSelect(value){      //添加房源
            const resource = this.resourceDataList.find(item => item.resourceId === value);
