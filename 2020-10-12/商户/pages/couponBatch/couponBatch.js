...
app.Base({
  ...
  deleBatch:function(e){
    ...
    wx.showModal({
      ...
      success(res){
        if(res.confirm){
          that.deleCouponBatch(id);
        }
      }
    })
  },

  deleCouponBatch:function(id){
     let that=this;
    wxrequest.deleCouponBatch(id).then(res=>{
       let resdata=res.data;
       if(resdata.code==0){
          wx.showToast({
            title: '操作成功',
  ...
  swtichbtn:function(e){
    ...
    console.log(index)
    let nowswitchJudge = "batchData[" + index + "].switchJudge"
    this.setData({
