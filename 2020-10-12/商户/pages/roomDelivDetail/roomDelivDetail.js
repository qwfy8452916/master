// pages/roomDelivDetail/roomDelivDetail.js
const app = getApp();
...
app.Base({
  ...
  platDeliveryDetail: function (id) {
    ...
    wxrequest.platDeliveryDetail(id).then(res => {
      ...
      if (resdata.code == 0) {
        if (resdata.data.rmavcRecordsDto != null){
          let detailObj = resdata.data.rmavcRecordsDto.orderDetailMap;
          for (let i in detailObj) {
            switch (i) {
              case 'LIST_ORDER':
                that.setData({
                  isShowSelectList:true,
                  selectDataList: detailObj[i],
                })
                break
              case 'BANNER_DETAIL_ORDER':
                that.setData({
                  isShowBannerList: true,
                  bannerDataList: detailObj[i],
                })
                break
              case 'ICON_ORDER':
                that.setData({
                  isShowIconList: true,
                  iconDataList: detailObj[i],
                })
                break
              case 'DYNAMIC_FORM':
                that.setData({
                  isShowFormList: true,
                  formDataList: detailObj[i],
                })
                break
            }
          }
