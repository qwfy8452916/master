  ...
  onLoad: function (options) {
    ...
    if (subRespBeanList != null) {
      for (var i = 0; i < subRespBeanList.length;i++){
        ...
        } else if (subRespBeanList[i].businessSubType == '6' && subRespBeanList[i].isSubscribe == '1') {
          nowmsgjudge.incomeMsg = true
        }
      }
      //已订阅
      this.setData({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          if (res.data.data) {
            ...
            } else if (that.data.businessSubType == '6') {
              that.setData({
                'msgjudge.incomeMsg': !that.data.msgjudge.incomeMsg
              })
            }
          } else {
            wx.hideLoading()
