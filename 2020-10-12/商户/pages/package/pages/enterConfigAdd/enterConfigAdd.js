// pages/package/pages/enterConfigAdd/enterConfigAdd.js
const app = getApp();
...
//选择柜子类型
  ...
  getSelectedHotel:function(e){
    ...
    wxrequest.hotelFunctionList(linkData).then(res=>{
       let resdata=res.data;
      let nowareaList = that.data.configdata.areaList;
      let nowhomePageData = that.data.homePageData;
       if(resdata.code==0){
         let recordsData = resdata.data.records;
         let needsData = recordsData.filter(item=>{
           return item.id!=13 && item.id!=14
         })
         nowareaList = needsData.map(item=>{
           ...
           return {
             ...
             commonEnterDeviList: [],
             shareEnterDeviList: [],
             funnterData: JSON.parse(nowfunnterData),
             funshareData: JSON.parse(nowfunshareData),
             homePage: 3,
             funTypeIndex:0,
             funTypedata: [
               {id:0, name: "不支持"},
         ...
         nowhomePageData = needsData.map(item => {
           return {
             areaName: item.funcCnName,
             id: item.id,
             homePage:3
  ...
  sureBtn: function () {
    ...
    wxrequest.addenterCabConf(linkData).then(res => {
      let resdata = res.data;
      if (resdata.code == 0) {
        wx.showToast({
          title: '进场配置新增成功！',
