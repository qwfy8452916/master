// pages/classifySelect/classifySelect.js
const app = getApp();
...
app.Base({
  ...
  surebtn: function () {
    ...
    wx.setStorageSync('selectTypeId', nowselectTypeId)
    wx.setStorageSync('selectTypeData', this.data.selectTypeData)
    wx.setStorageSync('typesure', true)
  ...
  funitem: function (e) {
    ...
    if (nowselectTypeData.length > 0) {
      ...
      // if (JSON.stringify(nowselectTypeData).indexOf(JSON.stringify(nowtypeData[index])) == -1) {
      //   nowselectTypeData.push(nowtypeData[index])
      // } else {
      //   for (var i = 0; i < nowselectTypeData.length; i++) {
      //     if (nowselectTypeData[i].id == nowtypeData[index].id) {
      //       nowselectTypeData.splice(i, 1)
      //     }
      //   }
      // }
    } else {
  ...
  getFunctionClassify: function () {
    ...
    wxrequest.functionClassifyTree(linkData).then(res => {
      ...
      if (resdata.code == 0) {
        ...
        let nowselectTypeData = nowselectFunData[classIndex].getTypeData;
