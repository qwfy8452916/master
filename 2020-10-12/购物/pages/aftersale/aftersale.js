...
Page({
  data: {
    ...
    roomCode: '',//房间号
    prodcode: '',//商品code
    prodOwnerOrgId: '',//商品所属组织id
    prodOwnerOrgKind: '',//商品平台类型
    deliid: '',
    delivcode: '',
    devidetailid: '',
  ...
  get_orderprodlist: function (options) {//获取订单商品
    const that = this;
    let linkData = {
      deliId: options.deliid
    };
  ...
  bindPickerChange: function (e) {//选择售后类型
    this.setData({
  ...
  subfun: function () {//提交申请售后
    ...
    let linkData = {
      cabApplReason: that.data.aftersaletype[dictValue].dictValue,//迷你吧申请售后原因
      cabCusName: username,//联系人姓名
      cabCusPhone: usertel,//联系人电话
      cabId: that.data.cabId,//柜子id
      cabLatticeCode: that.data.latticeCode,//格子编号
      cabRoomCode: that.data.roomCode,//柜子房间号
      csDescription: userremark,//售后申请描述
      csType: 3,//售后类型（1：换货，2：退货退款，3：迷你吧，4：退款）
      cusLogisticsCode: '',//客户物流单号
      cusLogisticsInfo: '',//客户物流信息
      customerId: that.data.userid,//用户id
      funcId: 1,//功能区Id
      hotelId: that.data.hotelId,//酒店id
      prodCode: that.data.prodcode,//商品code
      hotelProdId: that.data.hotelProdId,//酒店商品Id
      prodOwnerOrgId: that.data.prodOwnerOrgId,//商品所属组织id
      prodOwnerOrgKind: that.data.prodOwnerOrgKind,//商品平台类型
      certificateImages: productInfo,//凭证图
      prodCount: 1,
      refoundAmount: 0,
