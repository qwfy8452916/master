...
Page({
  ...
  onLoad: function (options) {
    ...
    self.get_Location(options);
  },
  onShow: function () {
    let self = this;
    console.log(self.data.num);
    if(self.data.num == 1){
      self.get_Location2(self.data.optionsdata);
    }
  },
  get_Location: function (optiondata) {
    wx.showLoading({
      title: '加载中',
    });
    const self = this;
    let latitude = optiondata.lat;
    let longitude = optiondata.lng;
    wx.getLocation({
      type: 'wgs84',
      success(res) {
      ...
      fail(err) {
        wx.hideLoading();
        // self.get_Location(that.data.optionsdata);
        // wx.showToast({
        //   title: '请允许授权访问定位，否则将影响地图功能使用',
        //   icon: 'none',
        //   duration: 2000
        // });
        // setTimeout(() => {
        ...
        // }, 3000);
        wx.showModal({
          content: '检测到您没打开此小程序的定位权限，将影响地图功能使用，请打开“使用我的地理位置”开关',
          confirmText: "确认",
          showCancel: false,
          success: function (res) {//点击“确认”时打开设置页面
            self.setData({
              num: 1
            })
            if (res.confirm) {
  ...
  get_Location2: function (optiondata) {
    wx.showLoading({
      title: '加载中',
    });
    const self = this;
  ...
  addMarker: function (data) {
    ...
    mks.push({ // 获取返回结果，放到mks数组中
      ...
      iconPath: "/img/my_marker.png", //图标路径
      width: 25,
      height: 25
    })
  ...
  selectedOk: function () {
    ...
    //})
    app.globalData.addressinfo = this.data.centerData;
    wx.navigateBack({
      delta: 1
    })
  }
