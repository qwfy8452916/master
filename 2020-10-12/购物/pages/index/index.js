...
Page({
  ...
  onLoad: function (options) {
    ...
    that.setData({
      ...
      Approachtype: app.globalData.Approachtype
    });
    ...
    that.get_wififun();
    console.log(app.globalData.Approachtype)
  },
    ...
    wxrequest.getfuncdetail(id, linkData).then(res => {
      ...
      if (resdata.code == 0) {
        ...
        this.setData({
          ...
          isMinibar: resdatas.isSupportCab,
          isConvenienceStore: resdatas.isSupportStore,
          isSupportCab: resdatas.isSupportCab
        });
      } else {
        wx.showToast({
          title: res.data.msg,
          ...
          for (let j = 0; j < buy_list.length; j++){
            for (let i = 0; i < commoditylist.length; i++) {
              if (buy_list[j].latticeCode == commoditylist[i].latticeCode){
                ...
                if(commoditylist[i].prodnum > 0){
                  commoditylist[i].prodnumtype = true;
                } else {
                  commoditylist[i].prodnumtype = false;
                }
              }
          ...
          for (let j = 0; j < buy_list.length; j++) {
            for (let i = 0; i < store_list1.length; i++) {
              if (buy_list[j].prodCode == store_list1[i].prodCode && buy_list[j].prodtype == 2) {
                ...
                if(store_list1[i].prodnum > 0){
                  store_list1[i].prodnumtype = true;
                } else {
                  store_list1[i].prodnumtype = false;
                }
              }
            }
          }
  ...
  details: function (e) {//商品详情
    ...
    let prodamt = e.currentTarget.dataset.prodamt;
    let prodnumtype = e.currentTarget.dataset.prodnumtype;
    wx.navigateTo({
  ...
  getlength: function(list){//计算购物车商品数量和总价
    ...
    let num = 0;
    let hasfuncprod = 0;
    const shopcart_voucherlist = that.data.shopcartvoucherlist;
    for (let i = 0; i < list.length; i++) {
      let prodtotal = 0.00;
    ...
    if(shopcart_voucherlist.length > 0) {
      for(let i = 0; i < shopcart_voucherlist.length; i++){
        let prodtotal2 = 0.00;
        prodtotal2 = shopcart_voucherlist[i].prodnum * parseFloat(shopcart_voucherlist[i].prodRetailPrice);
        money = money + prodtotal2;
      }
    }
    if (num != 0 || shopcart_voucherlist.length > 0){
      hasfuncprod = 1;
    } else {
      hasfuncprod = 0;
    }
  ...
  getshoppingcart: function () {//处理购物车数据
    ...
    if (hasfuncprod == 1){
      ...
      let delivery_list5 = thatdata.shopcartvoucherlist;//卡券
      ...
      for (let i = 0; i < delivery_list5.length; i++) {
        delivery_list5[i].totalprice = parseFloat(delivery_list5[i].prodRetailPrice) * delivery_list5[i].prodnum;
