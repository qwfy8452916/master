...
Page({
  ...
  aprodamountchange: function (e) {//修改退款金额
    const that = this;
    const aproda_mount = that.data.aprodamount;
    if(e.detail.value > aproda_mount) {
      wx.showModal({
        title: '提示',
  ...
  aftersub: function () {//提交售后申请
    ...
    const lgcStatus = that.data.lgcstatus;
    let csK_ind = thatdata.csKind;
    that.setData({
      refundsubtype: false
    });
    if(lgcStatus == 3 || lgcStatus == 4) {
      csK_ind = 2;
    }
    ...
    if (csK_ind == 1) {//换货
      ...
      wx.showModal({
        ...
        success(res) {
          if (res.confirm) {
            that.setData({
              aprodamountAll: thatdata.prodinfo.aprodamount,
              refundsubtype: true
            })
          }
        }
    ...
    let linkData = {
      ...
      csType: csK_ind,//售后类型（1：换货，2：退货退款，3：迷你吧，4：退款）
