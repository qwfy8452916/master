<template>
    <div class="commoditylist">
        ...
        <div><el-button class="addbutton" @click="commodityAdd">添加商品</el-button></div>
...
export default {
    ...
    methods: {
        ...
        //新增
        commodityAdd(){
            const orgId = this.encryptedOrgId;
            const id = this.hotelId;
            this.$router.push({name: 'LonganHotelCommodityAdd', params:{orgId}, query: {id}});
        },
        //修改
        ModifyHotelCommodity(id){
            const orgId = this.encryptedOrgId;
            const hotelId = this.hotelId;
            this.$router.push({name: 'LonganHotelCommodityModify', query: {id}, params: {orgId: orgId, hotelId: hotelId}});
        },
        //查看历史价格
        lookHistoryPrice(id){
            const params = {
