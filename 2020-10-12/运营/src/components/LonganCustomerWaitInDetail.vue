<template>
    <div class="customerWaitInDetail">
         ...
         <table class="firsttable">
          ...
          <tr>
            <td>openid</td><td>{{recordDetailFormData.openId}}</td>
          </tr>
          <tr>
            <td>姓名</td><td>11{{recordDetailFormData.empOrCusName}}</td>
          </tr>
          <tr>
            <td>手机号</td><td>22{{recordDetailFormData.empOrCusPhone}}</td>
          </tr>
        </table>
        <p class="ptitle">订单信息</p>
         <table>
          ...
          <tr>
            <td>待入账金额</td><td>{{recordDetailFormData.pendingAmount}}</td>
          </tr>
        </table>
        ...
        <div v-if="recordDetailFormData.funcId===-1">
            <p class="ptitle">客房信息</p>
              <table>
                ...
                <tr>
                  <td>下单时间</td><td>{{recordDetailFormData.settlingTime}}</td>
                </tr>
              </table>
        </div>
...
export default {
    ...
    methods: {
       ...
       //返回
      cancelbtn(){
       this.$router.push({name:'LonganCustomerWaitIn'})
      },
       ...
       Getdata(){
            let that=this;
            this.$api.waitInDetail(that.detailId,2).then(response=>{
                if(response.data.code==0){
                  that.recordDetailFormData=response.data.data
                  console.log(that.recordDetailFormData)
                }else{
...
.customerWaitInDetail{
    ...
    table {
      text-align: left; border-collapse: collapse;width: 500px;
