<template>
  <div>
    ...
    <!-- 普通图片上传是否带星号，没有排序和链接 -->
    <div class="bannerwrap" v-if="showNormal">
      <el-form-item>
        <span slot="label" v-if="isRequired">
          <label class="required-icon">*</label> banner图
        </span>
        <el-upload
          ::disabled="isDisabled"
...
export default {
  ...
  props: {
    ...
    uploadType为normal的时候需要传递参数descList(数据列表)，isDisabled(是否禁用)，@descListEvent（移除图片）,isRequired(是否展示必选※)。
