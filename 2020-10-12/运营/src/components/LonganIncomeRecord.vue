  ...
  <div class="incomeRecord">
    <el-form :inline="true" :model="query" ref="query" align="left" class="searchform">
      ...
      <el-form-item label="收入类型">
        <el-select v-model="query.incomeType">
          <el-option
            v-for="item in incomeTypedata"
            :label="item.dictName"
    ...
    <el-table :data="incomeRecordDataList" border stripe style="width:100%;">
      <el-table-column prop="id" label="ID" align="center"></el-table-column>
      <el-table-column prop="empOrCusName" label="姓名" align="center"></el-table-column>
      ...
      <el-table-column prop="incomeType" label="收支类型" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.incomeType==1">分享奖励</span>
          <span v-if="scope.row.incomeType==2">管理奖励</span>
          <span v-if="scope.row.incomeType==3">红包</span>
          <span v-if="scope.row.incomeType==4">补货费</span>
          <span v-if="scope.row.incomeType==6">最优弹性价提成</span>
        </template>
      </el-table-column>
      <el-table-column prop="incomeAmount" label="收入金额" align="center"></el-table-column>
      <el-table-column prop="userBalance" label="账户余额" align="center"></el-table-column>
      ...
      <el-table-column label="操作" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_FIN_EMPINCOME_DETAIL']"
            type="text"
            size="small"
...
export default {
  ...
  methods: {
    ...
    //查看详情
    lookdetail(id) {
      this.$router.push({ name: "LonganIncomeRecordDetail", query: { id } });
    },
    ...
    incomeRecord() {
      ...
      this.$api
        .incomeRecord({ params }, 1)
        .then((response) => {
          if (response.data.code == 0) {
            this.incomeRecordDataList = response.data.data.records;
