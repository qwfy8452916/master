...
<template>
  <div class="messageTest">
    ...
    >
      ...
      <el-form-item>
        <el-button
          v-if="authzData['F:BO_MSG_TEST_SEND']"
          @click="sendMessage('messagedata')"
          type="primary"
        >发送</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-dialog class="messageone" :visible.sync="showhidemsgone">
      <div class="msghang">
        <span class="msg_title">发送结果：</span>
        <span>{{sendStaus}}</span>
      </div>
      <div class="msghang">
        <span class="msg_title" v-if="sendStaus==='发送失败'">原因：</span>
        <span>{{sendReason}}</span>
      </div>
      <div class="msghang" v-if="sendStaus==='发送成功'">
        <span class="msg_title">业务消息id：</span>
        <span>{{sendResult.msgId}}</span>
      </div>
      <div class="wrapbtn">
  ...
  methods: {
    ...
    receiveType() {
      ...
      this.$api
        ...
        .then((response) => {
          if (response.data.code == "0") {
            that.receiveTypedata = response.data.data;
          } else {
