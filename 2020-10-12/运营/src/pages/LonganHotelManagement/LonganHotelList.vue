<template>
  <div class="hotellist">
    ...
    <div v-if="authzData['F:BO_HOTEL_HOTEL_ADD']">
      <el-button class="addbutton" @click="hotelAdd">新&nbsp;&nbsp;增</el-button>
    </div>
    <el-table :data="HotelDataList" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" width="300px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_HOTEL_HOTEL_VIEW']"
            type="text"
            size="small"
            @click="LookHotelDetail(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_HOTEL_EDIT']"
            type="text"
            size="small"
            @click="ModifyHotel(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_HOTEL_DELETE']"
            type="text"
            size="small"
            @click="DeleteHotel(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_HOTEL_RESETPWD']"
            type="text"
            size="small"
            @click="ResetPWD(scope.row.id)"
          >重置密码</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_HOTEL_CAB']"
            type="text"
            size="small"
            @click="HotelCabinet(scope.row.id)"
          >管理柜子商品</el-button>
