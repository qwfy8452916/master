    ...
    <div class="PlatformCouponEdit">
         ...
         <el-form label-width="130px" style="width:70%" :model="addCoupondata" ref="addCoupondata" :rules="rules">
            <el-form-item class="textinput" label="批次名称" prop="couponBatchName">
                <el-input :disabled="true" v-model="addCoupondata.couponBatchName" maxlength="30"></el-input>
            </el-form-item>
            <el-form-item class="textinput" label="优惠券名称" prop="couponName">
                <el-input :disabled="true" v-model="addCoupondata.couponName" maxlength="30"></el-input>
            </el-form-item>
            ...
            <el-form-item label="酒店名称" prop="hotelRelationFlag">
                 <el-radio @change="changeHotel" :disabled="true" v-model="addCoupondata.hotelRelationFlag" :label="0">全部可用</el-radio>
                 <div>
                    <el-radio @change="changeHotel" :disabled="true" v-model="addCoupondata.hotelRelationFlag" :label="2">指定不可用</el-radio>
                    ...
                    <el-table v-if="addCoupondata.hotelRelationFlag=='2'" :data="addCoupondata.notUseHotel" border stripe style="width:500px">
                       ...
                       <el-table-column prop="id" label="操作" align="center">
                           <template slot-scope="scope">
                              <el-button type="text" :disabled="true" @click="delnouserhotel(scope.$index,scope.row)">删除</el-button>
                           </template>
                       </el-table-column>
                    </el-table>
                 </div>
                 <div>
                    <el-radio @change="changeHotel" :disabled="true" v-model="addCoupondata.hotelRelationFlag" :label="1">指定多个可以</el-radio>
                    ...
                    <el-table v-if="addCoupondata.hotelRelationFlag=='1'" :data="addCoupondata.moreUseHotel" border stripe style="width:500px">
                       ...
                       <el-table-column prop="id" label="操作" align="center">
                           <template slot-scope="scope">
                              <el-button type="text" :disabled="true" @click="delmoreuserhotel(scope.$index,scope.row)">删除</el-button>
                           </template>
                       </el-table-column>
                    </el-table>
                 </div>
                 <div>
                    <el-radio @change="changeHotel" :disabled="true" v-model="addCoupondata.hotelRelationFlag" :label="3">指定单个可用</el-radio>
                    <el-form-item v-if="addCoupondata.hotelRelationFlag=='3'" label="酒店名称" prop="hotelObj">
                       <el-select :disabled="true" v-model="addCoupondata.batchHotelRelation.hotelId"  @change = "selectOneHotel"
            ...
            <el-form-item label="场景" prop="sceneCodes">
               <el-select :disabled="true" class="scene" multiple collapse-tags v-model="addCoupondata.sceneCodes" placeholder="请选择">
                   <el-option
            ...
            <el-form-item label="类型" prop="couponLimit">
                <el-radio @change="changecouponLimit" :disabled="true" v-model="addCoupondata.couponLimit" :label="0">唯一券</el-radio>
                <div>
                   <el-radio @change="changecouponLimit" :disabled="true" v-model="addCoupondata.couponLimit" :label="1">分组券</el-radio>
                   <el-form-item v-if="addCoupondata.couponLimit=='1'" label="选择分组" prop="groupId">
            ...
            <el-form-item v-if="addCoupondata.discountWay == 1" label="使用门槛" prop="useLimitMoney">
                满 <el-input v-model.number="addCoupondata.useLimitMoney" :disabled="true" placeholder="使用门槛需高于金额" style="width:334px;margin-right:5px"></el-input>元
            </el-form-item>
            <el-form-item v-if="addCoupondata.discountWay == 1" label="优惠券金额" prop="reduceMoney">
                <el-input v-model.number="addCoupondata.reduceMoney" :disabled="true" placeholder="请输入优惠券金额" style="width:349px;margin-right:5px"></el-input>元
            </el-form-item>
            <el-form-item v-if="addCoupondata.discountWay == 2" prop="couponDiscount">
                <span slot="label"><label class="required-icon">*</label> 折扣</span>
                <el-input v-model="addCoupondata.couponDiscount" :disabled="true" maxlength="10" placeholder="请输入折扣" style="width:349px;margin-right:5px" oninput ="value=value.replace(/[^0-9.]/g,'')"></el-input>%
            </el-form-item>
            <el-form-item v-if="addCoupondata.discountWay == 2" label="最高优惠金额" prop="discountMaxMoney">
                <el-input v-model="addCoupondata.discountMaxMoney" :disabled="true" maxlength="10" placeholder="请输入最高优惠金额" style="width:349px;margin-right:5px" oninput ="value=value.replace(/[^0-9.]/g,'')"></el-input>元
            </el-form-item>
            <el-form-item v-if="addCoupondata.discountWay == 2" label="最低消费金额" prop="discountMinBuyMoney">
                <el-input v-model="addCoupondata.discountMinBuyMoney" :disabled="true" maxlength="10" placeholder="请输入最低消费金额" style="width:349px;margin-right:5px" oninput ="value=value.replace(/[^0-9.]/g,'')"></el-input>元
            </el-form-item>
            ...
            <el-form-item label="使用有效期" prop="couponTermType">
               <div>
                  <el-radio :disabled="true" @change="changecouponTermType" v-model="addCoupondata.couponTermType" :label="0">领取后</el-radio>
                    <el-form-item v-if="addCoupondata.couponTermType =='0'" prop="couponTermAfterDay" class="couponTermType">
                      <el-select :disabled="true" v-model="addCoupondata.couponTermAfterDay">
                    ...
                    <el-form-item v-if="addCoupondata.couponTermType=='0'" prop="couponTermDays" class="couponTermType">
                        <el-input :disabled="true" v-model.number="addCoupondata.couponTermDays"></el-input> 天内有效
                    </el-form-item>
               </div>
               <div style="margin-top:10px;">
                  <el-radio :disabled="true" @change="changecouponTermType" v-model="addCoupondata.couponTermType" :label="1">固定日期</el-radio>
            ...
            <el-form-item label="领取/发放限制">
               <el-checkbox :disabled="true" @change="changecanDraw" v-model="addCoupondata.canDraw" :true-label="1" :false-label="0">可领取</el-checkbox>
               <el-form-item prop="drawWays">
                   <span slot="label"><label class="required-icon">*</label> 领取渠道</span>
                   <el-select :disabled="true" multiple collapse-tags v-model="addCoupondata.drawWays">
                      <el-option
               ...
               <el-form-item label="每人领取数量" style="margin-top:20px;" prop="drawCountPerUser">
                   <el-input :disabled="true" v-model.number="addCoupondata.drawCountPerUser" style="width:218px;margin-right:5px"></el-input>张
               </el-form-item>
               <el-checkbox :disabled="true" @change="changecanGive" v-model="addCoupondata.canGive" :true-label="1" :false-label="0">可发放</el-checkbox>
               <el-form-item label="发放总数量" style="margin-top:20px;" prop="giveCountTotal">
                   <el-input :disabled="true" v-model.number="addCoupondata.giveCountTotal" style="width:218px;margin-right:5px"></el-input>张
               </el-form-item>
               <el-form-item label="每人发放数量" style="margin-top:20px;" prop="giveCountPerUser">
                   <el-input :disabled="true" v-model.number="addCoupondata.giveCountPerUser"  style="width:218px;margin-right:5px"></el-input>张
               </el-form-item>
            </el-form-item>
            <el-form-item label="可售卖" prop="canSell">
              <el-switch :disabled="true" v-model="addCoupondata.canSell"></el-switch>
            </el-form-item>
            <el-form-item label="可发送" prop="canGift">
              <el-switch :disabled="true" v-model="addCoupondata.canGift"></el-switch>
            </el-form-item>
            <el-form-item>
...
export default {
    ...
    methods:{
        ...
        cancelBtn(){
          ...
          this.$router.push({name:"LonganPlatformCoupon",query:{query}})
        },
        //switch转换
        switchFunc(val) {
            ...
            if(valid){
               this.$api.editCouponBatch(params,that.batchId).then(response=>{
                 if(response.data.code=='0'){
                    ...
                    that.$router.push({name:"LonganPlatformCoupon"})
                 }else{
