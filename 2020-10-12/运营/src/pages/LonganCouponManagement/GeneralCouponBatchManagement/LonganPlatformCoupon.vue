<template>
  <div class="PlatformCoupon">
    ...
    <div class="alignleft">
      <el-button
        class="addbutton"
        v-if="authzData['F:BO_COUPON_CURRBATCH_ADD']"
        @click="addbatch"
      >新增优惠券批次</el-button>
    </div>
    <el-table :data="batchData" border stripe style="width:100%">
      ...
      <el-table-column prop="isActive" label="禁/启用状态" align="center">
        <template slot-scope="scope">
          <el-switch
            v-if="authzData['F:BO_COUPON_CURRBATCH_SWITCH']"
      ...
      <el-table-column fixed="right" prop="id" label="操作" align="center" min-width="300px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_COUPON_CURRBATCH_MODIFY']"
            type="text"
            @click="editBatch(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_COUPON_CURRBATCH_DELETE']"
            type="text"
            @click="delBatch(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BO_COUPON_CURRBATCH_DETAIL']"
            type="text"
            @click="checkBtn(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="scope.row.canGive=='1' && authzData['F:BO_COUPON_CURRBATCH_GRANT']"
            :disabled="scope.row.isActive=='0'"
            type="text"
            @click="grant(scope.row.id,scope.row.couponBatchName)"
          >发放</el-button>
          <!-- <el-button
            v-if="authzData['F:BO_COUPON_CURRBATCH_ORDER']"
            type="text"
    ...
    <el-dialog title="发放" :visible.sync="datchdialog" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BO_COUPON_CURRBATCH_GRANTSUBMIT']"
          type="primary"
