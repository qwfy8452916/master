<template>
  <div class="GrantCouponRecord">
    ...
    <div class="alignleft">
      <el-button class="addbutton" type="primary" @click="grantBtn">发放</el-button>
    </div>
    <el-table :data="GrantRecordData" border stripe style="width:100%">
      ...
      <el-table-column fixed="right" prop="id" label="操作" min-width="80px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_COUPON_GRANT_GRANTRECORD']"
            type="text"
            @click="grantRecord(scope.row.id)"
          >发放记录</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="发放" :visible.sync="datchdialog" width="30%" class="datchdialog">
      <el-form label-width="100px">
        <el-form-item prod="discountWay">
          <span slot="label">
            <label class="required-icon">*</label> 优惠方式
          </span>
        ...
        <el-form-item prod="batchId">
          <span slot="label">
            <label class="required-icon">*</label> 批次名称
          </span>
          <el-select
            v-model="diaGrantData.batchId"
            filterable
            remote
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BO_COUPON_GRANT_SUBMIT']"
          type="primary"
          @click="sureGrantBtn"
        >确 定</el-button>
      </span>
