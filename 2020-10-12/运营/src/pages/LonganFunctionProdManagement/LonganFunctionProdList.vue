<template>
  <div class="functionlist">
    ...
    <div v-if="authzData['F:BO_PROD_FUNCPROD_ADD']">
      <el-button class="addbutton" @click="functionProdAdd">添&nbsp;&nbsp;加</el-button>
    </div>
    <el-table :data="functionProdDataList" border stripe style="width:100%;">
      ...
      <el-table-column
        v-if="authzData['F:BO_PROD_FUNCPROD_USING']"
        prop="onShelfProd"
        label="上下架"
        min-width="80px"
        align="center"
      >
        <template slot-scope="scope">
          <el-switch
            v-model="scope.row.onShelfProd"
            @change="updateStatus(scope.row.id, scope.row.onShelfProd)"
          ></el-switch>
        </template>
      </el-table-column>
      <el-table-column
        prop="hotelProduct.prodSupplyPrice"
        label="供货价"
        min-width="80px"
        align="center"
      ></el-table-column>
      <el-table-column
        prop="hotelProduct.prodRetailPrice"
        label="零售价"
        min-width="80px"
        align="center"
      ></el-table-column>
      ...
      <el-table-column
        prop="hotelProduct.availableSaleQty"
        label="可售数量"
        min-width="80px"
        align="center"
      ></el-table-column>
      <el-table-column
        prop="hotelProduct.reviewStatus"
        label="审核状态"
        min-width="80px"
        align="center"
      >
      ...
      <el-table-column fixed="right" label="操作" width="200px" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="authzData['F:BO_PROD_FUNCPROD_EDIT']"
            type="text"
            size="small"
            @click="funcProdModify(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_PROD_FUNCPROD_DELETE']"
            type="text"
            size="small"
            @click="funcProdDelete(scope.row.id)"
          >移除</el-button>
          <el-button
            v-if="scope.row.hotelProduct.isSupportSpec == 1"
            type="text"
            size="small"
            @click="funcProdSpecs(scope.row.id)"
          >规格管理</el-button>
        </template>
      </el-table-column>
    </el-table>
