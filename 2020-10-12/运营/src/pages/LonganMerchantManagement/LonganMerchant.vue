<template>
  <div class="LonganMerchant">
    ...
    <div v-if="authzData['F:BO_MER_MER_ADD']">
      <el-button class="addbutton" @click="addMerchant">新&nbsp;&nbsp;增</el-button>
    </div>
    <el-table :data="LonganMerchantDataList" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" min-width="160px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_MER_MER_EDIT']"
            type="text"
            size="small"
            @click="changeMerchant(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_MER_MER_DELETE']"
            type="text"
            size="small"
            @click="deleteshow(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BO_MER_MER_RESETPWD']"
            type="text"
            size="small"
            @click="changepwdshow(scope.row.id)"
          >重置密码</el-button>
        </template>
      </el-table-column>
    </el-table>
