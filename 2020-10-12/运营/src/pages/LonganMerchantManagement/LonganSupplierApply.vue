  ...
  <div class="supplierapply">
    ...
    <el-table :data="supplierapplyDataList" border stripe style="width:100%;">
      ...
      <el-table-column label="操作" width="120px" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.reviewStatus=='0' && authzData['F:BO_SUPPL_SUPPLIERAPPLY_TRIAL']"
            type="text"
            size="small"
            @click="handle(scope.$index, supplierapplyDataList)"
          >审核</el-button>
          <el-button
            v-if="authzData['F:BO_SUPPL_SUPPLIERAPPLY_CHECKDETAIL']"
            type="text"
            size="small"
            @click="lookdetail(scope.$index, supplierapplyDataList)"
          >详情</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    supplierExamine(reviewResult) {
      ...
      let params = "";
      this.$api
        .supplierExamine({ params }, that.id, reviewResult)
        .then((response) => {
