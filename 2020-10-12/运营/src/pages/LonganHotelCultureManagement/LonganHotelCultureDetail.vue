<template>
    <div class="servicetypelist">
        ...
        <div v-if="authzData['F:BO_HOTEL_CULTURE_STORY_ADD']"><el-button class="addbutton" @click="hotelCultureDetailAdd">新增</el-button></div>
        <el-table :data="HotelCultureDetailDataList" border stripe style="width:100%;" >
            ...
            <el-table-column label="操作" width="180px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_HOTEL_CULTURE_STORY_EDIT']" type="text" size="small" @click="hotelCultureDetailModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BO_HOTEL_CULTURE_STORY_DELETE']" type="text" size="small" @click="hotelCultureDetailDelete(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        hotelCultureDetailAdd(){
            const cid  = this.hcId;
            this.$router.push({name:'LonganHotelCultureDetailAdd', query: {cid}});
        },
        //修改
        hotelCultureDetailModify(id){
            const cid = this.hcId;
            this.$router.push({name:'LonganHotelCultureDetailModify', query: {id, cid}});
        },
        //删除
