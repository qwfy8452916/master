<template>
  <div class="servicetypelist">
    ...
    <div v-if="authzData['F:BO_HOTEL_CULTURE_ADD']">
      <el-button class="addbutton" @click="hotelCultureAdd">新增</el-button>
    </div>
    <el-table :data="hotelCultureDataList" border stripe style="width:100%;">
      ...
      <el-table-column label="操作" width="180px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_HOTEL_CULTURE_EDIT']"
            type="text"
            size="small"
            @click="hotelCultureModify(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_CULTURE_DELETE']"
            type="text"
            size="small"
            @click="hotelCultureDelete(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_CULTURE_VIEW']"
            type="text"
            size="small"
            @click="hotelCultureDetail(scope.row.id)"
          >内容设置</el-button>
        </template>
      </el-table-column>
    </el-table>
