<template>
    <div class="platdeliverydetail">
        ...
        <table cellpadding="0" cellspacing="0" class="deliveryTable">
            ...
            <tr v-if="deliveryDateDetail.status != 0">
                <td class="subTitle">确认时间</td>
                <td class="subcont">{{deliveryDateDetail.confirmTime =='1970-01-01 00:00:00'?"":deliveryDateDetail.confirmTime}}</td>
            </tr>
            ...
            <tr v-if="deliveryDateDetail.delivWay == 1 && deliveryDateDetail.status != 0">
                <td class="subTitle">配送时间</td>
                <td class="subcont">{{deliveryDateDetail.shipmentsTime =='1970-01-01 00:00:00'?"":deliveryDateDetail.shipmentsTime}}</td>
            </tr>
            ...
            <tr v-if="deliveryDateDetail.lgcHotelId != 0">
                ...
                <td class="subcont">{{deliveryDateDetail.lgcFinishTime =='1970-01-01 00:00:00'?"":deliveryDateDetail.lgcFinishTime}}</td>
            </tr>
        </table>
        <el-table :data="deliveryDateDetail.orderDeliveryDetailDTOList" border style="width:100%;">
            ...
            <el-table-column prop="refundTime" label="退款时间" min-width="120px" align=center>
                <template slot-scope="scope">
                    <span>{{scope.row.refundTime == '1970-01-01'?'':scope.row.refundTime}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="returnTime" label="退货时间" min-width="120px" align=center>
                <template slot-scope="scope">
                    <span>{{scope.row.returnTime == '1970-01-01'?'':scope.row.returnTime}}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" width="160px" align=center>
                <template slot-scope="scope">
                    ...
                    <el-button type="text" size="small" :disabled="deliveryDateDetail.orderStatus==0?true:false" @click="divideDetail(scope.row.id)">分成详情</el-button>
                </template>
            </el-table-column>
        </el-table>
        <br/><br/>
        ...
        <el-dialog title="分成详情" :visible.sync="divideDetaildialog" v-if="divideDetaildialog" class="dividedialog">
            <el-form align="left" label-width="120px" :model="divideDetailData" ref="LogisticsEditData">
               <div v-if="divideDetailData.orderType==1">
                  ...
                  <template v-for="(item,key) in divideDetailData.orderDetailAllocDetailDTOS">
                    <div :key="key" v-if="item.deductObj=='酒店' || item.deductObj=='供应商'" style="margin-bottom:5px">
