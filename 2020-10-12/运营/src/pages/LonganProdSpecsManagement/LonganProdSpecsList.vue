<template>
  <div class="prodspecs">
    ...
    <div>
      <el-button class="addbutton" @click="prodSpecsAdd">新增规格</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    PlatformCommodityDetail() {
      const params = {};
      this.$api
        .PlatformCommodityDetail(params, this.pId)
        .then((response) => {
    ...
    prodSpecsAdd() {
      ...
      this.$router.push({ name: "LonganProdSpecsAdd", query: { prodId } });
    },
    //详情
    prodSpecsDetail(id) {
      this.$router.push({ name: "LonganProdSpecsDetail", query: { id } });
    },
    //修改
    prodSpecsModify(id) {
      this.$router.push({ name: "LonganProdSpecsModify", query: { id } });
    },
    //删除
    prodSpecsDelete(id) {
      this.psId = id;
    ...
    deleteConfirm() {
      const params = {};
      this.$api
        .prodSpecsDelete(params, this.psId)
        .then((response) => {
          // console.log(response);
          const result = response.data;
          if (result.code == "0") {
