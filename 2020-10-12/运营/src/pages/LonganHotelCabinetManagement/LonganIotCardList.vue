<template>
  <div class="iotCardList">
    ...
    <el-table :data="dataList" border stripe style="width:100%">
      ...
      <el-table-column label="禁/启用" align="center">
        <template slot-scope="scope">
          <el-switch
            v-if="authzData['F:BO_CAB_IOTCARD_SWITCH']"
            v-model="scope.row.isActive"
            @change="changeStatus(scope.row.id,scope.row.isActive,scope.$index)"
          ></el-switch>
        </template>
      </el-table-column>
      <el-table-column label="禁用原因" align="center">
        <template slot-scope="scope">{{scope.row.isActive?'-':scope.row.forbiddenReason}}</template>
      </el-table-column>
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="authzData['F:BO_CAB_IOTCARD_MODIFY']"
            @click="changeiotcard(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_CAB_IOTCARD_DELETE']"
            type="text"
            @click="deliotcard(scope.row.id)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      title="新增"
      @close="dialogcreatedit=false;formdata.iotcardval=''"
      :visible.sync="dialogcreatedit"
      width="40%"
      center
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          v-if="authzData['F:BO_CAB_IOTCARD_ADDSUBMIT']"
          type="primary"
    ...
    <el-dialog
      title="修改"
      @close="dialogchangeEdit=false;formdata.iotcardval=''"
      :visible.sync="dialogchangeEdit"
      width="40%"
      center
    >
      <el-form :model="formdata" :rules="rules" ref="formdata">
...
export default {
  ...
  methods: {
    ...
    sureChange() {
      ...
      this.$api
        .changeControlCcid(params, this.selectID)
        .then((response) => {
          if (response.data.code == 0) {
            this.$message.success("操作成功");
