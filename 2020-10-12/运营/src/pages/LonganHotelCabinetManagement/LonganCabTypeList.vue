    ...
    <el-table :data="cabinetTypeData" border stripe style="width:100%">
      ...
      <el-table-column label="操作" align="center" width="200px">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="authzData['F:BO_CAB_CABTYPE_MODIFY']"
            @click="editCabType(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_CAB_CABTYPE_DELETE']"
            type="text"
            @click="deleBtn(scope.row.id)"
          >删除</el-button>
          <el-button
            type="text"
            v-if="authzData['F:BO_CAB_CABTYPE_DETAIL']"
...
export default {
  ...
  methods: {
    ...
    confirmBtn() {
      ...
      let params = "";
      this.$api
        .delteCabinetType(params, that.cabinetTypeId)
        .then((response) => {
