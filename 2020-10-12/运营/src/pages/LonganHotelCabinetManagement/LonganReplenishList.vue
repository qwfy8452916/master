<template>
  <div class="ReplenishList">
    ...
    <el-table :data="replenishData" border stripe style="width:100%">
      ...
      <el-table-column prop label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="authzData['F:BO_CAB_REPLENISH_DETAIL']"
            @click="checkDetail(scope.$index,replenishData)"
          >详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      :title="diatitle.hotelName+'-'+diatitle.roomFloor+'楼'+diatitle.roomCode"
      ...
      <el-table :data="latticeData" border stripe>
        ...
        <el-table-column prop="prodName" label="商品名称" align="center">
          <template slot-scope="scope">
            ...
            <span
              v-if="scope.row.opFlag!='3' && scope.row.opFlag!='0'"
            >{{scope.row.prodProductDTO.prodName}}</span>
          </template>
        </el-table-column>
      ...
      <div class="wraptotal">
        ...
        <div>
          <span
            ...
            v-for="(item,key) in latticeData"
            :key="key"
          >
            <span
              v-if="item.prodProductDTO!=null && item.opFlag!='3' && item.opFlag!='0'"
            >{{item.prodProductDTO.prodName}}*{{item.prodCount}}</span>
            <span
              v-if="item.changeProdProductDTO!=null && item.opFlag=='3'"
            >{{item.changeProdProductDTO.prodName}}*{{item.changeProdCount}}</span>
          </span>
        </div>
      </div>
    </el-dialog>
