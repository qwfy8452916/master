<template>
  <div class="functionadd">
    <el-form
      :model="FunctionProdData"
      :rules="rules"
      ref="FunctionProdData"
      label-width="100px"
      class="functionform"
    >
      ...
      <el-form-item label="商品" prop="hotelProdId" @change="selectProd">
        <!-- <el-select 
                    :disabled="true"
        ...
        <el-select :disabled="true" v-model="FunctionProdData.hotelProdId" placeholder="请选择">
          <el-option
      ...
      <el-form-item prop="prodType">
        <span slot="label">
          <label class="required-icon">*</label> 商品形式
        </span>
      ...
      <el-form-item prop="prodWarrantyPeriod">
        <span slot="label">
          <label class="required-icon">*</label> 保质期
        </span>
      ...
      <el-form-item prop="prodRetailPrice">
        <span slot="label">
          <label class="required-icon">*</label> 零售价
        </span>
      ...
      <el-form-item prop="availableSaleQty">
        <span slot="label">
          <label class="required-icon">*</label> 可售数量
        </span>
      ...
      <!-- <el-form-item v-if="isPickUp" label="自提点" prop="pickUpPointIds">
            ...
            <el-form-item label="排序" prop="sort">
                    <el-input :disabled="true" v-model.number="FunctionProdData.sort"></el-input>
                </el-form-item>
            <el-form-item label="分成协议" prop="allocId">
                <el-select :disabled="true" v-model="FunctionProdData.allocId" placeholder="请选择">
                    <el-option
    ...
    <div>
      <el-button class="addbutton" @click="funcProdSpecsAdd">添加规格</el-button>
    </div>
    <div>
      <el-button class="addbutton" @click="funcProdSpecsAllAdd">添加全部规格</el-button>
    </div>
    ...
    <el-dialog title="提示" :visible.sync="dialogVisibleAllAdd" width="30%">
      <span>
        确定要添加全部酒店商品规格吗？
        <br />确定后将清除此功能区商品所有的规格并且复制酒店商品规格
      </span>
      <span slot="footer">
  ...
  methods: {
    ...
    hotelChange() {
      if (this.FunctionProdData.hotelId == "") {
        ...
        this.functionList = [];
        this.FunctionProdData.prodName = "";
        this.prodList = [];
      } else {
        this.getFunctionList();
        this.getProdList();
        ...
        .then((response) => {
          ...
          if (result.code == 0) {
            ...
            const addProd = {
              ...
              vouBatchName: that.FunctionProdData.vouBatchName,
        ...
        .then((response) => {
          ...
          if (result.code == "0") {
            ...
            this.FunctionProdData = {
              ...
              vouBatchName: result.data.hotelProduct.vouBatchName,
              ...
              allocId: result.data.allocId,
            };
            if (result.data.marketCategories != null) {
              this.selectMarketIdList = result.data.marketCategories.map(
                (item) => {
                  return item.id;
                }
              );
              ...
              for (let i = 0; i < result.data.marketCategories.length; i++) {
                treeNameArr +=
                  result.data.marketCategories[i].categoryName + "、";
              }
              this.selectMarketData = treeNameArr;
            }
            this.getFunctionClassify();
    ...
    allAddConfirm() {
      const params = {};
      this.$api
        .funcProdSpecsAllAdd(params, this.fpId)
        .then((response) => {
          // console.log(response);
          const result = response.data;
          if (result.code == "0") {
    ...
    deleteConfirm() {
      const params = {};
      this.$api
        .funcProdSpecsDelete(params, this.fpsId)
        .then((response) => {
          // console.log(response);
          const result = response.data;
          if (result.code == "0") {
