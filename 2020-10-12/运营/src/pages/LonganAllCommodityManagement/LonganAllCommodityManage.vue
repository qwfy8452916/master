  ...
  <div class="commoditymanage">
    ...
    <el-table :data="CommodityDataList" border stripe style="width:100%;">
      ...
      <el-table-column prop="reviewStatus" label="审核状态" min-width="80px" align="center">
        <template slot-scope="scope">
          <el-tooltip
            ...
            class="item"
            effect="dark"
            :content="scope.row.reviewRemark"
            placement="right"
          >
      ...
      <el-table-column fixed="right" label="操作" width="160px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_PROD_ALLPROD_REVIEWPROGRESS'] && scope.row.reviewStatus == 2"
            type="text"
            size="small"
          ...
          <el-button
            v-if="authzData['F:BO_PROD_ALLPROD_VIEW']"
            type="text"
            size="small"
            @click="allCommodityDetail(scope.row.id)"
          >详情</el-button>
          <el-button
            :disabled="scope.row.specQty>0?false:true"
            type="text"
            size="small"
            @click="prodSpecsDetail(scope.row.id)"
          >规格</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="提示" :visible.sync="dislogVisibleReject" width="30%">
      <el-form :model="RejectForm" :rules="rules" ref="RejectForm">
        <el-form-item prop="causeReject">
          <el-input type="textarea" :row="2" placeholder="请输入驳回原因" v-model="RejectForm.causeReject"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
