  ...
  <div class="Franchisee">
    <el-form :model="query" ref="query" :inline="true" align="left" class="searchform">
      <el-form-item label="合作伙伴名称" prop="allyId">
        ...
        >
          ...
          <el-option
            v-for="item in selectpartnerlist"
            :key="item.index"
            :label="item.name"
            :value="item.id"
          ></el-option>
    ...
    <el-table :data="FranchiseeDataList" border stripe style="width:100%;">
      ...
      <el-table-column label="操作" align="center" fixed="right" width="240px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_ALLY_HOTEL_DELETE']"
            type="text"
            size="small"
            @click="remove(scope.row.id)"
          >移除</el-button>
          <el-button
            v-if="authzData['F:BO_ALLY_HOTEL_VIEW']"
            type="text"
            size="small"
            @click="lookdetail(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="scope.row.hotelAs=='8' && authzData['F:BO_ALLY_PARTNER_MANAGECABINET']"
            type="text"
            size="small"
            @click="choicecabinet(scope.row.id,scope.row.hotelId,scope.row.hotelAs)"
          >管理柜子</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    sureRemove() {
      ...
      let params = "";
      this.$api
        .Removehotelpartner({ params }, that.removeId)
        .then((response) => {
