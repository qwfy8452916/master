<template>
  <div class="Franchisee">
    ...
    <div v-if="authzData['F:BO_ALLY_ALLY_ADD']" class="addranchisee">
      <el-button class="addbutton" @click="addranchisee">新增</el-button>
    </div>
    <el-table :data="FranchiseeDataList" border stripe style="width:100%;">
      ...
      <el-table-column prop="addressAll" label="区域" align="center" width="320px">
        <template slot-scope="scope">
          <span v-if="scope.row.provinceName!=null">{{scope.row.provinceName.dictName}}</span>
          <span v-if="scope.row.cityName!=null">{{scope.row.cityName.dictName}}</span>
          <span v-if="scope.row.areaName!=null">{{scope.row.areaName.dictName}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" fixed="right" width="320px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_ALLY_ALLY_EDIT'] && scope.row.isDeleted=='1'"
            :disabled="true"
            type="text"
            size="small"
          ...
          >修改</el-button>
          <el-button
            v-else-if="authzData['F:BO_ALLY_ALLY_EDIT']"
            type="text"
            size="small"
            @click="edit(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_ALLY_ALLY_USING']"
            type="text"
            size="small"
            @click="prohibit(scope.$index,FranchiseeDataList)"
          >
            <span v-if="scope.row.isDeleted=='1'">启用</span>
            <span v-if="scope.row.isDeleted=='0'">禁用</span>
          </el-button>
          <el-button
            v-if="authzData['F:BO_ALLY_ALLY_RESETPWD'] && scope.row.isDeleted=='1'"
            :disabled="true"
            type="text"
            size="small"
          ...
          >重置密码</el-button>
          <el-button
            v-else-if="authzData['F:BO_ALLY_ALLY_RESETPWD']"
            type="text"
            size="small"
            @click="reset(scope.row.id)"
          >重置密码</el-button>
          <el-button
            v-if="authzData['F:BO_ALLY_PARTNER_SETTING'] && scope.row.isDeleted=='1'"
            :disabled="true"
            type="text"
            size="small"
            @click="partnersetup(scope.row.id,scope.row.name)"
          >合作伙伴设置</el-button>
          <el-button
            v-else-if="authzData['F:BO_ALLY_PARTNER_SETTING']"
            type="text"
            size="small"
            @click="partnersetup(scope.row.id,scope.row.name)"
          >合作伙伴设置</el-button>
          <el-button
            v-if="authzData['F:BO_ALLY_ALLY_VIEW'] && scope.row.isDeleted=='1'"
            :disabled="true"
            type="text"
            size="small"
          ...
          >详情</el-button>
          <el-button
            v-else-if="authzData['F:BO_ALLY_ALLY_VIEW']"
            type="text"
            size="small"
            @click="lookdetail(scope.row.id)"
          >详情</el-button>
...
export default {
  ...
  methods: {
    ...
    sureswitch(id) {
      ...
      let params = "";
      this.$api
        .prohibitpartner({ params }, that.prohibitId)
        .then((response) => {
    ...
    surereset() {
      ...
      this.$api
        .Resetpassword(params, that.resetId)
        .then((response) => {
          if (response.data.code == 0) {
            that.$message.success("操作成功");
