<template>
  <div class="prodspecs">
    <el-form v-model="ProdDetailData" label-width="100px" class="commodityform">
      ...
      <el-form-item v-if="pType == 'all'" label="类型" prop="prodKindName">
        <el-input :disabled="true" v-model="ProdDetailData.prodKindName"></el-input>
      </el-form-item>
      <el-form-item v-if="pType == 'all'" label="供应商" prop="prodSupplName">
        <el-input :disabled="true" v-model="ProdDetailData.prodSupplName"></el-input>
      </el-form-item>
    ...
    <div>
      <el-button class="addbutton" @click="hotelProdSpecsAdd">添加规格</el-button>
    </div>
    <div>
      <el-button class="addbutton" @click="hotelProdSpecsAllAdd">添加全部规格</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    hotelPlatCommodityDetail() {
      const params = {};
      this.$api
        .hotelPlatCommodityDetail(params, this.hpId)
        .then((response) => {
          // console.log(response);
          const result = response.data;
          if (result.code == "0") {
            this.ProdDetailData = result.data;
            this.ProdDetailData.prodShowName =
              result.data.prodProductDTO.prodShowName;
            this.ProdDetailData.prodKindName =
              result.data.prodProductDTO.prodKindName;
            this.ProdDetailData.prodSupplName =
              result.data.prodProductDTO.prodSupplName;
            this.ProdDetailData.prodWarrantyPeriod =
              result.data.prodProductDTO.prodWarrantyPeriod;
            this.ProdDetailData.prodUnitMeasure =
              result.data.prodProductDTO.prodUnitMeasure;
            this.ProdDetailData.prodSupplyPrice =
              result.data.prodProductDTO.prodSupplyPrice;
            this.ProdDetailData.prodRetailPrice =
              result.data.prodProductDTO.prodRetailPrice;
            this.ProdDetailData.prodAdvisePrice =
              result.data.prodProductDTO.prodAdvisePrice;
    ...
    allAddConfirm() {
      const params = {};
      this.$api
        .hotelProdSpecsAllAdd(params, this.hpId)
        .then((response) => {
          // console.log(response);
          const result = response.data;
          if (result.code == "0") {
