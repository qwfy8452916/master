<template>
  <div class="commoditymanage">
    ...
    <div>
      <el-button class="addbutton" @click="addHotelMerProd">添加酒店入驻商品</el-button>
    </div>
    <el-table :data="CommodityDataList" border stripe style="width:100%;">
      ...
      <el-table-column label="商品形式" min-width="80px" align="center">
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.prodType == 3">菜品</span>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" min-width="200px" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="authzData['F:BO_HOTEL_PROD_EDIT']"
            type="text"
            size="small"
            @click="hotelAllCommodityModify(scope.row.id)"
          >修改</el-button>
          ...
          <el-button
            ...
            type="text"
            size="small"
            @click="hotelAllProdSpecs(scope.row.id)"
          >规格管理</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="dialogProdVisible" :close-on-click-modal="false" width="86%">
      <div
        style="text-align: left; margin: -40px 0px 20px 0px; border-bottom: 1px solid #ddd;padding-bottom: 20px;"
      >
        <span>酒店&nbsp;&nbsp;&nbsp;</span>
        <el-select
          v-model="hotelId"
      ...
      <el-form :model="prodFormData" :rules="prodFormData.rules" ref="prodFormData">
        <el-table
          ...
          style="width:100%;"
        >
          <el-table-column fixed type="selection" :reserve-selection="true" width="40px"></el-table-column>
