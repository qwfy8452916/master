<template>
  <div class="functionlist">
    ...
    <div v-if="authzData['F:BO_HOTEL_FUNCTION_ADD']">
      <el-button class="addbutton" @click="hotelFunctionAdd">新&nbsp;&nbsp;增</el-button>
    </div>
    <el-table :data="hotelFunctionDataList" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" width="260px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_HOTEL_FUNCTION_DETAIL']"
            type="text"
            size="small"
            @click="hotelFunctionDetail(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_FUNCTION_EDIT']"
            type="text"
            size="small"
            @click="hotelFunctionModify(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_FUNCTION_DELETE']"
            type="text"
            size="small"
            @click="hotelFunctionDelete(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_FUNCTION_CLASSIFY']&&scope.row.funcType==4"
            type="text"
            size="small"
            @click="hotelFunctionClassify(scope.row.hotelId, scope.row.hotelName, scope.row.id, scope.row.funcCnName)"
          >分类管理</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_FUNCTION_PROD']&&scope.row.funcType==4"
            type="text"
            size="small"
  ...
  methods: {
    ...
    hotelFunctionClassify(hotelId, hotelName, funcId, funcName) {
      this.$router.push({
        ...
        query: { hotelId, hotelName, funcId, funcName },
      });
    },
    //商品管理
