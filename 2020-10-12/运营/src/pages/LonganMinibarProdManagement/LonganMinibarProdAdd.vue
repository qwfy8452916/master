    ...
    <div class="functionadd">
        ...
        <el-form :model="MiniDeployData" :rules="rules" ref="MiniDeployData" label-width="100px" class="functionform">
            <el-form-item label="酒店名称" prop="hotelId">
                <el-select 
                    v-model="MiniDeployData.hotelId"
                    filterable
                    remote
            ...
            <el-form-item label="柜子类型" prop="cabType">
                <el-select v-model="MiniDeployData.cabType" placeholder="请选择">
                    <el-option v-for="item in cabList" :key="item.id" :label="item.cabName" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item>
                <el-button @click="resetForm">取消</el-button>
                <el-button v-if="authzData['F:BO_PROD_MINIPROD_ADD_SUBMIT']" type="primary" :disabled="isSubmit" @click="submitForm('MiniDeployData')">确定</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        submitForm(MiniDeployData){
            const params = {
                hotelId: this.MiniDeployData.hotelId,
