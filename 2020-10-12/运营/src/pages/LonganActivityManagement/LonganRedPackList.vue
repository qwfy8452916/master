    ...
    <el-table :data="PacketDataList" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" width="100px" align="center">
        <template slot-scope="scope">
          ...
          <el-button
            v-if="authzData['F:BO_ACT_RED_RECORD']"
            type="text"
            size="small"
...
export default {
  ...
  methods: {
    ...
    packetGetRecord(redId) {
      this.$router.push({ name: "LonganRedPackGetRecord", query: { redId } });
    },
    packetGetDetail(id) {
      this.$router.push({ name: "LonganRedpackDetail", query: { id } });
    },
    packetShareRecord(redId) {
      this.$router.push({ name: "LonganRedPackShareRE", query: { redId } });
    },
  },
};
</script>
