...
当设置为“每X月”时，例如每1月，指这个月的月初到月末任何时间都可以参加。下个月仍旧是从月初到月末计算参与时间。">
            ...
            <el-form-item label="酒店名称：" prop="ifALlHotel">
                <el-radio-group :disabled="Commoditygai.activityType == 2" class="radio-group" v-model="Commoditygai.ifALlHotel">
                    <el-radio class="radioItem" :disabled='Commoditygai.grade == 1' :label="1">全部酒店</el-radio>
                    <el-radio class="radioItem" :label="0">选择酒店
                        <div class="choose">
                            <el-button :disabled="Commoditygai.activityType == 2" type="primary" @click="chooseHotel">选择酒店</el-button>
                        </div>
                        <div class="hotelTable">
                            ...
                            <el-table-column label="操作" align="center">
                                <template slot-scope="scope">
                                    <el-button :disabled="Commoditygai.activityType == 2" type="text" size="small" @click="hotelCancel(scope.$index)">删除</el-button>
                                </template>
                            </el-table-column>
                            </el-table>
                        </div>
        ...
        getFillbackData(){
            ...
            this.$api.selectActivityOne(this.actID).then(response => {
                if(response.data.code == 0){
                    ...
                    this.pickerOptions = {
                        disabledDate: (time) => {
                            if(new Date(response.data.data.actBegin)>Date.now()){
                                return time.getTime() < (Date.now()-24*60*60*1000);
                            }else{
                                return time.getTime() < new Date(response.data.data.actBegin);
                            }
                        }
                    }
