<template>
  <div>
    <el-form :inline="true" align="left" class="searchform">
      <el-form-item label="酒店名称">
        <el-select v-model="hotelId" placeholder="请选择">
          <el-option v-for="item in hotelList" :key="item.id" :label="item.label" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
    ...
    <div v-if="authzData['F:BO_FS_CAB_ADD']">
      <el-button class="addbutton" @click="addNewSetting">新增投放柜子</el-button>
    </div>
    <el-table :data="CabinetList" border stripe style="width:100%;">
      ...
      <el-table-column label="首发时间" align="center">
        <template
          slot-scope="scope"
        >{{scope.row.launchTime=='1970-01-01 00:00:00'?'-':scope.row.launchTime}}</template>
      </el-table-column>
      <el-table-column prop="bindingPeopleName" label="绑定人" align="center">
        <template
          slot-scope="scope"
        >{{scope.row.bindingPeopleName== null?'-':scope.row.bindingPeopleName}}</template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            v-if="authzData['F:BO_FS_CAB_EDIT'] && scope.row.status=='未绑定'"
            @click="Cabinetglchange(scope.$index, CabinetList)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_FS_CAB_DELETE']"
            type="text"
            size="small"
