<template>
  <div>
    ...
    <div v-if="authzData['F:BO_FS_CABTYPE_ADD']">
      <el-button class="addbutton" @click="addNewSetting">新增柜子类型</el-button>
    </div>
    <el-table :data="CabinetList" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_FS_CABTYPE_EDIT']"
            type="text"
            size="small"
            @click="CabinetTypechange(scope.$index, CabinetList)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_FS_CABTYPE_DELETE']"
            type="text"
            size="small"
            @click="CabinetTypedelete(scope.$index, CabinetList)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
