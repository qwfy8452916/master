<template>
  <div>
    ...
    <div v-if="authzData['F:BO_FS_MEMBER_ADD']">
      <el-button class="addbutton" @click="addNewSetting">新增业务员</el-button>
    </div>
    <el-table :data="CabinetList" border stripe style="width:100%;">
      ...
      <el-table-column label="类型" align="center">
        <template slot-scope="scope">{{scope.row.isSalesman?'业务员':'会员'}}</template>
      </el-table-column>
      <el-table-column label="所属社群" align="center">
        <template slot-scope="scope">{{scope.row.upperShareCode ? scope.row.upperShareCode:'-'}}</template>
      </el-table-column>
      <el-table-column label="是否是财富合伙人" align="center">
        <template
          slot-scope="scope"
        >{{scope.row.isFortunePartner == 2?'是(高级)':scope.row.isFortunePartner?"是":'否'}}</template>
      </el-table-column>
      <el-table-column label="身份" width="160" align="center">
        <template
          slot-scope="scope"
        >{{scope.row.specialEnvoyLevel?scope.row.specialEnvoyLevel== '-1'?'普通会员(可升级)':'财富特使':'普通会员'}}</template>
      </el-table-column>
      <el-table-column label="财富特使等级" align="center">
        <template
          slot-scope="scope"
        >{{scope.row.specialEnvoyLevel>0?scope.row.specialEnvoyLevel:'-'}}</template>
      </el-table-column>
      <el-table-column v-if="authzData['F:BO_FS_MEMBER_SHARE']" label="分享" align="center">
        <template slot-scope="scope">
          <span v-if="!scope.row.isSalesman">-</span>
          <el-switch
            @change="changeMemberShare(scope.row.shareFlag,scope.row.id)"
            v-else
      ...
      <el-table-column label="账号余额" align="center">
        <template slot-scope="scope">{{scope.row.fsBalanceDTO?scope.row.fsBalanceDTO.balance:'-'}}</template>
      </el-table-column>
      <el-table-column v-if="authzData['F:BO_FS_MEMBER_USING']" label="禁/启用" align="center">
        <template slot-scope="scope">
          <span v-if="!scope.row.isSalesman">-</span>
          <el-switch
            @change="changeMemberStatus(scope.row.status,scope.row.id,scope.$index)"
            v-else
      ...
      <el-table-column label="注册时间" align="center">
        <template
          slot-scope="scope"
        >{{scope.row.loginTime=='1970-01-01 00:00:00'?'-':scope.row.loginTime}}</template>
      </el-table-column>
      <el-table-column fixed="right" label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_FS_MEMBER_EDIT'] && scope.row.isSalesman"
            type="text"
            size="small"
            @click="CabinetTypechange(scope.$index, CabinetList)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_FS_MEMBER_ASSOCIATION'] && scope.row.specialEnvoyLevel"
            type="text"
            size="small"
            @click="CabinetTypeCom(scope.$index, CabinetList)"
          >财富特使社群</el-button>
          <el-button
            v-if="authzData['F:BO_FS_MEMBER_PARTNERRECORD'] && !scope.row.specialEnvoyLevel"
            type="text"
            size="small"
            @click="CabinetTypeComRe(scope.$index, CabinetList)"
          >财富合伙人投资记录</el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    CabinetTypeCom(index, row) {
      ...
      this.$router.push({
        ...
        query: { modifyid: guiId, fullName: fullName },
      });
    },
    //财富合伙人投资记录
