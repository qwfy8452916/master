<template>
    <div class="servicetypedetail">
        <div v-if="authzData['F:BO_RMSVC_TYPE_DETAILTPL_ADDONE']" class="addlevelone"><el-button type="primary" @click="levelOneAdd">新增一级条目</el-button></div>
        <el-tree
            :data = "typeDataDetail"
            node-key = 'id'
            default-expand-all
...
export default {
    ...
    methods: {
        ...
        getServiceTypeDetail(){
            ...
            this.$api.getServiceTypeDetail(params)
                .then(response => {
        ...
        modify(data){
            ...
            const params = {};
            this.$api.stlevelOneDetail(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data; 
                    if(result.code == '0'){
        ...
        ensureModify(SubclausesDataModify){
            ...
            this.$refs[SubclausesDataModify].validate((valid) => {
                if(valid){
                    ...
                    // console.log(params);
                    this.$api.stlevelOneModify(params, id)
                        .then(response => {
                            // console.log(response);
                            const result = response.data; 
                            if(result.code == '0'){
        ...
        ensureDelete(){
            ...
            const params = {};
            this.$api.stlevelOneDelete(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data; 
                    if(result.code == '0'){
                        if(result.data == true){
                            this.getServiceTypeDetail();
                            // const parentD = that.deleteNode.parent.data;
                            // if(!parentD.children){   // 一级
                            //     const indexD = parentD.findIndex(d => d.id === that.deleteData.id);
                            //     parentD.splice(indexD, 1);
                            // }else{   
                            //     const childrenD = parentD.children;
                            //     const indexD = childrenD.findIndex(d => d.id === this.deleteData.id);
                            //     childrenD.splice(indexD, 1);
                            // }
                        }else{
