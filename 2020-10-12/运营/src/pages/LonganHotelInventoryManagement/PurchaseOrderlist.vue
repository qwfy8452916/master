<template>
  <div class="purchaselist">
    ...
    <div v-if="authzData['F:BO_PUR_PURORDER_ADD']">
      <el-button class="addbutton" @click="addproduct()">新增采购</el-button>
    </div>
    <el-table :data="Productlist" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" prop label="操作" width="170px" align="center" is-center>
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_PUR_PURORDER_VIEW']"
            type="text"
            size="small"
            @click="Seeorder(scope.$index, Productlist)"
          >查看采购单</el-button>
          <el-button
            v-if="authzData['F:BO_PUR_PURORDER_EDIT']"
            type="text"
            size="small"
            @click="Modifyproduct(scope.$index, Productlist)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_PUR_PURORDER_DELETE']"
            type="text"
            size="small"
            @click="Deleteproduct(scope.$index, Productlist)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    Confirmdel() {
      ...
      let params = "";
      this.$api
        .delpurchaseorder({ params }, that.delindexid)
        .then((response) => {
