    ...
    <div class="featuredetailadd">
        ...
        <el-form :model="FeatureDetailData" :rules="FeatureDetailData.rules" ref="FeatureDetailData" label-width="120px" class="hotelfeatureform">
            ...
            <el-form-item label="描述" prop="describeFeature">
                <el-input v-model="FeatureDetailData.describeFeature" type="textarea" :rows="5"></el-input>
            </el-form-item>
            ...
            <el-form-item>
                ...
                <el-button v-if="authzData['F:BO_HOTEL_FEATURE_DETAIL_ADD_SUBMIT']" type="primary" :disabled="isSubmit" @click="submitForm('FeatureDetailData')">确定</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
        ...
        return {
            ...
            FeatureDetailData: {
                rules: {
                    ...
                    annotation: [
                        {min: 1, max: 18, message: '文字注释请保持在18个字符以内', trigger: ['blur','change']}
                    ]
                },
                AnnotationData: [{
                    annotationTxt: ''
                ...
                if(valid){
                    ...
                    this.$api.HotelFeatureDetailAdd(params)
                        .then(response => {
                            ...
                            if(result.code == 0){
                                if(result.data){
                                    this.$message.success('添加客房设施明细成功！');
                                    const id = this.hfId;
                                    this.$router.push({name: 'LonganHotelFeatureDetail', query: {id}});
                                }else{
                                    this.$message.error('添加客房设施明细失败！');
                                    this.isSubmit = false;
                                }
                            }else{
