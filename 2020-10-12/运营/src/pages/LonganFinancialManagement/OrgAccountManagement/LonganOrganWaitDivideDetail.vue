<template>
    <div class="OrganWaitDivideDetail">
         <h3 class="alignleft">查看详情</h3>
         <p class="ptitle">基本信息</p>
         <table>
          <tr>
            <td>入账状态</td><td>
              <span v-if="recordDetailFormData.pendingStatus==0">待入账</span>
              <span v-if="recordDetailFormData.pendingStatus==1">已入账</span>
              <span v-if="recordDetailFormData.pendingStatus==2">已取消</span>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td>分成身份</td><td>
              ...
              <span v-if="recordDetailFormData.revenueAs==6">城市运营商</span>
          ...
          <tr>
            <td>待入账金额</td><td>{{recordDetailFormData.revenueAmount}}</td>
          </tr>
          <tr>
            <td>入账时间</td><td>{{recordDetailFormData.settlingTime}}</td>
          </tr>
        </table>
    ...
    methods: {
      ...
      cancelbtn(){
       this.$router.push({name:'LonganOrganWaitDivide'})
         ...
         Getdata(){
            let that=this;
            this.$api.WaitDivideDetail(that.detailId).then(response=>{
                const result=response.data;
                if(result.code==0){
                  that.recordDetailFormData=result.data
                }else{
