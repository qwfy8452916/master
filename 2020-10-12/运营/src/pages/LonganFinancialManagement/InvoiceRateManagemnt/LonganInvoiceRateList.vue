<template>
  <div class="protocollist">
    ...
    <div v-if="authzData['F:BO_FIN_PRODRATE_ADD']">
      <el-button class="addbutton" @click="invoiceRateAdd">新增</el-button>
    </div>
    <el-table :data="invoiceRateDataList" border stripe style="width:100%;">
      ...
      <el-table-column
        v-if="authzData['F:BO_FIN_PRODRATE_DEFAULT']"
        prop="isDefaultRate"
        label="设为默认税率"
        align="center"
      >
      ...
      <el-table-column fixed="right" label="操作" width="120px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_FIN_PRODRATE_EDIT']"
            type="text"
            size="small"
            @click="invoiceRateModify(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_FIN_PRODRATE_DELETE']"
            type="text"
            size="small"
            @click="invoiceRateDelete(scope.row.id)"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
