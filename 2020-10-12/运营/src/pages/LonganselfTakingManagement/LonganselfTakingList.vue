<template>
  <div class="selfTakingList">
    ...
    <div>
      <el-button
        v-if="authzData['F:BO_HOTEL_SELTTAKING_ADD']"
        class="addbutton"
        @click="addbutton"
      >新增</el-button>
    </div>
    <el-table :data="tabledata" border stripe>
      ...
      <el-table-column prop="isActive" label="状态" align="center">
        <template slot-scope="scope">
          <el-switch
            :disabled="!authzData['F:BO_HOTEL_SELTTAKING_SWITCH']"
      ...
      <el-table-column prop="id" label="操作" align="center" width="240px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BO_HOTEL_SELTTAKING_DETAIL']"
            type="text"
            @click="checkDetail(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_SELTTAKING_MODIFY']"
            type="text"
            @click="editBtn(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_SELTTAKING_DELETE']"
            type="text"
            @click="deldata(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BO_HOTEL_SELTTAKING_STAFF']"
            type="text"
            @click="hexiao(scope.row.id,scope.row.hotelId)"
          >核销人员</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="提示" :visible.sync="deljudge" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BO_HOTEL_SELTTAKING_DELETESUBMIT']"
          type="primary"
          @click="confirm"
        >确定</el-button>
      </span>
    ...
    <el-dialog title="管理用户" :visible.sync="dislogVisibleRole" width="42%" class="userMangeDialog">
      <el-transfer
        filterable
        :data="hexiaoDataList"
      ...
      <div slot="footer">
        ...
        <el-button
          v-if="authzData['F:BO_HOTEL_SELTTAKING_STAFFSUBMIT']"
          type="primary"
          @click="manageEnsure"
        >确 定</el-button>
      </div>
...
export default {
  ...
  methods: {
    ...
    confirm() {
      ...
      const params = {};
      this.$api
        .deleteSelftake(params, this.deleId)
        .then((response) => {
          const result = response.data;
          if (result.code == 0) {
            this.deljudge = false;
