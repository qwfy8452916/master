<template>
    <div class="platdeliverydetail">
        ...
        >
            ...
            >
                <template slot-scope="scope">
                    ...
                    <span v-else-if="scope.row.discountWay == 2">折扣券</span>
                </template>
            </el-table-column>
            <el-table-column
                prop="couponAmount"
        ...
        <el-button
            type="primary"
            v-if="deliveryDateDetail.status == 0"
            @click="ensurePlatDelivery"
        >确认</el-button>
        <el-button
            type="primary"
            v-if="deliveryDateDetail.delivWay == 2 && deliveryDateDetail.status == 1"
...
export default {
  ...
  methods: {
    ...
    couponUserDetails(id) {
      //跳 ‘ 查看用户卡券详情 ’
      // this.$router.push({name: 'MerchantOrderCouponDetails', query: {id}});
    },
