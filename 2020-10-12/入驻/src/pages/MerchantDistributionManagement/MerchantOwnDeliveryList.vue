<template>
  <div class="owndeliverylist">
    ...
    <el-table :data="DeliveryDataList" border stripe style="width:100%;">
      ...
      <el-table-column prop="serialNumber" label="流水号" width="80px" align="center"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="140px" align="center">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            v-if="scope.row.status == 0 && authzData['F:BM_DELIV_DELIVERYSUREBTN']"
            @click="ensurePlatDelivery(scope.row.id)"
          >确认</el-button>
          <el-button
            type="text"
            size="small"
            v-if="scope.row.delivWay == 2 && scope.row.status == 1 &&authzData['F:BM_DELIV_DELIVERYSENDBTN']"
            @click="shipmentsPlatDelivery(scope.row.id)"
          >发货</el-button>
          <el-button
            v-if="authzData['F:BM_DELIV_DELIVERYCHECK']"
            type="text"
            size="small"
    ...
    <el-dialog title="提示" :visible.sync="dialogVisibleOrder" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BM_DELIV_DELIVERYDIASUREBTN']"
          type="primary"
          @click="ensureOrder"
        >确认</el-button>
      </span>
    ...
    <el-dialog title="发货" :visible.sync="dislogVisibleShipments" width="30%">
      ...
      <div slot="footer">
        ...
        <el-button
          v-if="authzData['F:BM_DELIV_DELIVERYSENDSUREBTN']"
          type="primary"
          :disabled="isSubmit"
