<template>
  <div class="CouponList">
    ...
    <el-table :data="CouponList" border stripe style="width:100%">
      ...
      <el-table-column prop="discountWay" label="优惠范围+方式" align="center">
      ...
      <el-table-column prop="isActive" label="是否有效" align="center">
        <template slot-scope="scope">
          <el-switch
            v-if="authzData['F:BM_COUPON_MERCOUPON_SWITCH']"
      ...
      <el-table-column prop="id" label="操作" align="center" fixed="right" width="200px">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="authzData['F:BM_COUPON_MERCOUPON_EXTEND'] && scope.row.isUsed=='0'"
            @click="extendTime(scope.row.id,scope.row.couponBatchName,scope.row.couponName)"
          >延长有效期</el-button>
          <el-button
            type="text"
            v-if="scope.row.orderId!='0' && authzData['F:BM_COUPON_MERCOUPON_ORDER']"
            @click="checkCouponDetail(scope.row.id,scope.row.orderId)"
          >优惠券订单</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="延长有效期" :visible.sync="datchdialog" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BM_COUPON_MERCOUPON_EXSUBMIT']"
          type="primary"
