<template>
  <div class="ProdCouponBatch">
    ...
    <div class="alignleft">
      <el-button
        v-if="authzData['F:BM_COUPON_MERBATCH_ADD']"
        class="addbutton"
        @click="addbatch"
      >新增优惠券批次</el-button>
    </div>
    <el-table :data="batchData" border stripe style="width:100%">
      <el-table-column prop="discountWay" label="优惠范围+方式" align="center" width="200px">
      ...
      <el-table-column prop="discountMaxMoney" label="最高优惠金额" align="center"></el-table-column>
      ...
      <el-table-column prop="isActive" label="禁/启用状态" align="center">
        <template slot-scope="scope">
          <el-switch
            v-if="authzData['F:BM_COUPON_MERBATCH_SWITCH']"
      ...
      <el-table-column prop="id" label="操作" align="center" width="300px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BM_COUPON_MERBATCH_MODIFY']"
            type="text"
            @click="editBatch(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BM_COUPON_MERBATCH_DELETE']"
            type="text"
            @click="delBatch(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BM_COUPON_MERBATCH_DETAIL']"
            type="text"
            @click="checkBtn(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="authzData['F:BM_COUPON_MERBATCH_GRANT'] && scope.row.canGive=='1'"
            type="text"
            :disabled="scope.row.isActive=='0'"
            @click="grant(scope.row.id,scope.row.couponBatchName)"
          >发放</el-button>
          <!-- <el-button
            v-if="authzData['F:BM_COUPON_MERBATCH_ORDER']"
            type="text"
            @click="checkOrder"
          >查看订单</el-button> -->
        </template>
    ...
    <el-dialog title="发放" :visible.sync="datchdialog" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BM_COUPON_MERBATCH_GRANTSUB']"
          type="primary"
