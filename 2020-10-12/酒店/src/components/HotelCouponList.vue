<template>
   <div class="CouponList">
       ...
       <el-table :data="CouponList" border stripe style="width:100%">
          <el-table-column fixed prop="cusId" label="用户ID" min-width="80px" align="center"></el-table-column>
          ...
          <el-table-column prop="cusNickName" label="昵称" min-width="100px" align="center"></el-table-column>
          <el-table-column prop="cusPhone" label="手机号" min-width="120px" align="center"></el-table-column>
          <el-table-column prop="getWayName" label="获取方式" min-width="120px" align="center"></el-table-column>
          ...
          <el-table-column prop="couponStartDate" label="使用有效期" min-width="190px" align="center">
             <template slot-scope="scope">
          ...
          <el-table-column prop="isActive" label="是否有效" min-width="80px" align="center">
              <template slot-scope="scope">
                    <el-switch v-if="authzData['F:BH_COUPON_HOTELCOUPON_SWITCH']" v-model="scope.row.isActive" :active-value="1" :inactive-value="0" @change="updateStatus(scope.row.id)"></el-switch>
              </template>
          </el-table-column>
          <el-table-column prop="orderCode" label="订单编号" min-width="180px" align="center"></el-table-column>
          <el-table-column prop="useTime" label="使用时间" min-width="160px" align="center"></el-table-column>
          <el-table-column prop="id" label="操作" align="center" fixed="right" min-width="180px">
              <template slot-scope="scope">
                  <el-button v-if="authzData['F:BH_COUPON_HOTELCOUPON_EXTEND'] && scope.row.isUsed==0" type="text" @click="extendTime(scope.row.id,scope.row.couponBatchName,scope.row.couponName)">延长有效期</el-button>
                  <el-button type="text" v-if="scope.row.orderId!='0' && authzData['F:BH_COUPON_HOTELCOUPON_ORDER']" @click="checkCouponDetail(scope.row.orderId)">优惠券订单</el-button>
              </template>
          </el-table-column>
       </el-table>
       <div class="pagination">
        ...
        <el-dialog title="延长有效期" :visible.sync="datchdialog" width="30%">
            ...
            <span slot="footer">
                ...
                <el-button v-if="authzData['F:BH_COUPON_HOTELCOUPON_EXSUBMIT']" type="primary" @click="sureExtendTimeBtn">确 定</el-button>
            </span>
        </el-dialog>
   </div>
</template>
