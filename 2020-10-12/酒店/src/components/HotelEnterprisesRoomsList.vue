<template>
    <div>
        <el-form :inline="true" align=left class="searchform">
            ...
            <el-form-item label="房源">
                <el-select 
                    ...
                    placeholder="请选择房源">
                    <el-option v-for="item in roomResourceList" :key="item.id" :label="item.label" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
        ...
        <el-table :data="CabinetList" border stripe style="width:100%;" >
            <el-table-column fixed prop="contractedEnterprisesName" label="协议单位" align=center></el-table-column>
...
export default {
    ...
    methods: {
        ...
        Cabinetglchange(index,row){
            ...
            this.$router.push({name:'HotelEnterprisesRoomsChange',query:{modifyid: guiId}});
        },
        //房源列表
        getRoomList(){
            const params = {
            ...
            this.$api.getBookResourceList(params).then(response=>{
        ...
        viewDetail(index,row){
            let guiId=row[index].id
            this.$router.push({name:'HotelEnterprisesRoomsDetail',query:{modifyid: guiId}});
        },
        //删除
        Cabinetglcancel(index,row){
            ...
            this.$confirm('是否确认删除该活动?', '提示', {
                ...
                this.$api.deleteEnterprisesRooms(guiId).then(response => {
                    if(response.data.code == 0){
        ...
        Getdata(){
            ...
            this.$api.getEnterprisesRooms({params}).then(response => {
                if(response.data.code == 0){
