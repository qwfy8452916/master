    ...
    <div class="servicetypeadd">
        ...
        <el-form v-model="ServiceCommonData" :model="ServiceCommonData" :rules="rules" ref="ServiceCommonData" label-width="100px" class="servicefrom">
            ...
            <el-form-item>
                ...
                <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_FORM_EDIT_SUBMIT']" type="primary" :disabled="isSubmit" @click="submitForm('ServiceCommonData')">确定</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        serviceFormDetail(){
            ...
            this.$api.serviceFormDetail(params, hsId, sfId)
                .then(response => {
        ...
        submitForm(ServiceCommonData){
            ...
            this.$refs[ServiceCommonData].validate((valid) => {
                if(valid){
                    ...
                    // console.log(params);
                    this.isSubmit = true;
                    this.$api.serviceFormModify(params, hsId, sfId)
                        .then(response => {
                            // console.log(response);
                            const result = response.data; 
                            if(result.code == '0'){
