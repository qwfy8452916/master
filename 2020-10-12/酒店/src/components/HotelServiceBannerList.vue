<template>
    <div class="servicetypelist">
        ...
        <div v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_BANNER_ADD']"><el-button class="addbutton" @click="serviceCommonAdd">新增Banner明细</el-button></div>
        <el-table :data="ServiceCommonDataList" border style="width:100%;" >
            ...
            <el-table-column prop="picUrl" label="banner图" min-width="90px" align=center>
            ...
            <el-table-column fixed="right" label="操作" min-width="100px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_BANNER_EDIT']" type="text" size="small" @click="serviceCommonModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_BANNER_DELETE']" type="text" size="small" @click="serviceCommonDelete(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        serviceCommonList(){
            ...
            this.$api.serviceCommonList(params, hsId)
                .then(response => {
        ...
        serviceCommonAdd(){
            const hsId = this.hsId;
            this.$router.push({name:'HotelServiceBannerAdd', query: {hsId}});
        },
        //修改
        serviceCommonModify(id){
            const hsId = this.hsId;
            this.$router.push({name:'HotelServiceBannerModify', query: {hsId, id}});
        },
        //删除
        ...
        EnsureDetail(){
            ...
            const id = this.ssId;
            const params = {};
            this.$api.serviceCommonDelete(params, hsId, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
