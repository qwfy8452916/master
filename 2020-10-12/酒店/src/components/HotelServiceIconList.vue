<template>
    <div class="servicetypelist">
        ...
        <div v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_ICON_ADD']"><el-button class="addbutton" @click="serviceCommonAdd">新增图标明细</el-button></div>
        <el-table :data="ServiceCommonDataList" border style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" min-width="100px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_ICON_EDIT']" type="text" size="small" @click="serviceCommonModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_ICON_DELETE']" type="text" size="small" @click="serviceCommonDelete(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        serviceCommonAdd(){
            const hsId = this.hsId;
            this.$router.push({name:'HotelServiceIconAdd', query: {hsId}});
        },
        //修改
        serviceCommonModify(id){
            const hsId = this.hsId;
            this.$router.push({name:'HotelServiceIconModify', query: {hsId, id}});
        },
        //删除
