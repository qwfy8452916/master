  ...
  <div class="CardticketList">
    ...
    <el-table :data="cardData" border stripe>
      ...
      <el-table-column prop="isActive" label="状态" align="center">
        <template slot-scope="scope">
          <el-switch
            :disabled="!authzData['F:BH_VOU_CARDTICKET_SWITCH']"
      ...
      <el-table-column prop="id" label="操作" align="center" width="200px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BH_VOU_CARDTICKET_DETAIL']"
            type="text"
            @click="detailCard(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="authzData['F:BH_VOU_CARDTICKET_MODIFY']"
            type="text"
            @click="editCard(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BH_VOU_CARDTICKET_DELETE']"
            type="text"
...
export default {
  ...
  methods: {
    ...
    confirm() {
      ...
      let params = {};
      this.$api
        .deleCardticket(params, that.delelId)
        .then((response) => {
