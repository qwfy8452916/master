  ...
  <div class="CardCouponList">
    ...
    <el-table :data="cardData" border stripe>
      ...
      <el-table-column prop="vouDeductibleType" label="抵扣内容" align="center">
        <template slot-scope="scope">
          <div v-if="scope.row.vouUseScene==2">
            ...
            <span
              v-if="scope.row.vouDeductibleType===1"
            >{{scope.row.deductHotelProdName}}{{scope.row.deductHotelProdSpecName}}</span>
          </div>
          <div v-if="scope.row.vouUseScene==1">
            <span>/</span>
          </div>
      ...
      <el-table-column fixed="right" prop="id" label="操作" align="center" width="240px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BH_VOU_CARDCOUPON_DETAIL']"
            type="text"
            @click="detailCard(scope.row.id)"
          >详情</el-button>
          <el-button
            v-if="authzData['F:BH_VOU_CARDCOUPON_DELAY'] && scope.row.isUsed===0"
            type="text"
            @click="yancDate(scope.row.id)"
          >延长有效期</el-button>
          <el-button
            v-if="scope.row.vouUseScene==2 && scope.row.isUsed==1"
            type="text"
            @click="checkOrder(scope.row.id)"
          >查看订单</el-button>
          <el-button
            v-if="authzData['F:BH_VOU_CARDCOUPON_CHECKDETAIL'] && scope.row.isUsed===1"
            type="text"
            @click="checkcard(scope.row.id)"
          >查看使用明细</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="延长有效期" :visible.sync="datchdialog" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BH_VOU_CARDCOUPON_DELAY_SUBMIT']"
          type="primary"
          @click="sureExtendTimeBtn('diayancData')"
        >确 定</el-button>
      </span>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    sureExtendTimeBtn(diayancData) {
      ...
      this.$refs[diayancData].validate((valid, model) => {
        if (valid) {
          ...
          that.$api
            .delayCardticketDate(params, that.delayId)
            .then((response) => {
    ...
    //查看使用明细
    checkcard(id) {
      this.$router.push({ name: "HotelCardCouponRecord", query: { id } });
    },
