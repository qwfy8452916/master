<template>
  <div class="functionlist">
    ...
    <div v-if="authzData['F:BH_PROD_MINIPROD_ADD']">
    ...
    <el-table :data="miniDeployDataList" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" width="240px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BH_PROD_MINIPROD_EDIT'] && scope.row.isDefault != 1"
            type="text"
            size="small"
            @click="miniDeployModify(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="authzData['F:BH_PROD_MINIPROD_DELETE'] && scope.row.isDefault != 1"
            type="text"
            size="small"
            @click="miniDeployDelete(scope.row.id)"
          >删除</el-button>
          <el-button
            v-if="authzData['F:BH_PROD_MINIPROD_PROD']"
            type="text"
            size="small"
            @click="deployProdSelect(scope.row.hotelId, scope.row.id, scope.row.isDefault)"
          >选择商品</el-button>
          <el-button
            v-if="authzData['F:BH_PROD_MINIPROD_ROOM'] && scope.row.isDefault != 1"
            type="text"
            size="small"
