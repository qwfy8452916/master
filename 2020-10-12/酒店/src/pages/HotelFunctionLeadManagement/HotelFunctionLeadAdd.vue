  ...
  <div class="CabTypeListAdd">
     ...
     <el-form align="left" :model="addLinkData" label-width="120px" :rules="rules" ref="addLinkData">
         <el-form-item label="功能区" prop="hotelFuncId">
            <el-select @change="clearData1()" @focus="hotelFunctionList()" :loading="loadingH" v-model="addLinkData.hotelFuncId" placeholder="选择功能区">
        ...
        <el-form-item label="关联酒店" prop="hotelRelateId">
            <el-select :disabled="addLinkData.hotelFuncId == ''" @focus="relateHotel()" :loading="loadingH"  v-model="addLinkData.hotelRelateId" placeholder="选择关联酒店">
  ...
  export default{
    ...
    data(){
      ...
      return {
        ...
        rules:{
            ...
            sort:[{ type: 'number', message: '请输入数字', trigger: ['blur','change']}],
        },
    ...
    methods:{
        ...
        relateHotel(){
            ...
            this.$api.getWithoutAdd({params}).then(response => {
                this.loadingH = false;
                if(response.data.code==0){
                    let recordsData = response.data.data;
                    let areaList = recordsData.map(item=>{
