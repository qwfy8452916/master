<template>
  <div class="aftersaleapplylist">
    ...
    <el-table :data="aftersaleapplylistDataList" border stripe style="width:100%;">
      ...
      <el-table-column label="操作" width="120px" align="center" fixed="right">
        <template slot-scope="scope">
          <el-button
            type="text"
            v-if="scope.row.status==1 && authzlist['F:BH_CS_HOTELPROD_DEAL']"
            size="small"
            @click="handle(scope.$index, aftersaleapplylistDataList)"
          >处理</el-button>
          <el-button
            v-if="authzlist['F:BH_CS_HOTELPROD_VIEW']"
            type="text"
            size="small"
            @click="lookdetail(scope.$index, aftersaleapplylistDataList)"
          >详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      title="处理售后申请"
      :visible.sync="dialogVisibleDelete1"
      center
      width="30%"
      class="hanginput"
    >
...
export default {
  ...
  methods: {
    ...
    handleaftersale(csType, status) {
      ...
      this.$api
        .handleSaleApply(params, that.aftersaleId)
        .then((response) => {
