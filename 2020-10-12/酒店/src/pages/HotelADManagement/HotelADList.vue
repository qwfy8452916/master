  ...
  <div class="hotellist">
    ...
    <el-table :data="AdDataList" border stripe style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" min-width="280px" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.status == 2 && scope.row.adLevel == 1"
            type="text"
            size="small"
          ...
          <el-button
            v-if="scope.row.adLevel == 1"
            type="text"
            size="small"
            @click="hotelADModify(scope.row.id)"
          >修改</el-button>
          <el-button
            v-if="scope.row.adLevel == 1"
            type="text"
            size="small"
            @click="hotelADDelete(scope.row.id)"
          >删除</el-button>
          <el-button
          ...
          <el-button
            v-if="scope.row.status != 1 && scope.row.adLevel == 1"
            type="text"
            size="small"
    ...
    <el-dialog title="预览" :visible.sync="dialogVisiblePreview" width="48%">
      <div v-html="PreviewData" class="PreviewStyle"></div>
    </el-dialog>
    <el-dialog title="发布" :visible.sync="dialogVisibleIssue" width="48%">
      <div v-html="IssueData" class="PreviewStyle"></div>
      <span slot="footer">
