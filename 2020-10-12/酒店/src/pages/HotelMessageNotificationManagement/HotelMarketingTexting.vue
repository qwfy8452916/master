  ...
  <div class="marketingDetail">
    ...
    <el-dialog title="选择用户" :visible.sync="dialogTableVisible">
      ...
      <el-table :data="userlist" stripe style="width:100%">
        <el-table-column fixed prop="commonId" label="用户ID" align="center"></el-table-column>
        <el-table-column prop="nickName" label="用户昵称" align="center"></el-table-column>
        <el-table-column prop="mobile" label="手机号" align="center"></el-table-column>
        <el-table-column prop="firstVisitTimeStr" label="首次访问时间" align="center"></el-table-column>
        <el-table-column prop="lastVisitTimeStr" label="最后访问时间" align="center"></el-table-column>
      </el-table>
      <HotelPagination
        :pageSize="pageSize"
        :layout="layout"
      ...
      <div class="alignleft">
        ...
        <el-button type="primary" @click="get_userlistAll">选择全部用户({{totalnum}})</el-button>
      </div>
    </el-dialog>
  </div>
</template>
    ...
    get_userlist() {
      ...
      that.$api
        ...
        .then((response) => {
          const result = response.data;
          if (result.code == "0") {
            that.userlist = result.data.records;
            that.totalnum = result.data.total;
          } else {
            that.$message.error("已认证手机号的酒店顾客获取失败！");
          }
        })
        .catch((error) => {
          that.$alert(error, "警告", {
