<template>
  <div class="outhouselist">
    ...
    <div class="godownentryadd">
      <el-button
        class="addbutton"
        v-if="authzlist['F:BH_INV_OUTHOUSELIST_ADD']"
    ...
    <el-table :data="HotelGodownEntryDataList" border style="width:100%;">
      ...
      <el-table-column fixed="right" label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.reviewStatus== 2 && authzlist['F:BH_INV_OUTHOUSELIST_SCHEDULE']"
            type="text"
            size="small"
            @click="examineDetail(scope.row.wfId)"
          >审核进度</el-button>
          <el-button
            v-if="scope.row.reviewStatus == 0 && authzlist['F:BH_INV_OUTHOUSELISTALTER']"
            type="text"
            size="small"
            @click="modifyInfo(scope.row.id)"
          >修改</el-button>
          <el-button
            v-else-if="authzlist['F:BH_INV_OUTHOUSELIST_DETAIL']"
            type="text"
            size="small"
