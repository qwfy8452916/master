  ...
  <div class="CabTypeListAdd">
     ...
     <el-form align="left" :model="addLinkData" label-width="120px" :rules="rules" ref="addLinkData">
        ...
        <el-form-item label="进场配置：" prop="enterId">
            <el-select :loading="loadingH" @focus="getEnterSettings" :disabled="!addLinkData.relateHotelId?true:false" v-model="addLinkData.enterId" placeholder="选择进场配置">
  ...
  export default{
    ...
    methods:{
      ...
      getHotelList(hName){
          ...
          this.$api.getAllNameHotel({params})
            .then(response => {
                this.loadingH = false;
                const result = response.data;
                if(result.code == 0){
      ...
      sureBtn(addLinkData){
        ...
        this.$refs[addLinkData].validate((valid,model)=>{
          if(valid){
            this.$api.addRelateHotel(params).then(response=>{
