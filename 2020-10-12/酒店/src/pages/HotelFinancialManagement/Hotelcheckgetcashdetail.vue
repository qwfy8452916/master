<template>
    <div class="purchaseadd">
       <h3 class="alignleft">查看详情</h3>
        <table>
          <tr>
            <td>状态</td><td>{{HotelWithdrawalsRecordDetails.status=='1'?'待处理':(HotelWithdrawalsRecordDetails.status=='2'?'已转账':'转账失败')}}</td>
          </tr>
          <tr>
            <td>提现金额(元)</td><td>{{HotelWithdrawalsRecordDetails.withdrawalAmount}}</td>
          </tr>
          <tr>
            <td>申请人</td><td>{{HotelWithdrawalsRecordDetails.withdrawalName}}</td>
          </tr>
          <tr>
            <td>申请时间</td><td>{{HotelWithdrawalsRecordDetails.withdrawalTime}}</td>
          </tr>
          <tr>
            <td>开户银行</td><td>{{HotelWithdrawalsRecordDetails.bank}}</td>
          </tr>
          <tr>
            <td>账户名称</td><td>{{HotelWithdrawalsRecordDetails.accountName}}</td>
          </tr>
          <tr>
            <td>账号</td><td>{{HotelWithdrawalsRecordDetails.account}}</td>
          </tr>
          <tr v-if="HotelWithdrawalsRecordDetails.status=='2'">
            <td>转账时间</td><td>{{HotelWithdrawalsRecordDetails.transferTime}}</td>
          </tr>
          <tr v-if="HotelWithdrawalsRecordDetails.status=='2'">
            <td>转账凭证</td><td>
          ...
          <tr>
            <td>转账说明</td><td>{{HotelWithdrawalsRecordDetails.remark}}</td>
          </tr>
          <tr v-if="HotelWithdrawalsRecordDetails.status!='1'">
            <td>处理人</td><td>{{HotelWithdrawalsRecordDetails.dealer}}</td>
          </tr>
          <tr v-if="HotelWithdrawalsRecordDetails.status!='1'">
            <td>处理时间</td><td>{{HotelWithdrawalsRecordDetails.dealTime}}</td>
          </tr>
...
export default {
    ...
    data() {
        return{
            prodchangeid:"",  //查看id
            HotelWithdrawalsRecordDetails:{},  //数据
            voucherdata:[],
            divwrap:'divwrap',
            fangda:'desty',
            imgData:'',
       ...
       Getdata(){
            ...
            this.$api.getcashdetail({params},that.prodchangeid).then(response=>{
                if(response.data.code==0){
                  that.HotelWithdrawalsRecordDetails=response.data.data;
