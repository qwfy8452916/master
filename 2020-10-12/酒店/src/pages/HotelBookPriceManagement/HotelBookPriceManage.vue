<template>
  <div class="pricemanage">
    <div v-if="isHaveResource" class="batchmbtnSearch">
      <el-button type="primary" @click="searchChangePrice()">查看变价单</el-button>
    </div>
    <div v-if="authzData['F:BH_BOOK_PRICE_EDIT_BATCH'] && isHaveResource" class="batchmbtn">
      <el-button @click="batchModifyPrice">批量修改</el-button>
    </div>
    ...
    <el-dialog :visible.sync="dislogVisibleModify" width="30%">
      <span slot="title" style="color:#2c3e50;">
        修改
        <span class="mstyle">{{mDate}}</span>
        <span class="mstyle">{{mWeek}}</span>
      </span>
      <el-form :model="modifyForm" ref="modifyForm" label-width="80px">
        <el-form-item label="房价：" prop="roomPrice">
          <el-input v-model.trim="modifyForm.roomPrice" maxlength="10" @blur="changeRoomPrice()"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer">
        <el-button @click="dislogVisibleModify = false">取 消</el-button>
        <el-button
          v-if="authzData['F:BH_BOOK_PRICE_EDIT']"
          type="primary"
...
export default {
  ...
  methods: {
    changeRoomPrice() {
      let decimalsRegs = /^\d+(\.\d+)?$/;
      if (!decimalsRegs.test(this.modifyForm.roomPrice)) {
        this.$message.warning("输入格式有误");
        return false;
      }
    },
    searchChangePrice() {
      this.$router.push({ name: "HotelBookPriceManageList" });
    },
    //获取房型列表
    getTypeList() {
      const params = {
    ...
    modifyEnsure(modifyForm) {
      ...
      const that = this;
      const params = {
        changeName: "单个修改", //单个修改/批量修改
        // roomResourceId: this.resourceNameTab.split(),
        roomResourceId: this.resourceNameTab,
        changeStartDate: this.mDate,
        changeEndDate: this.mDate,
        afterChangePrice: this.modifyForm.roomPrice,
      };
    ...
    batchMEnsure(batchMForm) {
      ...
      this.$refs[batchMForm].validate((valid) => {
        if (valid) {
          ...
          const params = {
            changeName: "批量修改",
            // roomResourceId: this.batchMForm.resourceId.split(),
            roomResourceId: this.batchMForm.resourceId,
            changeStartDate: this.batchMForm.rangeDate[0],
            changeEndDate: this.batchMForm.rangeDate[1],
            weekSuitList: this.batchMForm.useWeek,
            afterChangePrice: this.batchMForm.roomPrice,
          };
          this.$api
            .bookPriceChange(params)
            .then((response) => {
