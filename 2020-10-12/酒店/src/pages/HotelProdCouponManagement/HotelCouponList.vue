  ...
  <div class="CouponList">
    <el-form :inline="true" align="left">
      ...
      <el-form-item label="获取方式">
        <el-select v-model="getWay">
          <el-option v-for="item in wayData" :key="item.dictValue" :value="item.dictValue" :label="item.dictName"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-table :data="CouponList" border stripe style="width:100%">
      ...
      <el-table-column prop="isActive" label="是否有效" min-width="80px" align="center">
        <template slot-scope="scope">
          <el-switch
            v-if="authzData['F:BH_COUPON_HOTELCOUPON_SWITCH']"
      ...
      <el-table-column prop="id" label="操作" align="center" fixed="right" min-width="180px">
        <template slot-scope="scope">
          <el-button
            v-if="authzData['F:BH_COUPON_HOTELCOUPON_EXTEND'] && scope.row.isUsed==0"
            type="text"
            @click="extendTime(scope.row.id,scope.row.couponBatchName,scope.row.couponName)"
          >延长有效期</el-button>
          <el-button
            type="text"
            v-if="scope.row.orderId!='0' && authzData['F:BH_COUPON_HOTELCOUPON_ORDER'] && scope.row.isUsed==1"
            @click="checkCouponDetail(scope.$index,CouponList)"
          >优惠券订单</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="延长有效期" :visible.sync="datchdialog" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          v-if="authzData['F:BH_COUPON_HOTELCOUPON_EXSUBMIT']"
          type="primary"
...
export default {
  ...
  methods: {
    ...
    //优惠券订单
    checkCouponDetail(index,data) {

      let id=data[index].orderId;
      let couponRange=data[index].couponRange;
      if(couponRange==1){
    ...
    basicDataItems:function(){
      ...
      this.$api.basicDataItems(params).then(res=>{
        console.log(res)
         let resData=res.data;
         if(resData.code==0){
           that.wayData=resData.data
           let allObject={
             dictValue:'',
             dictName:'全部'
           }
            that.wayData.unshift(allObject)
         }else{
           that.$alert(resData.msg, "警告", {
              confirmButtonText: "确定",
            });
         }
      }).catch(err=>{
