<template>
    <div class="featuredetaillist">
        ...
        <div class="featuredetailadd"><el-button class="addbutton" type="primary" @click="featureDetailAdd">添加客房设施明细</el-button></div>
        <el-table :data="FeatureDetailDataList" border style="width:100%;" >
            ...
            <el-table-column label="操作" width="180px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="scope.row.reviewStatus == 2 && authzlist['F:BH_HOTEL_FEATURE_REVIEWPROGRESS']" type="text" size="small" @click="lookReviewProcess(scope.row.wfId)">审核进度</el-button>
                    <el-button v-if="authzlist['F:BH_HOTEL_FEATURETYPE_DETAILALTER']" type="text" size="small" @click="modifyFeatureDetail(scope.row.id)">修改</el-button>
                    <el-button v-if="authzlist['F:BH_HOTEL_FEATURETYPE_DETAILDELETE']" type="text" size="small" @click="deleteFeatureDetail(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-dialog title="提示" :visible.sync="dialogVisibleDelete" width="30%">
            <span>是否确认删除该客房设施明细？</span>
            <span slot="footer">
...
export default {
    ...
    methods: {
            ...
            this.$api.hotelFeatureDetail(params)
                .then(response => {
                    ...
                    if(result.code == '0'){
                        ...
                        this.$message.error('客房设施明细列表获取失败！');
                    }
                })
                .catch(error => {
                    this.$alert(error,"警告",{
        ...
        featureDetailAdd(){
            const id = this.hfId;
            this.$router.push({name: 'HotelFeatureTypeDetailAdd', query: {id}});
        },
        //查询
        inquire(){
            this.pageNum = 1;
        ...
        modifyFeatureDetail(id){
            ...
            this.$router.push({name:'HotelFeatureTypeDetailModify',  query: {hfDetailId, hfId}});
        },
        //删除
        ...
        EnsureDetail(){
            ...
            this.$api.hotelFeatureDetailDelete(params, id)
                .then(response => {
                    ...
                    if(result.code == '0'){
                        if(result.data){
                            this.$message.success('删除客房设施明细成功！');
                            this.dialogVisibleDelete = false;
                            this.featureDetailList();
                        }else{
                            this.$message.error('删除客房设施明细失败！');
                            this.dialogVisibleDelete = false;
                        }
                    }else{
                        this.$message.error('删除客房设施明细失败！');
