(function  ($) {
	$.fn.dargFlex = function  (classN,json) {
		...
		$(".drag_handle").bind('mousedown',function (ev){
			var oEe = ev || event;
			var oSrc = $(oEe.srcElement).parent()[0] || $(oEe.target).parent()[0];//默认不带参数，默认全部子项拖拽
			if ($(oEe.target).parent().hasClass(classN)){		//默认以类名匹配拖拽元素
				var oldY = oEe.clientY;
				var oldX = oEe.clientX;
				var iTop = $(oEe.target).parent()[0].offsetTop;
				var iLeft =$(oEe.target).parent()[0].offsetLeft;
				var iW  =$(oEe.target).parent()[0].offsetWidth;
				var iH = $(oEe.target).parent()[0].offsetHeight;
				$($(oEe.target).parent()[0]).css({
					'position':'absolute',
					'top':iTop+'px',
					'left':iLeft + 'px',
					'width':iW +'px',
