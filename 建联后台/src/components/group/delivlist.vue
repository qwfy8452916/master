  ...
  <el-tabs v-model="activeName" @tab-click="handleClick" style="width:50%;">
    ...
    <el-table :data="tableData" border stripe style="width:100%;" class="top">
      ...
      <el-table-column prop="receiveAt" label="收货日期" align="center">
        <template
          slot-scope="scope"
        >{{ scope.row.receiveAt==='1970-01-01 00:00:00' ? '-' : scope.row.sendAt }}</template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <div>
            <el-button
              ...
              v-if="authzData['F:CM_DELIVERY_DELIVERY_DETAIL']"
              class="check-text"
            >查看详情</el-button>
          </div>
        </template>
    ...
    <el-dialog title="提示" :visible.sync="dialogCreate" width="20%" top="30vh">
      <span>是否确认发布？</span>
      <span slot="footer">
        <el-button @click="dialogCreate=false" class="cancel-btn">取消</el-button>
        <el-button type="primary" @click.native.prevent="createSupList()" class="btn-mid">确定</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog title="确认收货" :visible.sync="dialogVisible2" width="55%">
      ...
      <div class="align-left hangitem">
        <span class="hangitemtitle">物流凭证：</span>
        <input class="filese" type="text" v-model="detailData.logisticsVoucher" :disabled="true"/>
      </div>
      <div class="align-left hangitem">
        <span class="hangitemtitle">*收货时间：</span>
        <el-date-picker
      ...
      <h5 class="align-left">参数详情</h5>
      <el-table :data="tableData2" border stripe style="width:900px;margin-bottom:30px;" :row-style="{height:'40px'}">
      ...
      <el-button class="marginleft15" @click="back">返回</el-button>
      <el-button class="marginleft15" type="primary" @click="ensuredeliv()">确认收货</el-button>
    </el-dialog>
    <!-- 图片弹框预览 -->
      ...
      this.$api
        ...
        .then(response => {
          ...
          if (result.code == "0") {
            ...
            // this.detailData = result.data;
            // this.tableData2 = result.data.delivDetailDTOList;
            // console.log(this.detailData.scheduledReceiveAt);
          } else {
