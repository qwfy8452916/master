<template>
  <div class="purorderdetail">
    <el-tabs v-model="activeName" @tab-click="handleClick" style="width:50%;">
            ...
            <el-tab-pane label="供货单列表" name="supplyList" v-if="authzData['F:CM_ORDER_ORDER_DETAIL_DELIVERY']"></el-tab-pane>
            <el-tab-pane label="结算单列表" name="settleList" v-if="authzData['F:CM_ORDER_ORDER_DETAIL_SETTLE']"></el-tab-pane>
            <el-tab-pane label="付款单列表" name="paylist" v-if="authzData['F:CM_ORDER_ORDER_DETAIL_PAYMENT']"></el-tab-pane>
    </el-tabs>
      ...
      <el-table-column fixed="right" prop label="操作" width="200px" align="center">
        <template slot-scope="scope">
          <div v-if="scope.row.status==3">
            <el-button type="text" size="small" @click="confirmdeliv(scope.row.id)" v-if="authzData['F:CM_PAYMENT_PAYMENT_VERIFY']" class="edit-text">审核</el-button>
            <el-button type="text" size="small" @click="checkdetail(scope.row.id)" v-if="authzData['F:CM_PAYMENT_PAYMENT_DETAIL']" class="check-text">查看详情</el-button>
          </div>
          <div v-else>
            <el-button type="text" size="small" @click="checkdetail(scope.row.id)" v-if="authzData['F:CM_PAYMENT_PAYMENT_DETAIL']" class="check-text">查看详情</el-button>
          </div>
    ...
    <el-dialog title="审核发票信息" :visible.sync="dialogVisible2" width="55%">
      ...
      <el-button style="background-color:#EE1E1E" class="marginleft15 btn-mid" @click="refuback(0)" type="danger" v-if="authzData['F:CM_PAYMENT_PAYMENT_VERIFY_REJECT']">退回</el-button>
      <el-button class="marginleft15 btn-mid" type="primary" @click="ensuredeliv(1)" v-if="authzData['F:CM_PAYMENT_PAYMENT_VERIFY_APPROVE']">通过</el-button>
    </el-dialog>
    <!-- 图片弹框预览 -->
    ...
    <el-dialog title="提示" :visible.sync="pass" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="confirmpass" v-if="authzData['F:CM_PAYMENT_PAYMENT_VERIFY_APPROVE_SUBMIT']">是</el-button>
      </span>
    </el-dialog>
    <el-dialog title="请输入退回原因" :visible.sync="refusereson" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="conreject" v-if="authzData['F:CM_PAYMENT_PAYMENT_VERIFY_REJECT_SUBMIT']">确定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
