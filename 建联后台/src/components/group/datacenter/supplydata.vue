  ...
  <div class="mainBox">
    ...
    <el-form class="top">
      <el-form-item label="选择产品">
        <el-select v-model="value" placeholder="请选择" style="float:left" @change="selectProject">
          <el-option
    ...
    <div class="dataBox">
      <div class="dataLeft">
        <p>总需总数</p>
        <p class="money">{{totalconfirmDelivMoney}}吨</p>
      </div>
      <div class="dataRight">
        <p>实供总数</p>
        <p class="money">{{totalconfirmPayMoney}}吨</p>
      </div>
    </div>
  </div>
</template>
      ...
      this.$api
        ...
        .then(res => {
          ...
          if (result.code == "0") {
            ...
            let companyName = this.selectList.map(item => {
            ...
            this.selectList.map((item, index) => {
              let purchaseNum = 0;
              let confirmReceiveGoods = 0;
              item.userOrders.map(delivitem => {
                if (delivitem.productName == this.productName) {
                  delivesum += delivitem.purchaseNum;
                  paysum += delivitem.confirmReceiveGoods;
                  purchaseNum += delivitem.purchaseNum;
                  confirmReceiveGoods += delivitem.confirmReceiveGoods;
                }
              });
              deliveArr[index] = purchaseNum;
              payArr[index] = confirmReceiveGoods;
            });
            console.log(paysum)
            console.log(delivesum)
            //返回
            this.totalconfirmDelivMoney = delivesum;
            this.totalconfirmPayMoney = paysum;
            this.productDeliveTotal = deliveArr;
            this.productPayTotal = payArr;
            ...
            myChart.setOption({
              ...
              series: [
                {
                  name: "总需数量",
                  data: this.productDeliveTotal
                },
                {
                  name: "实供数量",
                  data: this.productPayTotal
                }
              ]
            });
          }
          ...
          if (result.code == "0") {
            ...
            this.list.map(item => {
              item.userOrders.map(orderItem => {
                if (!newProductArr.includes(orderItem.productName)) {
                  projectNameArr.push({
                    value: orderItem.productName,
                    label: orderItem.productName
                  });
                  newProductArr.push(orderItem.productName);
                }
              });
            });
            this.options = projectNameArr;
            let newArr = [];
            newArr = this.options.filter(function(item) {
      ...
      this.$api
        ...
        .then(res => {
          ...
          if (result.code == "0") {
              ...
              if (item.userName == this.userName) {
                item.userOrders.map(orderItem => {
                  if (this.productName == "") {
                    ...
                    payArr.push(orderItem.purchaseNum);
                    deliveArr.push(orderItem.confirmReceiveGoods);
                    purchaseNum += orderItem.purchaseNum;
                    confirmReceiveGoods += orderItem.confirmReceiveGoods;
                  } else {
                    projectName.push(orderItem.projectName);
                    if (orderItem.productName == this.productName) {
                      payArr.push(orderItem.purchaseNum);
                      deliveArr.push(orderItem.confirmReceiveGoods);
                      purchaseNum += orderItem.purchaseNum;
                      confirmReceiveGoods += orderItem.confirmReceiveGoods;
            ...
            this.payTotal = purchaseNum;
            this.deliveTotal = confirmReceiveGoods;
