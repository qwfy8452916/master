  ...
  <div class="purorderdetail">
        ...
        <template slot-scope="scope">
          <div v-if="scope.row.status==1">
            <el-button
              ...
              v-if="authzData['F:CM_BPAYMENT_BPAYMENT_VERIFY']"
              class="edit-text"
            >审核</el-button>
            <el-button
              type="text"
              ...
              v-if="authzData['F:CM_BPAYMENT_BPAYMENT_DETAIL']"
              class="check-text"
            >查看详情</el-button>
          </div>
        </template>
    ...
    <el-dialog title="审核发票信息" :visible.sync="dialogVisible2" width="55%">
      ...
      <el-button
        ...
        @click="refuback(0)"
        type="danger"
        v-if="authzData['F:CM_BPAYMENT_BPAYMENT_VERIFY_REJECT']"
      >退回</el-button>
      <el-button
        class="marginleft15"
        style="width:120px;height:38px;"
        type="primary"
        @click="ensuredeliv(1)"
        v-if="authzData['F:CM_BPAYMENT_BPAYMENT_VERIFY_APPROVE']"
      >通过</el-button>
    </el-dialog>
    <el-dialog title="提示" :visible.sync="pass" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          type="primary"
          @click="confirmpass"
          v-if="authzData['F:CM_BPAYMENT_BPAYMENT_VERIFY_APPROVE_SUBMIT']"
        >是</el-button>
      </span>
    </el-dialog>
    <el-dialog title="请输入退回原因" :visible.sync="refusereson" width="30%">
      ...
      <span slot="footer">
        ...
        <el-button
          type="primary"
          @click="conreject"
          v-if="authzData['F:CM_BPAYMENT_BPAYMENT_VERIFY_REJECT_SUBMIT']"
        >确定</el-button>
      </span>
    </el-dialog>
