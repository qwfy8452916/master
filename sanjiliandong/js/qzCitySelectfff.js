...
var selectQz = {
    ...
    events:function(j,k,l){
        ...
        $("body").on("click",".center-li",function(){
            ...
            if(j != parentid){
                selectQz.createRight(rlpca[j].children[id].children,parentid,id);
                k = id;
            }else{
                k = id;
                selectQz.createRight(rlpca[parentid].children[id].children,parentid,id); //创建三级联动 
        ...
        $("body").on("click",".right-li",function(){
            ...
            if(j!=rootid && k!=parentid){
                $('#showCityPicker2').val(rlpca[j].text+" "+rlpca[j].children[k].text+" "+rlpca[j].children[k].children[id].text).css('color',"#666");
            }else if(j == rootid && k!=parentid){
                k = 0;
                $('#showCityPicker2').val(rlpca[rootid].text+" "+rlpca[rootid].children[k].text+" "+rlpca[rootid].children[k].children[id].text).css('color',"#666");
            }else{
                $('#showCityPicker2').val(rlpca[rootid].text+" "+rlpca[rootid].children[parentid].text+" "+rlpca[rootid].children[parentid].children[id].text).css('color',"#666");
            }
            // $('#showCityPicker2').html('<i class="fa fa-map-marker"></i> '+rlpca[rootid].text+" "+rlpca[rootid].children[parentid].text+" "+rlpca[rootid].children[parentid].children[id].text).css('color',"#666"); 
