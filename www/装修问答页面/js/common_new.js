...
!(function() {
	...
	function hfDocReady() {
		...
		jq('#searchform').on('submit', function() {
			...
			if(searchtage) {
				try {
					clickStream.getCvParams(searchtage);
					clickStream.sendPv();
				} catch(e) {}
			}
		...
		jq('.header_bottom').on('click', 'a', function() {
			...
			try {
				ptag && clickStream.getCvParams(ptag);
			} catch(e) {}
		});
...
(function(jq) {
	var scrollTop = {
		init: function(qqArr, qqShow, editShow) {
...
! function() {
	jq.fn.windowBox = function(settings) {
		...
		if(bType.name == "msie" && bType.version == "6") {
			...
			jq('.translucence_layer').css({
				"top": h + "px",
				"height": box.fn.wHeight + "px"
			});
		};
...
function simplifyCheck2(chkArr) {
	...
	for(var i = 0, len = chkArr.length; i < len; i++) {
		...
		if(settings.require || (!settings.require && settings.condition)) { //必须，条件成立
			a = jq(settings.id).checkForm2({
				className: settings.className,
				info: settings.info,
				labl: settings.labl,
				lablClass: settings.lablClass,
				parCls: settings.parCls,
				callback: settings.callback,
				parentTip: settings.parentTip,
				chkType: settings.chkType
			});
		}
	...
	jq.fn.slider = function(settings, t) {
		...
		if(settings.markSlider && !scroller.moveSlider) {
			...
			scroller.markLi.mouseenter(function() {
				...
				scroller.li.eq(scroller.markNum - 1).stop(1, 1).css("opacity", '0.5').addClass('' + settings.play + '').css("display", 'block').animate({
					opacity: "0"
				}, settings.speed, function() {
					jq(this).css('display', 'none');
				});
				scroller.li.eq(scroller.markNum).stop(1, 1).css('opacity', '0.5').removeClass('' + settings.play + '').css("display", 'block').animate({
					opacity: "1"
				}, settings.speed);
			...
			switch(d) {
				...
				case "bRight":
					...
					if(scroller.moveSlider && (scroller.onCurNum + 1) == scroller.itemSum) { //5
						...
						obj.css('left', '-' + scroller.li.width() + 'px');
						obj.animate({
							left: '-' + 2 * scroller.li.width() + 'px'
						});
						scroller.li.removeClass('' + settings.play + '');
						scroller.li.eq(2).addClass('' + settings.play + '');
						if(settings.markSlider) {
							scroller.markLi.removeClass('' + settings.markLiClass + '').eq(1).addClass('' + settings.markLiClass + '');
						}
					...
					if(((scroller.onCurNum + 1) == scroller.itemSum) && !scroller.moveSlider) {
						...
						scroller.li.eq(0).stop(1, 1).css('opacity', '0.5').removeClass('' + settings.play + '').css("display", 'block').animate({
							opacity: "1"
						}, settings.speed);
						if(settings.fontLi) scroller.fontLi.eq(0).removeClass('' + settings.play + '').css("display", 'block').animate({
							opacity: "1"
						}, settings.speed);
						...
						scroller.li.eq(scroller.onCurNum + 1).stop(1, 1).css('opacity', '0.5').removeClass('' + settings.play + '').css("display", 'block').animate({
							opacity: "1"
						}, settings.speed);

						if(settings.fontLi) scroller.fontLi.eq(scroller.onCurNum + 1).removeClass('' + settings.play + '').css("opacity", '1').animate({
							opacity: "1"
						}, settings.speed, function() {
				...
				case "bLeft":
					...
					if(scroller.moveSlider && scroller.onCurNum == 0) { //0
						...
						obj.animate({
							left: '-' + (scroller.itemSum - 3) * scroller.li.width() + 'px'
						});
						scroller.li.removeClass('' + settings.play + '');
						scroller.li.eq(scroller.onCurNum - 3).addClass('' + settings.play + '');
						if(settings.markSlider) {
							scroller.markLi.removeClass('' + settings.markLiClass + '').eq(scroller.onCurNum - 2).addClass('' + settings.markLiClass + '');
						};
					...
					if(scroller.onCurNum == 0 && !scroller.moveSlider) {
						...
						scroller.li.eq(scroller.itemSum - 1).stop(1, 1).css('opacity', '0.5').removeClass('' + settings.play + '').css("display", 'block').animate({
							opacity: "1"
						}, settings.speed, function() {
							jq(this).css('display', 'block');
						});

						if(settings.fontLi) scroller.li.eq(scroller.itemSum - 1).removeClass('' + settings.play + '').css("display", 'block').animate({
							opacity: "1"
						}, settings.speed);
						...
						//sisi
						if(settings.markSlider) {
							scroller.markLi.removeClass('' + settings.markLiClass + '').eq(scroller.itemSum - 1).addClass('' + settings.markLiClass + '');
						};
						//sisi
						...
						if(settings.fontLi) scroller.fontLi.eq(scroller.onCurNum - 1).removeClass('' + settings.play + '').css("display", 'block').animate({
							opacity: "1"
						}, settings.speed);
		...
		obj.find('li').mouseenter(function() {
			...
			if(jq(this).parent().attr('v') != '1') {
				jq("img.lazy1").lazyload({
					placeholder: "http://img.to8to.com/front_end/bg/grey.gif",
					effect: "fadeIn",
					failurelimit: 54
				});
			};
	...
	jq('#subZbForm').on('submit', function() {
		var chkArr = [{
			...
			info: [{
				...
				reg: [/^[1-9]{1}\d{0,3}(\.\d{1,2})?$/],
				tip: '面积不能超过10000，支持两位小数'
			}]
		}, {
			id: '#zb_zxBurget',
			info: [{
...
jq(function() {
	jq("#side_slider li").click(function() {
		try {
			clickStream.getCvParams(jq(this).attr("j_data"));
		} catch(e) {}
	});
});
