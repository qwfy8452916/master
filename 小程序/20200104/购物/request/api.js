...
const apis = {
  ...
  //授权登陆
  postloginByWX(data){
    return post('user/customer/loginByWX', {data})
  },
  //获取优惠券列表
  getcouponlist(data){
  ...
  getcancouponlist(data){
    return post('coupon/coupon/customer/useful', {data})
  },
  //领取优惠券
  postcoupon(data){
  ...
  postverifyavailable(data){
    return post('coupon/coupon/customer/valid', {data})
  },
  //获取分享码
  postsharecode(data){
    return post('act/share/code', {data})
  },
  //快递商品分类
  postcategories(data){
  ...
  //获取cabcode、shareUser
  getcabcode(data){
    return get('act/share/code/check', {data})
  },
  //获取柜子类型配置
  getcabtype(data){
  ...
  //再次开门
  getaginopen(id){
    return get('order/detail/cab/open/' + id)
  },
  //获取酒店房型房源信息
  getroominfo(data){
  ...
  //领取分享红包
  getshareredbag(data){
    return get('act/redPacket/record', {data})
  },
  //修改红包为已分享
  putredbagtype(code){
