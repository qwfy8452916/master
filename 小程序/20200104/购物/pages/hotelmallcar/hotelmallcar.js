...
//delivWay 0：都没有 1：客服配送 2：快递配送，3:都有
Page({
  data: {
    province: '',//省代号
    cabCode: '',
  ...
  getshoppingcart: function (buylist){//处理购物车数据   
    const that = this;
  ...
  checkboxChange2: function (e) {//迷你吧选择
    const that = this;
    that.setData({
      coupon_selected: []
    });
  ...
  cutback: function(e){//减少
    ...
    if (listtype == 1) {//客房配送
      ...
      if (checkbox_listd_2[index].prodnum == 1) {//快递到家
        wx.showModal({
          title: '提示',
          content: '您确实删除此商品吗？',
          success(res) {
  ...
  checkshopcar: function (checkbox_listd_1, checkbox_listd_2, checkbox_listd_3) {//校验购物车
    const that = this;
    that.getcouponlist(1);
  ...
  increase: function(e){//增加
    ...
    setTimeout(function(){
      ...
      if (listtype == 1) {//现场送
        ...
        setTimeout(function(){
          if (wx.getStorageSync('plustype') == 0 && isneedinv == 1) {
            wx.setStorage({
              key: "plustype",
              data: 1
            });
        ...
        setTimeout(function(){
          if (wx.getStorageSync('plustype') == 0 && isneedinv == 1) {
            ...
            let index_mnb = checkbox_liste_1.findIndex(item => {//判断数组中是否存在当前数据,无：-1，有：返回下标
  ...
  testadd: function (cabCode, hotelId, hotelProdId, prod_Amt) {//检验商品是否可以增加数量
    const that = this;
    let linkData = {
      cabCode: cabCode,
      hotelId: hotelId,
