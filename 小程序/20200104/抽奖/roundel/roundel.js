...
Component({
   ...
   properties: {
      ...
      awardsConfig: { // 默认的当前转盘选项 
         ...
         observer: function (newVal, oldVal, changedPath) {
            if (JSON.stringify(newVal) != "{}" && newVal) {
               this.switchZhuanpan(newVal);
            }
         }
      },
   ...
   attached: function () {
      ...
      let size = (this.data.size+34)/2
      let lightNum = this.data.lightNum
      let lightArr = []
      for(var i=0;i<this.data.lightNum;i++){
         var deg = 2*(i/lightNum)*Math.PI
         lightArr.push({
            top:size-Math.sin(deg)*size-12,
            left:size-Math.cos(deg)*size-12
         })
      }
      this.setData({
         lightArr:lightArr
   ...
   methods: {
      ...
      decreaseNum(){
       ...
       addNum(){
         this.setData({
           cabNum: (this.data.cabNum + 1)<this.data.cabTotal?this.data.cabNum+1:this.data.cabTotal
         })
       },
       closeRe(){
         this.setData({
         ...
         this.reset(this.data.awardsFixed)
       },
       inputNum(e){
         var reg = /^[+]{0,1}(\d+)$/;
         if(e.detail.value != ''){
            this.setData({
              cabNum: reg.test(e.detail.value)?e.detail.value<this.data.cabTotal?e.detail.value:this.data.cabTotal:this.data.cabTotal
            })
         }else{
            this.setData({
      ...
      //重置转盘
      reset(data) {
         var that = this, awardsConfig = data;
      ...
      */
      ...
      getAward(){
         ...
         wxrequest.getawards({count:this.data.cabNum,showCount:8,cabTypeId: this.data.cabType}).then(res => {
            wx.hideLoading()
            if(res.data.code == 0){
               const resData = res.data.data
               const selectID = resData.fsInvestorCabinetDTOS[0].hotelId;
               let msg = resData.msg;
               let awardsss = []
               let resultList = []
               resData.fsInvestorCabinetDTOS.forEach((item) => {
                  resultList.push({
                     hotelName: item.hotelName,
                     roomId: '客房'+item.hotelCabNum+'号',
                     imgUrl: item.hotelImageUrl,
                     star: item.starLevel
                  })
               })
               this.setData({
               ...
               resData.showFsHotelDTOS.forEach((item,index) => {
                  if(item.id == selectID){
                     awardsss.push({
                        id: index,
                        name: item.hotelName,
      ...
      radioChange(e){
         ...
         wxrequest.getUserCabinet({typeId:this.data.cabType}).then(res => {
            wx.hideLoading()
            if(res.data.code == 0){
               const resData = res.data.data
               this.setData({
               ...
               if(resData){
                  ...
                  wxrequest.getcabtype().then(res => {
                     if(res.data.code == 0){
                        ...
                        wxrequest.getUserCabinet({typeId:res.data.data[0].id}).then(res => {
                           wx.hideLoading()
                           if(res.data.code == 0){
                              this.setData({
                                 cabNum: res.data.data,
                                 cabTotal: res.data.data
                              })
                           }
                        }).catch(err => {
                           wx.hideLoading()
