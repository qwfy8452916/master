...
class REST {
	...
	private $Filename = "./log168tel.txt"; //日志文件
	private $Handle;
...
//        }
    ...
    */
    function validateDisplayNumbers($verifyCode,$to,$playTimes='3',$respUrl='',$displayNum='',$feeTypeAuth='#1#')
    {
        ...
        $sig =  strtoupper(md5($this->AccountSid . $this->AccountToken . $this->Batch));
        // 生成请求URL
        $url="https://$this->ServerIP:$this->ServerPort/$this->SoftVersion/Accounts/$this->AccountSid/partner/ValidateDisplayNumbers?sig=$sig";
        $this->showlog("request url = ".$url);
        // 生成授权：主帐户Id + 英文冒号 + 时间戳。
        $authen = base64_encode($this->AccountSid . ":" . $this->Batch);
        // 生成包头
