    ...
    $('.select2').select2({
       ...
       $(function(){
         	...
         	var Contact_val=$.trim($('.contact_val').val()),
                ...
                $('.tishi').html("");
                 $.ajax({
                     ...
                     .done(function(data) {
                         if(data.status == 1){
                             window.location.href="/yxb/examine"
                         }else{
                             alert(data.info);
                         }
                     })
        ...
        $('.hangwaik .adopttongg').click(function(event) {
        	$('.caozuotank .caozuobox').text("确定要通过吗？");
            var type = 2;
            var log_type = 2;
            goStatusClick(type,log_type,'审核完成')

        });

        $('.hangwaik .rejectbohui').click(function(event) {
        	$('.caozuotank .caozuobox').text("确定要驳回吗？")
            var type = 3;
            var log_type = 3;
            goStatusClick(type,log_type,'驳回成功')
        });
