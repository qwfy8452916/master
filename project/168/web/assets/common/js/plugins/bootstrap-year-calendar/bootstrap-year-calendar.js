 ...
 * ========================================================= */
 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		_initializeOptions: function(opt) {
			...
			this.options = {
				...
				headerView: opt.headerView == null? 1:opt.headerView
			};
		...
		_initializeEvents: function(opt) {
			...
			if(opt.clickYear) { this.element.bind('clickYear', opt.clickYear); }
		},
		...
		_render: function() {
			...
			switch(this.options.headerView){
				case 1:
					this._renderHeader();
				break;
				case 2:
		...
		_renderHeader2:function(){
			var _this = this;
			var header = $(document.createElement('div'));
			header.addClass('calendar-header panel panel-default text-center calendar-header-new dropdown');

			var year = $(document.createElement('span'));
			year.addClass('year-title-new');
			year.text(this.options.startYear);
			header.append(year);

			var icon = $(document.createElement('i'));
			icon.addClass('fa fa-chevron-down year-icon');
			icon.attr("data-toggle","dropdown");

			header.append(icon);

			var years = $(document.createElement('ul'));
			years.addClass('dropdown-menu calendar-pop');

			var date=new Date;
 			var fullYear = date.getFullYear();
 			var start = 2016;

 			if (fullYear - start > 10) {
 				start = fullYear - 10;
 			}

			for (var i = 2016; i <= fullYear; i++) {
				...
				li.click(function(event) {
					year.text($(this).text());
					_this._triggerEvent('clickYear', {
						element: $(this),
						date: $(this).text()
					});
				});
				years.append(li);
				years.append("<li class='divider'></li>");
			}

			header.append(years);

			this.element.append(header);
			years.css("margin-left","-" + (years.outerWidth()/2)+"px");
	...
	$.fn.clickYear = function(fct) { $(this).bind('clickYear', fct); }
