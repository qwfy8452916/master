(function(factory) {
  ...
  $.cxSelect = function() {
    ...
    cxSelect.init = function() {
      ...
      self.settings = $.extend({}, $.cxSelect.defaults, _settings, {
        ...
        jsonMore: self.dom.box.data('jsonMore'),
    ...
    cxSelect.buildOption = function(select, data) {
      ...
      var _jsonMoreName = typeof select.data('jsonMoreName') === 'undefined' ? self.settings.jsonMoreName : String(select.data('jsonMoreName'));
      if(_jsonMoreName){
        var _htmlMoreName = typeof select.data('htmlMoreName') === 'undefined' ? self.settings.htmlMoreName : String(select.data('htmlMoreName'));
      }
      ...
      if (_jsonName.length) {
        ...
        if(_jsonMoreName){
          for (var i = 0, l = data.length; i < l; i++) {
            _html += '<option "'+ _htmlMoreName +'"="'+ String(data[i][_jsonMoreName]) +'" value="' + String(data[i][_jsonValue]) + '">' + String(data[i][_jsonName]) + '</option>';
          };
        }else{
          for (var i = 0, l = data.length; i < l; i++) {
  ...
  $.cxSelect.defaults = {
    ...
    jsonName: 'n',          // 数据标题字段名称
    jsonValue: 'v',          // 数据值字段名称
    jsonMoreName: '',          // 数据附加字段名称
    htmlMoreName: '',    // 数据附加自定义值名称
    jsonSub: 's'            // 子集数据字段名称
  };
