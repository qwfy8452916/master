    ...
    function savePswAction(event) {
        ...
        if( psw !== pswC ){
            ...
            successCallback : function (res) {
                if(res.status==1){
                    if(isWechat){
                        var hadusername = localStorage.getItem("username2");
                    }else{
                        var hadusername = store.get("username2");
                    }
                    if(hadusername){
                        if(isWechat){
                            localStorage.setItem("username", hadusername);
                        }else{
                            store.set("username", hadusername);
                        }
                    }
                    location.href = '/login'
                }else{
