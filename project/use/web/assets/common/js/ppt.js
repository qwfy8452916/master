(function(jq){
    jq.fn.ppt = function(settings) {
        var html = '<div class="cover"><div class="cover_bg"></div><div class="ppt"><a class="del" href="javascript:void(0)"></a><a class="lrow"  href="javascript:void(0)"></a><div class="lrow none"></div><div class="content"><ul id="pptUl">';
        ...
        ppt.fn.controlSize = function() {
            ...
            if(h >= 440) {
                ...
                ppt.oPpt.find(".info").css("top",(ch - 440)/2 + 445 + "px");
            } else {
        ...
        ppt.fn.hideUl = function() {
            ppt.fn.moveIe6css();
            ppt.oCover.remove();
            jq(window).unbind("DOMMouseScroll");
        };

        ppt.fn.changeImg = function(drec) {
            ppt.curNum = ppt.li.index(ppt.oCover.find("li."+settings.oClass));
            ...
            switch(drec) {
                ...
                ppt.li.removeClass(settings.oClass).addClass('none');
                ppt.li.eq(ppt.curNum).addClass(settings.oClass);
                ppt.describe.html(ppt.li.eq(ppt.curNum).attr("picname"));
                if(ppt.curNum == 0){
                    ppt.num.html(1+ "/" + ppt.liNum);
                }else{
                ...
                case "dRight":
                ppt.curNum += 1;
                if( ppt.curNum >= ppt.liNum){
                    ppt.curNum = ppt.liNum-1;
                }
                ppt.li.removeClass(settings.oClass).addClass('none');
                ppt.li.eq(ppt.curNum).addClass(settings.oClass);
                ppt.num.html(ppt.curNum+1 + "/" + ppt.liNum);
                ppt.describe.html(ppt.li.eq(ppt.curNum).attr("picname"));
                ppt.fn.showLrow();
                if(ppt.liNum-1 == ppt.curNum) {//last hide right
                    ppt.fn.hideRrow();
                }
                break;
            }
        ...
        ppt.fn.init = function() {
            ...
            var scrollFunc = function(e) {
                ...
                var detail = oEvent.originalEvent.wheelDelta?oEvent.originalEvent.wheelDelta:oEvent.originalEvent.detail;

                if (detail < 0) {
                    jq("a.lrow").trigger('click');
                }else{
                     jq("a.rrow").trigger('click');
                }
                return false;
            }

            if(jq(window)[0].addEventListener){
                jq(window).bind('mousewheel',scrollFunc);
                jq(window).bind('DOMMouseScroll', scrollFunc);
            }
