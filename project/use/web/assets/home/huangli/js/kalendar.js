...
/*************    方法     **************/
			...
			for(var n=0;n<7;n++){//列
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					...
					if((7*m+n+1-codeFirst == codeDay)&&(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6))){//当天是周末
						kalendar_html += "<li class='todayWeekend'><a href='/huangli/"+codeYear+"-"+thisMonth+"-"+(7*m+n+1-codeFirst)+".html'>"+(7*m+n+1-codeFirst)+"</a></li>";
					}else if(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6)){//仅是周末
						kalendar_html += "<li class='weekend'><a href='/huangli/"+codeYear+"-"+thisMonth+"-"+(7*m+n+1-codeFirst)+".html'>"+(7*m+n+1-codeFirst)+"</a></li>";
					}else if(7*m+n+1-codeFirst == codeDay){//仅是当天
						kalendar_html += "<li class='today'><a href='/huangli/"+codeYear+"-"+thisMonth+"-"+(7*m+n+1-codeFirst)+".html'>"+(7*m+n+1-codeFirst)+"</a></li>";
					}else{//其他日期
						kalendar_html += "<li><a href='/huangli/"+codeYear+"-"+thisMonth+"-"+(7*m+n+1-codeFirst)+".html'>"+(7*m+n+1-codeFirst)+"</a></li>";
					}
				}
			}
			kalendar_html += "</ul>\n";
	...
	function dateChange(dateObj,dateMonth,dateFirstDay,dateTotalDays,dateCurrentDay,thisyear){
		//判断新日历有几行,需要显示或隐藏
		...
		*/
		...
		for(var i=0;i<7*newLine;i++){
			if(i < dateFirstDay || i> (dateTotalDays+dateFirstDay-1)){//日历中 当月不存在的日期
				...
				dateObj.eq(i).html("<a href='/huangli/"+thisyear+"-"+(dateMonth+1)+"-"+(i+1-dateFirstDay)+".html'>"+(i+1-dateFirstDay)+"</a>");
...
/***************************/
	...
	yearPrev.bind('click',function(){
		...
		//修改 日期 列表
		dateChange(dateDay,someDay['month'],someDay['firstDay'],monthDays[someDay['month']],today['date'],yearChange);	
	});
