...
class Verify {
     ...
     */
    public function check($code, $id = '',$isReset = ture) {
    ...
    private function _writeCurve() {
        $_color = imagecolorallocate($this->_image, mt_rand(1,150), mt_rand(1,150), mt_rand(1,150));
        $px = $py = 0;

        // 曲线前部分
        $A = mt_rand(1, $this->imageH/1.1);                  // 振幅
        $b = mt_rand(-$this->imageH/2, $this->imageH/2);   // Y轴方向偏移量
        $f = mt_rand(-$this->imageH/2, $this->imageH/2);   // X轴方向偏移量
        ...
        for ($px=$px1; $px<=$px2; $px = $px + 1) {
            if ($w!=0) {
                ...
                while ($i > 0) {
                    imagesetpixel($this->_image, $px + $i , $py + $i, $_color);  // 这里(while)循环画像素点比imagettftext和imagestring用字体大小一次画出（不用这while循环）性能要好很多
                    $i--;
                }
            }
        }
        ...
        for ($px=$px1; $px<=$px2; $px=$px+ 1) {
            if ($w!=0) {
                ...
                $i = (int) ($this->fontSize/5);
                while ($i > 0) {
                    imagesetpixel($this->_image, $px + $i, $py + $i, $_color);
                    $i--;
                }
            }
        }
