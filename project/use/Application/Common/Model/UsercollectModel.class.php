    ...
    public function getCollectList($id,$type = "",$pageIndex,$pageCount)
    {
        ...
        switch ($type) {
            ...
            case '2':
                ...
                $threeSql = M("user_collect")->table($buildSql)->alias("t5")
                                          ->join("join qz_xiaoguotu_threedimension as m on m.id = t5.classid and t5.classtype = 7")
                                          ->field("t5.time,m.id as cid,m.title,'' as fengge,'' as cs,'7' as classtype,  '' as classname,'' as bm,'' as img_host, m.face as img_path, '' as img")
                                          ->order("m.id desc")
                                          ->buildSql();
