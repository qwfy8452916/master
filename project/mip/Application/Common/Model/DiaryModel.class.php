...
class DiaryModel extends Model{
    ...
    public function getMobileIndexDiaryList($limit = 3)
    {
        ...
        return M()->table($build)->alias('d')
                                 ->field('d.*, u.name AS user_name, u.logo AS user_logo')
                                 ->join('qz_user u ON u.id = d.user_id')
                                 ->select();
    }
    ...
    public function get_all_diary_list_api($limit = '',$cid =''){
        ...
        if($count)
        {
            ...
            $list =  M("diary_info")->table($buildSql)->group("t1.parent_id")->alias("t1")->order("t1.parent_id desc,t1.diary_time DESC")->limit($start.",".$pageCount)->select();
