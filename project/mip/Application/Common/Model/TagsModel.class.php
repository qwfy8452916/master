...
class TagsModel extends Model{
    ...
    public function getTags($id){
        return M('tags')->field('*')->where("id= '$id' ")->find();
    }
    ...
    public function getAskList($condition,$pagesize= 1,$pageRow = 10){
        ...
        if(isset($condition['tags'])){
            $tags = $condition['tags'];
            $tagName = M('tags')->field('id,name')->where(" id='$tags' ")->find();
    ...
    public function setTagsNum($id,$act,$type){
        ...
        if($act == 'Inc'){
            return M("tags")->where(" id=$id ")->setInc($type);
        }else{
            return M("tags")->where(" id=$id ")->setDec($type);
        }
    }
