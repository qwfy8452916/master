...
class ConstructionModel extends Model{
    ...
    public function _initialize(){
        //获取装修位置
        $this->location = S("Cache:m:Constructionlocation");
        if(!$this->location){
            $result =  M("construction_location")->field("id,name,jc,orders")->order("orders")->select();
            if(count($result)>0){
                $this->location =  $result;
                S("Cache:m:Constructionlocation",$result,3600*24);
            }
        }
        //获取装修施工明细
        $this->details = S("Cache:m:Constructiondetails");
        if(!$this->details){
            $result =  M("construction_details")->field("id,name,location,range,units,fangshi,parentid")->select();
            if(count($result)>0){
                $this->details =  $result;
                S("Cache:m:Constructiondetails",$result,3600*24);
            }
        }

        //获取建材信息
        $this->materials = S("Cache:m:Constructionmaterials");
        if(!$this->materials){
            $result =  M("construction_materials")->select();
            if(count($result)>0){
                $this->materials =  $result;
                S("Cache:m:Constructionmaterials",$result,3600*24);
            }
        }
    }
