...
class TagsModel extends Model{
    ...
    public function getTags($id){
        ...
        return M('tags')->field('*')->where($map)->find();
    }
    ...
    public function getAskList($condition,$pagesize= 1,$pageRow = 10){
        ...
        if(isset($condition['tags'])){
            ...
            $tagName = M('tags')->field('id,name')->where($map)->find();
    ...
    public function getCount($type,$tags){
        ...
        if($type == 'ask'){
            $tagName = M('tags')->field('id,name')->where(["id"=>["EQ",$tags]])->find();
            $tagName = $tagName['name'];
            $map['_string']= " find_in_set('$tagName',REPLACE(tags_name,',',',')) ";
    ...
    public function setTagsNum($id,$act,$type){
        ...
        if($act == 'Inc'){
            return M("tags")->where($map)->setInc($type);
        }else{
            return M("tags")->where($map)->setDec($type);
        }
    }
