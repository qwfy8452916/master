...
class AskModel extends Model{
    ...
    public function getNewQuestion($num){
        $map['visible'] ='0';
        $build = M('Ask')->field('id,uid,sub_category,title,post_time,views,anwsers,status')->where($map)->order('post_time DESC')->limit("0,".$num)->buildSql();
        return M()->table($build)->alias('a')
                                 ->field('a.*, c.name AS sub_category_name, u.name AS user_name, u.logo AS user_logo')
    ...
    public function getQuestionAndUserList($condition=[],$orderBy=['post_time'=> 'DESC'],$start= 1,$row = 10){
        //如果条件不为空
    ...
    public function getQUCList($map,$pagesize= 1,$pageRow = 10){
        ...
        $count = $DB->alias("a")->where($map)->count();
        $result = $DB->alias("a")
                      ->field('a.id,a.title,a.anwsers,a.views,a.post_time,c.name,c.cid,u.name username,u.logo')
                      ->join("inner join qz_ask_category as c on a.sub_category = c.cid")
