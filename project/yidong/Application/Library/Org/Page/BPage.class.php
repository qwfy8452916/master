...
class BPage {
    ...
    public function show(){
        ...
        if($this->pageRowCount > 0){
            ...
            $exp = array_flip($exp);
            $config = array(
                    "prev"=>str_replace("%LINK%","http://".$link.$pageLink."b=".($this->pageIndex-1),$this->config["prev"]),
                    "first"=>str_replace("%LINK%","http://".$link.$pageLink."b=1",$this->config["first"]),
                    "link_page"=>$buttons,
                    "last"=>str_replace("%LINK%","http://".$link.$pageLink."b=".$this->pageTotalCount,$this->config["last"]),
                    "next"=>str_replace("%LINK%","http://".$link.$pageLink."b=".($this->pageIndex+1),$this->config["next"]),
                    "down_page"=>$this->config["down_page"],
                    "header"=>str_replace("%TOTAL_ROW%", $this->pageRowCount, $this->config["header"])
                            );
