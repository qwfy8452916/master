    ...
    public function show(){
        ...
        if($this->pageRowCount > 0){
            ...
            $config = array(
                    ...
                    "first"=>str_replace("%LINK%","http://".rtrim(rtrim($link.$pageLink,'?'),'&'),$this->config["first"]),
                    "link_page"=>$buttons,
            ...
            if($this->pageIndex-1 == 1){
                $config['prev'] = str_replace("%LINK%","http://".rtrim(rtrim($link.$pageLink,'?'),'&'),$this->config["prev"]);
            }


            $config['last'] = '<a href="javascript:;">…</a><a href="http://'.$link.$pageLink."p=".$this->pageTotalCount.'">'.$this->pageTotalCount.'</a>';
