...
class QqOAuth2 extends BaseClient{
	...
	protected function _tokenFilter($response){
		...
		if (!is_array($token) || isset($token['error']) ) {
			...
			throw new \Think\Exception("get access token failed." . $token['error'] . ':' . $token['error_description']);
		}
	...
	public function getOpenid($access_token){
		...
		if (isset($json['error'])){
			...
			//throw new \Think\Exception("get access token failed." . $json['error']);
		}
	 ...
	 */
	public function getUserInfo($url,$method){
		$result = $this->http($url,$method);
