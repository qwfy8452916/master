...
<link rel="stylesheet" href="<?php echo ($static_host); ?>/assets/mobile/common/fonts/font-awesome.min.css?v=<?php echo C('STATIC_VERSION');?>" />
    ...
    <link href="/assets/mobile/meitu/css/meituztlist_m.2.8.4.css?v=<?php echo C('STATIC_VERSION');?>" rel="stylesheet" type="text/css" />
...
<div class="new-kefu" id="new-kefu">
    ...
    <div class="swiper-slide swiper-slide-duplicate" data-swiper-slide-index="4" style="width: 375px;">
     ...
     <?php if(is_array($banner)): $i = 0; $__LIST__ = $banner;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="swiper-slide">
             <a href="<?php echo ($vo["url"]); ?>"><img src="http://<?php echo C('QINIU_DOMAIN');?>/<?php echo ($vo["thumb"]); ?>" alt="<?php echo ($vo["title"]); ?>" /><p><?php echo ($vo["title"]); ?></p></a>
       ...
       <div class="zhutinr">
       	  <ul class="list">
              <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><!--$i是从1开始的-->
                  <?php if($i % 2 == 1): ?><li class="bianjv">
                          <a href="/zt/<?php echo ($vo["id"]); ?>.html">
                              <img src="http://<?php echo C('QINIU_DOMAIN');?>/<?php echo ($vo["thumb"]); ?>" alt="">
                              <div class="title_ms"><?php echo (mbstr($vo["title"],0, 10, 'utf-8', false)); ?></div>
                          </a>
                      </li>
                  <?php else: ?>
                      <li>
...
<script type="text/javascript" src="/assets/mobile/common/js/disclaimer.js?v=<?php echo C('STATIC_VERSION');?>"></script>
    ...
    $('.zhutinr').dropload({
        ...
        loadDownFn : function(me){
            ...
            $.ajax({
                type: 'GET',
                url: 'http://m.qizuang.com/meitu/meituztlist?p='+page,
                dataType: 'json',
                success: function(data){
                    if (data.status == 1) {
                        ...
                        for(var i=0; i<data.length; i++){
                            if(i % 2 == 0){
                                result = result + '<li class="bianjv"><a href="/zt/'+ data[i].id +'.html"><img src="http://<?php echo C('QINIU_DOMAIN');?>/'+ data[i].thumb +'" alt="">' +
                                    '<div class="title_ms">'+ data[i].title +'</div></a></li>';
                            }else{
                                result = result + '<li><a href="/zt/'+ data[i].id +'.html"><img src="http://<?php echo C('QINIU_DOMAIN');?>/'+ data[i].thumb +'" alt="">' +
                                    '<div class="title_ms">'+ data[i].title +'</div></a></li>';
                            }
                        }
                    // 如果没有数据
