...
<link href="/assets/mobile/baojia/m-zxbj.css?v=<?php echo C('STATIC_VERSION');?>" rel="stylesheet" type="text/css" />
...
<div class="new-kefu" id="new-kefu">
    ...
    <article>
        ...
        <div class="form-once">
            ...
            <!-- 立即计算报价按钮 -->
            <a href="javascript:void(0)" class="m-b-btn save-submit">
                立即计算报价
            </a>
        </div>
        <!-- from2表单填写 -->
    ...
    $('.save-submit-again').on('click',function(){
        ...
        window.order({
            ...
            success:function(data, status, xhr){
                if(data.status == 1){
                    if(src){
                        window.location.href = "http://<?php echo C('MOBILE_DONAMES');?>/details?src="+src;
                    }else{
