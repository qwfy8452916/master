...
<link href="http://<?php echo ($fenzhan); ?>/" rel="canonical" />
...
<body>
    ...
    <?php if($header == ''): ?><a href="/<?php echo ($cityInfo["bm"]); ?>/" class="m-header-left gl-logo"></a>
        <div class="m-header-city"><a href="/city/"><?php echo ((isset($cityInfo["name"]) && ($cityInfo["name"] !== ""))?($cityInfo["name"]):"全国"); ?><i class="fa fa-sort-desc"></i></a></div>
    <?php else: ?>
        <a class="m-header-left" href="http://<?php echo C('MOBILE_DONAMES');?>"></a>
        <div class="m-header-city">·<?php echo ($header); ?></div><?php endif; ?>
...
<div class="new-kefu" id="new-kefu">
    ...
    <article>
        ...
        <div class="white_bg">
          <div class="swiper-container index-top-banner">
            ...
            <ul class="home-nav-bottom">
               ...
               <li>
                    <a href="/<?php echo ($cityInfo["bm"]); ?>/xgt/">
                       <div class="nav_bottom_content">
        ...
        <?php if(count($info['zixun']) > 0): ?><div class="home-sec">
            <div class="home-sec-title">
                <h2 class="pull-left">本地资讯</h2>
                <a href="/<?php echo ($bm); ?>/zxinfo/bendi/" class="more pull-right">更多></a>
            </div>
            <div class="bd_list">
                <?php if(is_array($info["zixun"])): $i = 0; $__LIST__ = $info["zixun"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$oo): $mod = ($i % 2 );++$i;?><div class="fg-list-box">
                        <div class="fg-list clearfix">
                            <a href="http://m.<?php echo C('QZ_YUMING');?>/<?php echo ($bm); ?>/zxinfo/<?php echo ($oo["id"]); ?>.html">
                                <div class="fg-img">
                                    <img src="<?php echo ($oo["face"]); ?>">
                                </div>
                                <div class="fg-send">
                                    <div class="titlebt"><?php echo ($oo["title"]); ?></div>
                                    <p><?php echo ($oo["content"]); ?></p>
                                    <p class="date_time"><?php echo (date("Y-m-d H:i:s",$oo["addtime"])); ?></p>
                                </div>
                            </a>
                        </div>
                    </div><?php endforeach; endif; else: echo "" ;endif; ?>
      ...
      var mySwiper = new Swiper('.index-top-banner', {
        ...
        $('.save-submit').on('click',function() {
            ...
            if (!App.validate.run(mianji)) {
                ...
                if(!re.test(mianji) || mianji>10000 || mianji<1){
                    $(".m-bj-edit input[name=mianji]").focus();
                    $(".m-bj-edit input[name=mianji]").val('');
            ...
            if (!App.validate.run(tel)) {
                ...
                if(!reg.test(tel)){
                    $(".m-bj-edit input[name=tel-number]").focus();
                    $(".m-bj-edit input[name=tel-number]").val('');
                    alert("请填写正确的手机号码 ^_^!");
                    return false;
                }
                if(reg2.test(tel)){
