...
<link rel="stylesheet" href="<?php echo ($static_host); ?>/assets/mobile/common/fonts/font-awesome.min.css?v=<?php echo C('STATIC_VERSION');?>" />
    ...
    <link rel="stylesheet" href="<?php echo ($static_host); ?>/assets/mobile/cases/css/m-zxgs-case-list.css?v=<?php echo C('STATIC_VERSION');?>" />
    ...
    <article class="under-line">
        <div id="m-case">
            <div class="m-case-tit">
                <p class="home-tit"><span><?php echo ($info["case"]["title"]); ?></span>·<span><?php echo ($info["case"]["fengge"]); ?></span>·<span><?php echo ($info["case"]["mianji"]); ?> m²</span></p>
                <p  class="company-tit"><?php echo ((isset($info["case"]["comapny"]) && ($info["case"]["comapny"] !== ""))?($info["case"]["comapny"]):"装修案例"); ?></p>
            </div>
            <div class="m-case-con">
                <ul class="grid">
                    <?php if(is_array($info["case"]["child"])): $i = 0; $__LIST__ = $info["case"]["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                            <div class="zxpic">
                                <?php if($vo['img_host'] == 'qiniu'): ?><img alt="<?php echo ($vo["title"]); ?>图<?php echo ($key+1); ?>" src="http://<?php echo C('QINIU_DOMAIN');?>/<?php echo ($vo["img_path"]); ?>-w640.jpg">
                                <?php else: ?>
                                    <img alt="<?php echo ($vo["title"]); ?>图<?php echo ($key+1); ?>" src="http://<?php echo C('STATIC_HOST1'); echo ($vo["img_path"]); ?>m_<?php echo ($vo["img"]); ?>"><?php endif; ?>
                                <p class="zxpic-tit"><?php echo ($vo["title"]); ?>图<?php echo ($key+1); ?></p>
                            </div>
