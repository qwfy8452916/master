...
function() {
    ...
    $.widget("rili.dayDetail", {
        ...
        _create: function() {
            ...
            this.tmpl.push('<span class="mh-date">公历 {year}-{padmonth}-{paddate}</span>'),
        ...
        fillLuckyData: function(e) {
			...
			var yj = new Array();
			var yq = new Array();
			var mystr = new Array("修造","安床","解除","入宅","拆卸","坏垣","动土","移徙","起基","安门","开池");			
			for (x in o){
				if(mystr.indexOf(o[x]) != -1){
					yq.push(o[x]);
				}
			}
			for (x in s){
				if(mystr.indexOf(s[x]) != -1){
					yj.push(s[x]);
				}
			}			
            $(".mh-suited .mh-st-items").html(t(s)).attr("title", yj.join(" ")),
            $(".mh-tapu .mh-st-items").html(t(o)).attr("title", yq.join(" ")),
			$(".mh-suited .mh-st-items").html(yj.join(" ")),
            $(".mh-tapu .mh-st-items").html(yq.join(" "))
        }
    })
})(); (function() {
