...
class UsercollectModel extends Model{
    ...
    public function getOne($id,$userid, $type)
    {
        $map = array(
            "classid" => array("EQ", $id),
        ...
        return M("user_collect")->where($map)->find();
    }
    /**
     * 添加收藏
     */
     ...
     */
    public function delcollect($id, $userid, $type)
    {
        $map = array(
            "classid" => array("EQ", $id),
            "userid" => array("EQ", $userid),
    ...
    /**
     * [getCollectIdsByIds 根据目标ID获取已收藏的目标ID]
     * @param  string $classid [关注对象编号]
     * @param  string $userid  [用户编号]
     * @param  string $type    [类型]
     * @return [type]          [description]
     */
    public function getCollectIdsByIds($classid = '', $userid = '', $type = '')
    {
        if (empty($classid) || empty($userid) || empty($type)) {
            return false;
        }
        if (!is_array($classid)) {
        ...
        $map = array(
            "classid"   =>array("IN",$classid),
            "userid"    =>array("EQ",$userid),
            "classtype" =>array("EQ",$type)
        );
        $temp = M("user_collect")->field('classid')->where($map)->select();
        $result = array();
        ...
        switch ($type) {
            ...
            case '2':
                ...
                $meituSql =  M("user_collect")->table($buildSql)->alias("t5")
                                              ->join("INNER JOIN qz_meitu as m on (m.id = t5.classid) and (t5.classtype = 4) and (m.state = 1)")
