...
class UserModel extends Model{
    ...
    public function getUserInfoById($id = '',$cs=''){
        ...
        return M("user")->table($buildSql)->alias("t3")
			                 ...
			                 //获取计划任务中的公司好评率
			                 ->join("left join qz_user_company_rank uc1 on uc1.comid = t3.id and uc1.day = '$yesterday'")
                             ->field("t3.*, h.id as hid ,h.shortname,h.tel as htel,h.addr,h.qq as qq3,h.qq1 as qq4,h.nickname as nickname2,h.nickname1 as nickname3,uc1.ping,uc1.haoping")
                             ->select();
    ...
    */
    ...
    public function getDesingerListByCity($condition,$page = 1,$pageCount = 10,$order = 'orders desc,id desc' )
    {
        ...
        return M("user")->where($map)->alias("a")
            ...
            ->field("a.id as uid,a.name,a.cs,a.pv,a.logo,c.*,d.qc,d.id as comid,e.bm,b.zw")
            ->page("$page,$pageCount")
            ->order($order)
            ->select();
    }
