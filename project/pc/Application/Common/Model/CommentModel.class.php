    ...
    */
    ...
    public function getRemovalCommentsByCity($limit,$city_id){
        ...
        $buildSql = M("comment")->where($map)->alias("a")
            ...
            ->join("INNER JOIN qz_quyu as d on d.cid = c.cs")
            ->join("INNER JOIN qz_user as b on a.userid = b.id")
            ->order("paixu,a.recommend DESC,a.id desc")
        ...
        $sql = M("comment")->table($buildSql)->alias("t")->group("t.comid")->order("t.paixu,t.recommend DESC,t.id DESC")->buildSql();
        $sql = M("comment")->table($sql)->alias("t")->limit($limit)->group("t.userid")->order("t.paixu,t.recommend DESC,t.id desc")->buildSql();

        $result = M("comment")->table($sql)->alias("t1")->order("t1.paixu,t1.recommend DESC,t1.id desc")->select();
        if(count($result) > 0){
            foreach ($result as $key => $value) {
                $ids[] = $value["comid"];
            }
