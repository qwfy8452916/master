...
class InfoModel extends Model{
    ...
    public function getArticlesByComIdCount($comid ='',$cs = '',$type ='',$active = '',$notClass ="",$on = true){
        ...
        $dynamicKey_on = $on ? '1' : '0';
        $dynamicKey = 'Cache:Info:'.':comid'.$comid.':'. md5(':c'.$city.':t'.$type.
                      ':a'.$active.':nc'.serialize($notClass).':on'.$dynamicKey_on);

        $cacheCount = S($dynamicKey);

        if (!$cacheCount) {
            $cacheCount = M("info")->where($map)->alias("a")
                             ->order("a.id desc,ckinfo desc")
                             ->count();
            S($dynamicKey, $cacheCount, 60 * 5);
        }
