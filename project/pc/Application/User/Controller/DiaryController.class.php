...
class DiaryController extends UserBaseController
{
    ...
    public function add_follow_diary(){
        ...
        $tags_type=D("Diary")->get_tags_type($info['user']['id']);//查询标签列表
        $this->assign("tags_type",$tags_type);//获取标签列表
    ...
    public function add_follow_diary_do($value=''){
        ...
        //接收图片进行入库
        $id=I('post.id')+0;
        $field=array('title','first_type','second_type','tags','is_over','content','diary_time');//规定要接受的字段
        ...
        import('Library.Org.Util.Fiftercontact');
        $filter = new \Fiftercontact();
        $data["content"] = $filter->filter_common($data["content"],array("Sbc2Dbc","filter_script","filter_link","filter_url"));
        ...
        $my_tags=trim(I('post.my_tags'));//接收自定义标签
        ...
        //         $link =  "<a href='".$keywords[$key]["href"]."' target='_blank'>".$keywords[$key]["name"]."</a>";
        //         $data['content'] = preg_replace($value,$link,$data['content'],1);
        //         $i ++;
        //     }
        // }

        $data['tags']=$diary_model->get_real_tags($data['tags'],$my_tags);//传入接收tags和自定义tags
    ...
    public function edit_diary(){
        ...
        $diary_type=D('Diary')->get_diary_type();//查询日记类型
        $this->assign("diary_type",$diary_type);//赋值日记类型
        $tags_type = D("Diary")->get_tags_type($info['user']['id']);//查询我的标签列表

        foreach ($tags_type as $key => $value) {
            if($value['id'] = $diary_info['tags']){
                $diary_info['tagsName'] = $value['name'];
            }
        }
    ...
    public function edit_diary_do(){
        //接收图片进行入库
        $id=I('post.id')+0;
        $field=array('title','first_type','is_over','second_type','tags','content','diary_time','oldTags');//规定要接受的字段
        foreach ($field as $k => $v){
            $data[$v]=htmlspecialchars(trim($_POST[$v]));//接收这些字段
        }
        $data['diary_time']=strtotime($data['diary_time']);//转换日记时间
        ...
        $oldTags = $data['oldTags'];
         ...
         //Tags处理：新标签，旧标签，类型
        $data["tags"] = $this->getTags($my_tags,$oldTags,'3');
    ...
    public function write_diary_second(){
        ...
        $field=array('title','first_type','is_over','second_type','tags','content','diary_time');//规定要接受的字段
        foreach ($field as $k => $v){
            $data[$v]=htmlspecialchars(trim($_POST[$v]));//接收这些字段
        }
        $diary_model=D('Diary');
    ...
    public function write_diary_first(){
        ...
        $diary_type=D('Diary')->get_diary_type();//查询日记类型
        $this->assign("diary_type",$diary_type);//赋值日记类型
        $tags_type=D("Diary")->get_recom_tags();//查询标签列表
        $this->assign("tags_type",$tags_type);//获取标签列表
        $this->assign('info',$info);
    ...
    /**
     * [get_tags_type 获取标签列表]
     * @param  integer $diary_id [日记id]
     * @return [array]           [标签列表]
     */
    private function get_tags_type($diary_id=0){
        # 返回用户使用过的tags和我们推荐的tags
        # 用户使用过的tags 有
        $info["user"] = $this->baseInfo;//获取用户信息
        $id=$info["user"]['id'];//取得用户id
        $list=D('Diary')->get_tags_type($id,$diary_id);//获取标签列表
        return $list;//返回标签列表
    }
    /**
     * 检验是否是ajax的post提交
    ...
    private function getTags($tagsName,$oldTags,$type){
        ...
        //如果数据库中已存在此Tags
        if(!empty($isInTags)){
            //如果输入的标签等于之前的标签 不做任何处理
            if($isInTags['id'] == $oldTags){
                return $isInTags['id'];
            }
            $tags = $isInTags['id'];
            //新标签统计 加1
            D("Tags")->setTagsNum($tags,'Inc',$type);
        }else{
            //不存在，增加
            $tagData = array("name" => $tagsName,"type" => $type,"time" => time(),$countField => '1',);
            $tags = D("Tags")->addTags($tagData);
        }
        //之前的Tags统计 减1
        if(empty($oldTags)){
            D("Tags")->setTagsNum($oldTags,'Dec',$type);
        }
        return $tags;
    }
