...
class MeituController extends HomeBaseController{
    ...
    public function meitulist(){
        ...
        if(array_key_exists($_SERVER['REQUEST_URI'], $redirectArray)){
            $url = 'http://'.C('QZ_YUMINGWWW').$redirectArray[$_SERVER['REQUEST_URI']];
            header( "HTTP/1.1 301 Moved Permanently" );
            header( "Location:".$url);
            die();
        }
        ...
        if(!empty($_GET['p']) && !isset($_GET['a1'])){
            $url = 'http://'.C('QZ_YUMINGWWW').'/meitu/list-l0f0h0c0p'.I('get.p');
            header( "HTTP/1.1 301 Moved Permanently" );
            header( "Location:".$url);
            die();
        }
        if(!empty($_GET['q']) && !isset($_GET['a1'])){
            $url = 'http://'.C('QZ_YUMINGWWW').'/meitu/list-l0f0h0c0q'.I('get.q');
            header( "HTTP/1.1 301 Moved Permanently" );
            header( "Location:".$url);
            die();
        }
...
/*
        ...
        //设置canonical属性
        $header['canonical'] = 'http://'.C('QZ_YUMINGWWW').'/meitu/p'.$p.'.html';
