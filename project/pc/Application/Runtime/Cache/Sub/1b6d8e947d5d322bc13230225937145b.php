<?php if (!defined('THINK_PATH')) exit();?><!-- S 免费电话咨询 -->
<style>
.freetel{ width:525px; height:136px; position:fixed; bottom:50px; left:-404px; background:url(/assets/common/img/freetel.png) top right; z-index: 999;
transition:all 0.5s;
...
<div class="freetel">
    <div class="freetelbd">
        <input name="freetel" type="text" placeholder="此输入您的号码" maxlength="11">
...
$(document).ready(function() {
    ...
    $(".freetelbd").children(".btn").click(function(event){
        ...
        if ($("input[name=freetel]").val() == "" || $("input[name=freetel]").val().length == 0) {
            $("input[name=freetel]").focus();
            $("input[name=freetel]").val('');
            $.pt({
                    target: $("input[name=freetel]"),
                    content: '还没填写您的手机号噢 ^_^!',
                    width: 'auto'
            });
            return false;
        } else {
    ...
    $(".freeyzm").children(".tijiao").click(function(event){
        ...
        $.ajax({
            ...
            data: {
                code: safecode,
                cs : '<?php echo ($cityInfo["id"]); ?>'|| '000001',
                tel : tel,
                source : 99,
