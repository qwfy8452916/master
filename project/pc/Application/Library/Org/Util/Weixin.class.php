...
class Weixin {
    ...
    public function __construct($prefix = '') {
        ...
        $appenv = C('APP_ENV');
        if ('dev' == $appenv) {
            $this->config['validate_token']     = OP('WX_QZ_FW_TOKEN_DEV'); 
            $this->config['encoding_aes_key']   = OP('WX_QZ_FW_ENCODINGAESKEY');
            $this->config['app_id']             = OP('WX_QZ_FW_APPID_DEV');
            $this->config['app_secret']         = OP('WX_QZ_FW_APPSECRET_DEV');
        } else {
            //基本配置信息
...
//            }
            //从数据库拿取配置信息
            $defaultconfig = D('Common/Logic/YyWxManagerLogic')->getWxInfoByWxid('qizuangwx');   //此处先用诸葛装修的
            $this->config['app_id'] = $defaultconfig['appid'];
            $this->config['app_secret'] = $defaultconfig['appsecret'];
            $this->config['encoding_aes_key'] = $defaultconfig['encodingaeskey'];
            $this->config['validate_token'] = $defaultconfig['token'];
        }
        ...
        if (empty($cache)) {
            //如果S缓存中没有就从数据库中取
            $cache = $this->option($this->config['app_id']);
            if (empty($cache)  || time() > $cache['expires_in']) {
                //如果数据库中没有，  或者时间到期就重新获取
                ...
                if($result){
                    ...
                    $data = array(
                        ...
                        "expires_in" => time() + $info['expires_in'] - 100,
                    );
                    $this->option($this->config['app_id'], $data);
                    $cache = $data;
                    if (isset($info['expires_in'])) {
                        $expire = $info['expires_in'] - 100 ;
                    } else {
                        $expire = $cache['expires_in'] - time();
                    }
                    S('Class:Weixin:' . $prefix . ':ACCESS_TOKEN', $cache, $expire);
                }
            }
        }
        $this->access_token['token'] = $cache['token'];
    ...
    */
    ...
    private function option($key = '', $value = [])
    {
        ...
        if (empty($value)) {
            return $Wechat = D("WechatToken")->getLastToken($key);
        }
        
        if (!isset($value['appid'])) {
            $value['appid'] = $key;
        }
        return D("WechatToken")->addToken($value);
    }
}