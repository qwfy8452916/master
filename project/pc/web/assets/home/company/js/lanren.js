	...
	function slide(slideMode){//轮播方法
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				hidden.css({width:"500px",height:"282px",top:"40px","left":"0px","opacity":0});
				hidden.stop(true,true).animate({width:"500px",height:"282px",top:"40px",left:"0px",opacity:1},timer);
				curr.stop(true,true).animate({width:"500px",height:"282px",top:"40px",left:"700px",opacity:1},timer);
				next.stop(true,true).animate({width:"500px",height:"282px",top:"40px","left":"730px","opacity":0},timer,function(){next.find("span").css("opacity",0); isMoving = false;});
				//prev  -->  curr     prev中的图片li轮换到curr的位置      其他一次轮换
				prev.find("span").css("opacity",1);
				$(".main_banner_box li").find(".bottom_title").css({"bottom":"-50px"});//所有标题隐藏
				prev.stop(true,true).animate({width:"800px",height:"360px",left:"165px",top:0,opacity:1},timer,function(){
					$(this).find(".bottom_title").animate({"bottom":"0px"});	//当前这张图片的标题运动出来
				});
				index_2--;
			}else{			//执行next 操作
				...
				hidden.css({width:"500px",height:"282px",top:"40px","left":"730px","opacity":0});
				hidden.stop(true,true).animate({width:"500px",height:"282px",top:"40px",left:"700px",opacity:1},timer);
				curr.stop(true,true).animate({width:"500px",height:"282px",top:"40px",left:"0px",opacity:1},timer);
				//next  -->  curr     next中的图片li轮换到curr的位置      其他一次轮换
				next.find("span").css("opacity",1);
				$(".main_banner_box li").find(".bottom_title").css({"bottom":"-50px"});//所有标题隐藏
				next.stop(true,true).animate({width:"800px",height:"360px",left:"165px",top:0,opacity:1},timer,function(){
					$(this).find(".bottom_title").animate({"bottom":"0px"});	//当前这张图片的标题运动出来
				});
				prev.stop(true,true).animate({width:"500px",height:"282px",left:"0px",top:"40px",opacity:0},timer,function(){
					isMoving = false;
				});
				index_2++;
			}//if else

			hidden.find("span").css("opacity",0);
			curr.find("span").css("opacity",0);
