$(function() {
	...
	function nextclick() {
		...
		if (currrentindex != (length - 1)) {
			...
			var time = picshowtop.find(".bigshow").eq(currrentindex+1).find('img').attr("data-time");
			var collect = picshowtop.find(".bigshow").eq(currrentindex+1).find('img').attr("data-collect")?true:false;
			setValue(id,title,time,collect);
		} else {
			if($("#nexturl").attr("href") != 'http://meitu.qizuang.com/p.html'){
	...
	function preclick() {
		...
		if (currrentindex != 0) {
			...
			if($("#preurl").attr("href") == 'http://meitu.qizuang.com/p0.html'){
	...
	getli.click(function() {
		...
		var time = $(".picshowtop").find(".bigshow").eq(currentliindex).find('img').attr("data-time");
		var collect = $(".picshowtop").find(".bigshow").eq(currentliindex).find('img').attr("data-collect")?true:false;
		setValue(id,title,time,collect);
	});
	...
	function setValue(id,title,time,collect){
		var id = id;
		var title = title;
		var time = time;
		var collect = collect;
		document.title = title;
		$('.meitu-collect a').attr('data-id',id);
		$('.publish-time').text(time);
		$('.show-title a').attr('title',title);
		$('.show-title a').text(title);
		if(collect){
			$('.meitu-collect a').addClass('collect-bind');
			$('.meitu-collect a').text('已收藏');
			$('.meitu-collect a').attr('data-on',1);
		}else{
			$('.meitu-collect a').removeClass('collect-bind');
			$('.meitu-collect a').text('收藏');
			$('.meitu-collect a').attr('data-on',0);
		}
