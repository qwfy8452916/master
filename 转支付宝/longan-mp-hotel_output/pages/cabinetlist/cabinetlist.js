...
Page({
  ...
  data: {
    ...
    navtext: [{
      id: 0,
      name: '安装列表',
      quanxian: 'F:MH_CAB_INSTALLLISTTAB'
    }, {
      id: 1,
      name: '报修单',
      quanxian: 'F:MH_MAL_REPAIRTAB'
    }, {
      id: 2,
      name: '更换柜子',
      quanxian: 'F:MH_MAL_REPLACETAB'
    }],
    navindex: 0,
    //导航索引
    Tabindex: ''
  ...
  onLoad: function (options) {
    ...
    that.getreplacedatalist(that.data.replacepage); // if (options.navindex==1){
  ...
  getrepairdatalist: function (repairpage) {
    ...
    wx2my.request({
      url: apiUrl + '/mal?pageSize=20'+'&pageNo='+repairpage+'&dealStatus=0'+'&orgAs=3',
      // data: {
      //   pageSize: 20,
      //   pageNo: repairpage,
  ...
  replacehuan: function (e) {
    ...
    let cabinetid = e.currentTarget.dataset.id; // if (cabinetstatus==1){
    //   wx.showToast({
    //     title: '请先清空货物!',
    //     icon: 'none',
    //     duration: 1200
    ...
    wx2my.scanCode({
      success(res) {
        ...
        let fictitious = res.result.substring(res.result.length - 14, res.result.length - 12); // let nowsaomadata = JSON.parse(nowsaomadata)

        wx2my.request({
          url: apiUrl + '/cabinet/cabCode?cabCode'+nowsaomadata+'&cabId='+cabinetid,
          header: {
            'content-type': 'application/json',
            'Authorization': wx2my.getStorageSync("Token")
          },
