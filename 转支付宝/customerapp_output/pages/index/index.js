...
Page({
  ...
  get_hotelinfo: function (hotelid, cabCode) {
    //获取酒店信息
    const that = this;
    wxrequest.gethotelinfo(hotelid).then(res => {
      ...
      if (resdata.code == 0) {
        ...
        that.setData({
          ...
          //酒店荣誉
          hotelBookingPhone: resdatas.hotelBookingPhone,
          //酒店订房电话
          ...
          //是否支持酒店房费发票
          isShowInvoiceReminder: resdatas.isShowInvoiceReminder,
          //酒店设置是否显示滚动字幕
  ...
  getpordinfo: function (cabCode, hotelId) {
    //获取迷你吧商品信息 prodtype：1-迷你吧，2-便利店，3-其他功能区
    const that = this;
    let linkData = {
      cabCode: cabCode
    };
  ...
  getstorelist: function (hotelId, pageNo) {
    //获取便利店商品
    const that = this;
  ...
  plusfun2: function (e) {
    ...
    if (edata.ptype == 1) {
      index = mnbcommodity.findIndex(item => {
        //判断数组中是否存在当前数据,无：-1，有：返回下标
      ...
      index = storelist.findIndex(item => {
        //判断数组中是否存在当前数据,无：-1，有：返回下标
  ...
  suborder: function (delivery_list1, delivery_list3, customerId) {
    //提交订单
    const that = this;
    that.setData({
      subordertype: false
