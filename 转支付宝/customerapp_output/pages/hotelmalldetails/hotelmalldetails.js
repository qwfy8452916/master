...
Page({
  ...
  get_pordinfo: function (funcProdId) {
    //获取商品信息
    const that = this;
    let linkData = {
    ...
    wxrequest.getpordinfo(linkData).then(res => {
      ...
      if (resdata.code == 0) {
        ...
        const spotpurchase_flag = app.globalData.spotpurchaseflag;

        if (delivWay == 0) {
          isCanBuy = 0;
        } else if (delivWay == 1) {
          if ((invProdAmount > 0 || isNeedInv == 0) && spotpurchase_flag == 1) {
            isCanBuy = 1;
            delivWaynum = 1;
          } else {
            isCanBuy = 0;
          ...
          if (invProdAmount > 0 && spotpurchase_flag == 1) {
            delivWaynum = 1;
          } else {
            delivWaynum = 2;
          }
  ...
  getpjdatafun: function (prodcode) {
    //获取评价信息
    const that = this;
    let linkData = {
      hotelId: app.globalData.hotelId,
  ...
  addcarfun: function (e) {
    //加入购物车
    const that = this;
    const shareuser = that.data.shareuser;
    let edata = e.currentTarget.dataset;
    let money = that.data.money;
    const spotpurchase_flag = that.data.spotpurchaseflag;
    ...
    if (delivWaynum == 1) {
      ...
      if (spotpurchase_flag == 1) {
        if (index == -1) {
          ...
          if (buy_list[index].prodnum < details_data.invProdAmount || details_data.isNeedInv == 0) {
            //检验库存
            wx2my.showToast({
              title: '已成功加入购物车',
              icon: 'none',
        ...
        wx2my.showToast({
          title: '该商品暂不支持购买，请选择其它商品',
          icon: 'none',
          duration: 2000
        });
        return;
      }
    } else if (delivWaynum == 2) {
      //快递配送
  ...
  onShareAppMessage: function (options) {
    ...
    return {
      ...
      // 路径，传递参数到指定页面。
      imageUrl: that.data.detailsdata.bannerImageList[0],
      // 分享的封面图
      success: function (res) {
        // 转发成功
        console.log('用户取消转发');
