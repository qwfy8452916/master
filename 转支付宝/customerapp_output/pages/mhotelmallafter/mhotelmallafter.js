...
Page({
  data: {
    ...
    //商品已申请售后数量
    delivstatus: '',
    //配送单状态0=待确认 1=已确认 2=已配送 3=部分退款 4=全部退款 5=已收货
    aprodamountAll: '',
    //申请售后总价
    aprodamount: '',
    //退款金额（用于修改的）
  ...
  onLoad: function (options) {
    ...
    if (options.prodstatus == 5) {
      //0：正常，1:确认前退款，2：退款 ，3：换货，4： 退货退款 , 5:售后待处理
      ...
      that.setData({
        name: namedata,
        //商品显示名称
        prodnum: prodinfo.prodCount,
        //商品数量
        prodjg: prodinfo.prodPrice,
        //商品单价
        imgurl: imgdata,
        ...
        proddeliv: prodinfo.prodCsTimes,
        //商品已申请数量
        orderDeviCode: options.delivcode,
        //配送单号
        orderDeviDetailId: prodinfo.id,
        //配送单详情Id
        orderDeviId: prodinfo.orderDeliveryId,
        //配送单id
  ...
  getTkmoney: function (id, prodCount) {
    //获取退款金额
    const that = this;
    wx2my.showLoading({
      title: '加载中'
  ...
  shipmentinfofun: function (e) {
    //物流信息
    this.setData({
      shipmentinfo: e.detail.value
    });
  },
  shipmentfun: function (e) {
    //物流单号
    this.setData({
  ...
  aftersub: function () {
    ...
    let linkData = {
      ...
      cabLatticeCode: '',
      //格子编号
      cabRoomCode: thatdata.roomCode,
      //柜子房间号
      csDescription: thatdata.rebateState,
      //售后申请描述
      csType: thatdata.csKind,
      //售后类型（1：换货，2：退货退款，3：迷你吧，4：退款）
      cusLogisticsCode: thatdata.shipmentnum,
      //客户物流单号
      cusLogisticsInfo: thatdata.shipmentinfo,
      //客户物流信息
      customerId: thatdata.customerId,
      //用户id
      funcId: thatdata.funcid,
      //功能区Id
      hotelId: thatdata.hotelId,
      //酒店id
      prodCode: thatdata.prodinfo.prodCode,
      //商品code
      hotelProdId: thatdata.prodinfo.hotelProdId,
      //酒店商品Id
      prodOwnerOrgId: thatdata.prodinfo.prodOwnerOrgId,
      //商品所属组织id
      prodOwnerOrgKind: thatdata.prodinfo.prodOwnerOrgKind,
      //商品平台类型
      certificateImages: thatdata.productInfo,
      //凭证图
      prodCount: thatdata.afternum,
      //售后数量
      refoundAmount: aprodamount_All,
      //退款金额
      orderDeviCode: thatdata.orderDeviCode,
      //配送单号
      orderDeviDetailId: thatdata.orderDeviDetailId,
      //配送单详情Id
      orderDeviId: thatdata.orderDeviId,
      //配送单id
      id: thatdata.id
    };
  ...
  getafterdata: function (csid) {
    ...
    wxrequest.getafterdetail(csid).then(res => {
      ...
      if (resdata.code == 0) {
        ...
        that.setData({
          name: resdatas.productName,
          //商品显示名称
          prodnum: resdatas.prodCount,
          //商品数量
          prodjg: resdatas.prodAmount,
          //商品单价
          imgurl: resdatas.productImage,
          //商品图片
          csKind: resdatas.csType,
          //售后类型
          csStatus: resdatas.status,
          //售后处理状态
          canceltype: canceltype,
          //是否显示撤销按钮
          csRequestDTOs: resdatas.certificateImages,
          //凭证图
          csRequestDTOstype: csRequestDTOstype,
          //是否显示凭证
          prodinfo: resdatas,
          //商品信息
          ...
          orderDeviCode: resdatas.orderDeviCode,
          //配送单号
          orderDeviDetailId: resdatas.orderDeviDetailId,
          //配送单详情Id
          orderDeviId: resdatas.orderDeviId,
          //配送单id
