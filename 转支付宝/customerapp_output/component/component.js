...
var xianxuNZ = [];
var xuanZheData = [];
var windowHeight; // var util = require('./longli.js');
...
Component({
  ...
  methods: {
    ...
    yunXian: function () {
      ...
      let timedata = parseInt(this.data.timedata);
      let type;
      let xianDay_ = JSON.stringify(day);
      xianDay_ = JSON.parse(xianDay_); // console.log(timedata, hours);
      ...
      if (cunIndex != -1) {
        ...
        if (xuanZheData.length < 2) {
          if (xuanZheData.length >= 1) {
            ...
            twoZHI = twoZHI.replace(new RegExp("-", "gm"), "/"); //console.log('firstZHI', firstZHI)
