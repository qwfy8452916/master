<template>
	<view class="list-wrapper">
					...
					<view class="item-content">
						...
						<view class="operation-wrapper">
							...
							<view class="approve-view">
								<view
									class="btn"
									v-if="item.status === 'WAIT_SIGN' && slaveOrderListAuthObj.receiveGoodsBtn.show"
									@click="receiveGoods(orderId, item.slaveOrderNo, item.id)">确认收货</view>
								<view
									class="btn"
									v-if="item.status === 'WAIT_BRANCH_CONFIRM_PRICE_AND_QUANTITY' && slaveOrderListAuthObj.confrimBtn.show"
									@click="confirm(orderId, item.slaveOrderNo, item.id)">确认量价</view>
							</view>
						</view>
					</view>	
				</view>
		...
		<view class="button-wrapper" v-if="orderStatus === 'PENDING'">
			<view class="btn-add" @click="addSlaveOrder(orderId)">创建批次订单</view>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			addSlaveOrder(orderId) { //创建批次订单
				uni.navigateTo({
					url: `/pages/branch/order/order-release/order-release?orderId=${orderId}`
				})
			},
			receiveGoods(orderId, slaveOrderNo, slaveOrderId) { //确认收货
				uni.navigateTo({
					url: `/pages/branch/order/slave-order/detail/detail?orderId=${orderId}&slaveOrderNo=${slaveOrderNo}&slaveOrderId=${slaveOrderId}`
				})
			},
