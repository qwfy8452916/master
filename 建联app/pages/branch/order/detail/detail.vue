						...
						<view class="content">
							<view class="quote-view">
								...
								<view class="content">
									...
									<view class="payment-list" v-for="(item, index) in paymentList" :key="index">
	...
	export default {
		...
		methods: {
			viewSlaveOrderList() { //查看批次列表
				uni.navigateTo({
					url: `/pages/branch/order/slave-order/list/list?orderId=${this.orderId}`
				})
			},
			getDetail() { //获取联采订单详情
				const params = {
				...
				this.$api.branchOrderApi.orderDetail(params, this.orderId)
					.then(res => {
						...
						if(result.msg_code === 100000) {
							...
							const paymentList = JSON.parse(demandPurchaseInfo.branch_approve_price);
							
							this.programCode = demandInfo.project_code;
							this.purchaser = result.response.branch_name;
							...
							this.paymentList = paymentList.map((item, index) => { //支付方式列表
								return {
			...
			closeOrder() { //申请关闭订单
				uni.navigateTo({
					url: `/pages/branch/order/close/close?orderId=${this.orderId}`
				})
			}
		}
	}
