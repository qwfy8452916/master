			...
			<view class="content">
				...
				<view class="wrapper">
					<view class="protocol-wrapper">
						<view class="protocol-input-wrapper">
							<switch  id="checkProtocolBox" @change="checkProtocolBox" type="checkbox"/>
							<label for="checkProtocolBox" class="checkProtocolBoxLabel">
								<text> 我已阅读并同意</text>
								<text class="protocol-link" @tap="toProtocol">《筑牛网注册服务协议》</text>
							</label>
						</view>
						<view class="error-tip" v-if="isShowCheckProtocolErrorTip">
							<view class="prepend">
								<text class="iconfont">&#xe61b;</text>
							</view>
							<view class="error-view">
								<text class="error" v-text="checkProtocolErrorTip"></text>
							</view>
						</view>
					</view>
				</view>
	...
	export default {
		data() {
			return {
				...
				/* protocol */
				"checkProtocol": false,
				"isShowCheckProtocolErrorTip": false,
				"checkProtocolErrorTip": "未勾选筑牛网用户注册服务协议"
		...
		methods: {
			...
			getCode() {
				...
				validater.phoneNumber(_this.mobile, (result, errorTip = '') => {
					if (!result) {
						...
						_this.$api.registerApi.sendCode(_this.mobile).then(function(result){
			...
			checkProtocolBox(e) {
				let _this = this;
				_this.checkProtocol = e.detail.value;
				if (_this.checkProtocol) {
					_this.checkProtocolErrorTip = "";
					_this.isShowCheckProtocolErrorTip = false;
				}
			},
			goNext() {
				let _this = this;
				...
				/* check protocol */
				if (!_this.checkProtocol) {
					_this.checkProtocolErrorTip = "未勾选筑牛网注册服务协议";
					_this.isShowCheckProtocolErrorTip = true;
				}
				
				if (_this.isShowMobileErrorTip || _this.isShowCodeErrorTip || _this.isShowCheckProtocolErrorTip) {
					return false;
				}
				
				/* 记录手机号 */
				_this.$api.registerApi.checkCode(_this.mobile, _this.code).then(function(result) {
					if (result.data.msg_code == 100000) {
						uni.navigateTo({
		...
		.main {
			...
			.content {
				...
				.wrapper {
					...
					.protocol-wrapper {
						height: 120upx;
						.protocol-input-wrapper {
							.checkProtocolBoxLabel {
								font-size: 26upx;
								.protocol-link {
									color: $uni-color-primary;
								}
