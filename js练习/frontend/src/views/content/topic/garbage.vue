        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              size="small"
              @click="removeForeverHandle(scope.row)"
            >永久删除</el-button>
            <span class="line">|</span>
            <el-button
              type="text"
              size="small"
              @click="recoverTopichandle(scope.row)"
            >恢复</el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  import { fetchTopicList, fetchTopTopic, fetchSwitchTopic, fetchStatistic, fetchDrop, fetchId } from '@/api/topic'
  export default {
    name:'topic',
    data() {
      return {
    ...
    methods:{
      ...
      // 永久删除
      removeForeverHandle(obj) {
        this.$confirm('确认要永久删除？', '删除确认', {
      ...
      // 恢复
      recoverTopichandle(obj) {
        this.$confirm('确认要恢复吗？恢复后在全部中查看', '恢复提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        })
          .then(() => {
