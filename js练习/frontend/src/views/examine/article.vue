  ...
  <div class="container">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <router-link :to="{name:'CheckArticle',query:{articleId:scope.row.id}}">
              <el-button type="text" style="color: #333;" size="small">查看</el-button>
            </router-link>
            ...
            <el-button
              ...
              type="text"
              size="small"
              @click="auditPassHandle(scope.row)">审核通过</el-button>
            <span class="line">|</span>
            <el-button
              :class="{ 'red':scope.row.status==3 }"
              type="text"
              size="small"
              @click="auditNoPassHandle(scope.row)">审核不通过
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div>
      ...
      <el-dialog
        ...
        width="30%">
        <el-input
          v-model="resultofaudit"
          type="textarea"
          rows="5"/>
        <span slot="footer" class="dialog-footer">
          <el-button @click="dialogVisible=false">取 消</el-button>
          <el-button type="primary" @click="noPassHandle">确 定</el-button>
        </span>
      </el-dialog>
      <!--通过-->
      <el-dialog
        ...
        title="审核通过"
        width="30%">
        <span>确认审核通过此篇文章吗?</span>
        <span slot="footer" class="dialog-footer">
          <el-button @click="passDialog=false">取 消</el-button>
          <el-button type="primary" @click="passDialogHandle">确 定</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    passDialogHandle() {
      ...
      const current = this.tableData.filter((item) => {
