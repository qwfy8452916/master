<template>
    <div>
        ...
        <el-form :inline="true" :model="formInline" size='medium' class="demo-form-inline align-left">
            ...
            <el-form-item>
                <el-button type="primary" class="nomalBtn1" @click="Getdata()">查询</el-button>
            </el-form-item>
        </el-form>
...
export default {
    ...
    methods:{
        ...
        checkdetail:function(index,row){
           let id=row[index].id
           this.$router.push({path:"/Purorderdetail", query:{id}})
        },
        ...
        Getdata:function(){
            ...
            let params = {
                ...
                productId:this.formInline.productId[length],
                status:this.formInline.status,
            }
            that.$api.orderList({params}).then(response=>{
                let result = response.data;
                if(result.code===0){
