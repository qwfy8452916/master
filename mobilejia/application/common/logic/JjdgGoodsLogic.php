...
class JjdgGoodsLogic extends Model
{
    ...
    public function goodsFilter($map, $order = [], $p = 1, $p_size = 40, $user = null)
    {
        ...
        switch ($order['order_by']) {
            ...
            case GoodsSortConfig::VOLUME_ASC:
                ...
                break;
            default://综合排序
                return $this->getListByWhereMultipleOrder($where, $with, $p, $p_size, $find);
                break;
        }

        return $this->getListByWhere($where, $order_by, $with, $p, $p_size, $find);
    }
    ...
    public function getGoodsDetail($where = [], $with = [], $with_count = [])
    {
        ...
        $goods = JjdgGoods::where($condition)
            ->field('code,views,avg_score,describtion,volume,has_detail,expand_link,detail,expand_link,zk_final_price,has_custom_score,custom_score,title,buy_num,top_cate_id,sub_cate_id')
            ->with($with)
     ...
     * @param $remove_code
     */
    public function getGoodsRecommendBySubCateId($sub_cate_id, $remove_code, $limit)
    {
        $where = [
            'is_del' => 2,
            'on_sale' => 1,
    ...
    {
        return model('JjdgGoods')->with(['goodsImgs','goodsSubCate' => function ($query) {$query->field('id,short_name');}])->alias('g')
