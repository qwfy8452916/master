    ...
    if (!empty($data["code"])) {
        ...
        if (!empty($safecode)) {
            ...
            if ($tel != $safecode["tel"]) {
                ...
                return ["data" => "", "info" => '亲,帐号不符,请输入正确的手机/邮箱', "status" => 0];
            }
            if (strtolower($data["code"]) != authcode($safecode["code"])) {
                //清除验证session
                session('safecode',null);
                return ["data" => "", "info" => '亲,您输入的验证码不正确', "status" => 0];
            }
            //清除验证session
            session('safecode',null);
            return ["data" => "", "info" => '亲,验证码输对了！', "status" => 1];
        } else {
            return ["data" => "", "info" => '亲,验证码失效了！', "status" => 0];
        }
    } else {
        //清除验证session
        session('safecode',null);
        return ["data" => "", "info" => '亲,验证码不正确！', "status" => 0];
    }
}
