/**公共函数库 zwl**/
/*********************************/
/**  * 字符串切割  * @param str  * @param start 起始位置  * @param end 结束位置  * @returns str  */
...
/**  * ajax请求，所有的请求都通过这里发送  * @param options  */
...
/**  * 验证手机号  * @param str  * @returns {boolean}  */
...
/**  * 倒计时  * @param sec 倒计时实现，如60s  * @param ele 显示容器  * @param callback  */
...
/**  * 计算字符串长度，中文2个字符，英文一个字符  * @param str  * @returns {number}  */
...
/**  * 禁止某些字符输入输入  * @param ele 显示容器  * @param type 1：禁止空格输入 2：禁止非数字字符输入 3：禁止数字小数点以外的字符输入 4：禁止特殊字符输入  */
...
/**  * 校验url  * @param str_url  * @returns {boolean}  */
...
/**  * 批量向本地缓存中存储数据  * @param item 格式为[{name:"对象名",val:"存储的值"},{name:"对象名",val:"存储的值"}]  */
...
/**  * 删除指定缓存  * @param name  * @param storageName  */
...
/**  * 获取指定缓存  * @param [string] storageName  * @return [array|object]  */
...
/**  * 检测IE版本  */
function IEVersion() {
    var userAgent = navigator.userAgent; //取得浏览器的userAgent字符串  var isIE = userAgent.indexOf("compatible") > -1 && userAgent.indexOf("MSIE") > -1; //判断是否IE<11浏览器  var isEdge = userAgent.indexOf("Edge") > -1 && !isIE; //判断是否IE的Edge浏览器  var isIE11 = userAgent.indexOf('Trident') > -1 && userAgent.indexOf("rv:11.0") > -1; if(isIE) { var reIE = new RegExp("MSIE (\\d+\\.\\d+);");
    reIE.test(userAgent);
    ...
    if (fIEVersion == 7) {
        ...
        return 'edge'; //edge  } else if(isIE11) { return 11; //IE11  }else{ return -1;//不是ie浏览器  }
    }
    /**  * 检测属性是否可用，检测属性是否可用，比检测浏览器更靠谱  * @param ele 元素  * @param pn 属性  * @returns {boolean}  */
    function checkProperty(ele, pn) {
        ...
        if (test[pn] == "undefined") { // 若属性不存在返回的将是undefined，否则返回的是空字符串""  return false;
        }
        return true;
    }
    /**  * 监听paste事件  * @param ele 需要监听的元素  * @param callback  */
    ...
    /**  * 全选/取消全选  * @param ele string/jQuery Object  * @param coc bool  */
