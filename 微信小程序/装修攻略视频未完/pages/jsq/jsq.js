...
Page({
  ...
  onLoad: function (options) {
    // console.log(app.globalData.sourceMark)
    let that = this;
    let json = [];
    let prevJson = [];
    let cityJson = [];
  ...
  ljjsbjff: function (e) {
    ...
    wx.request({
      ...
      method: "POST",
      success: function (res) {
        alertViewWithCancel("提示", "发送成功!我们会尽快和您取得联系，请注意接听电话", function () {
        });
  ...
  okbtn: function () {
    ...
    this.setData({ isHide: true, colorCont: [true], selectText: selectText, prevCityAreaId: prevCityAreaId, areaId: areaId, serverVal: areaText, selectTextDefault: ''  });
  },
