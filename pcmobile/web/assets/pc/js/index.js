...
function picPointer(options) {
    ...
    if("forEach" in Array.prototype){
        ...
        for(var i=0,len=options.ele.length; i<len; i++){
            var xPerc = options.pointer[i].x / options.pointer[i].w;
            var yPerc = options.pointer[i].y / options.pointer[i].h;
            var $img = $(options.ele[i]).parent().find("img");
            $(options.ele[i]).css({
                "position": "absolute",
                "left": $img.width() * xPerc,
                "top": $img.height() * yPerc
...
+function ($) {
    if( $("#ibanner").find(".ieswiper-slide").length > 1 ){
        var mySwiper = new ieSwiper('#ibanner', {
            ...
            onlyExternal:true, // 调用mySwiper.swipePrev()，mySwiper.swipeNext()接口必须有这个
            autoplayDisableOnInteraction : false //无则会导致手动滑动轮播图后无法再自动滑动
            // observer:true,//修改swiper自己或子元素时，自动初始化swiper
            // observeParents:true,//修改swiper的父元素时，自动初始化swiper
        });
        // 上一张
        $('.ibanner-prev').click(function (event) {
            mySwiper.swipePrev();
        });
        // 下一张
        $('.ibanner-next').click(function (event) {
            mySwiper.swipeNext();
        });
    }
    if( $("#ibanner").find(".ieswiper-slide").length <= 1 ){
