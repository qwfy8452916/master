...
// 在IE8,9下手动模拟placeholder效果
$(function () {
    if( isIE89 ){
        var $searchInput = $(".search-input"), placeholderV = $searchInput.attr("placeholder");
        $searchInput.val(placeholderV);
    }
});
// 点击头部搜索框，弹出相关搜索下拉框
$(function(){
...
$(function(){
	var $siginBtn = $("span[data-type='signin-btn']"),
		...
		$siginBtn.on("click",function(){
			...
			if( !isIE89 ){ // 不是IE8，9才清除
                $siginContainer.find("input").val("");
            }else{
                $siginContainer.find("input[data-type='phone']").val("手机号");
                $siginContainer.find("input[data-type='sign-password']").val("密码");
            }
            $siginContainer.find(".user-tips").text("").css("opacity",0);
            $siginContainer.find(".user-btn").attr("disabled",true);
		...
		$registerBtn.on("click",function(){
			...
			$registerContainer.find("button[data-type='token']").attr("disabled",true);
			if( !isIE89 ){
                $registerContainer.find("input").val("");
            }else{
                $registerContainer.find("input[data-type='phone']").val("手机号");
                $registerContainer.find("input[data-type='token']").val("验证码");
            }
            ...
            $registerContainer.find(".user-btn").attr("disabled",true);
...
$(function(){
	...
	$(document).on("keyup",function(event){
		...
		// 判断是否要更改注册密码框下一步按钮状态
		if( $.trim($registerContainer.find("input[data-type='phone']").val()).length > 0 && $registerContainer.find("input[data-type='token']").val() != "验证码" && $registerContainer.find("input[data-type='token']").val().length > 0 ){
	...
	function registerAction(ele){
		...
		if( isIE89 ){
            if( nickname == "昵称" && password == "密码" && repassword == "确认密码" ){
                return;
            }
        }
		if( !phoneV || !token ){
	...
	function forgetAction(ele){
		...
		if( isIE89 ){
            if( password == "密码" && repassword == "确认密码" ){
                return;
            }
        }
		if( !phoneV || !token ){
		...
		submitPhoneAndToken(phoneV, tokenV, checkUser, function(res){
			...
			if( purpose == "forget-password" ){
                ...
                if( !isIE89 ){
                    ...
                    $forgetPasswordContainer.find("input[data-type='password']").val("密码");
                    $forgetPasswordContainer.find("input[data-type='confirm-password']").val("确认密码");
                }
                $forgetPasswordContainer.find(".user-tips").text("").css("opacity",0);
                $forgetPasswordContainer.find(".user-btn").attr("disabled",true);
                ...
                if( !isIE89 ){
                    ...
                    $setPasswordContainer.find("input[data-type='nickname']").val("昵称");
                    $setPasswordContainer.find("input[data-type='password']").val("密码");
                    $setPasswordContainer.find("input[data-type='confirm-password']").val("确认密码");
                }
                $setPasswordContainer.find(".user-tips").text("").css("opacity",0);
                $setPasswordContainer.find(".user-btn").attr("disabled",true);
				$setPasswordContainer.fadeIn("fast",function(){
					checkNickName();
					checkPassword();
