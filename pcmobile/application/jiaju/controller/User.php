...
class User extends JiajuBase
{
    ...
    public function _initialize()
    {
        ...
        if (empty($this->userinfo)) {
            $member = model('User')->find(session('u_userInfo.id'));
            if (empty($member)){
                //清除用户session
    ...
    public function store(JjdgGoodsLogic $JjdgGoodsLogic,ScoreLogic $scoreLogic,CollectLogic $collectLogic)
    {
        ...
        $page = 1;
        if (input('p','')!==''){
            preg_match('/(?<=p)\d+/', input('p',''), $mm);
            $page =  $mm[0];
        }
        //关联商品查询所有收藏
        $list = $JjdgGoodsLogic->getCollectGoods($where,$page,10);
        $count = $JjdgGoodsLogic->getCollectGoodsCount($where);
        $page = new Page($count, 10);
        $this->assign('page', $page->show());
        $this->assign("list", $list);
        //推荐商品
        $recomGoods = cache('Cache:RecomGoods:limit-20');
        if (empty($recomGoods)){
            $recomGoods = $JjdgGoodsLogic->selectRecommendData(20);
            ...
            //推荐商品
            cache('Cache:RecomGoods:limit-20',$recomGoods,['expire'=>900],'jjdg_goods'); //设置5分钟过期
        }
    ...
    public function collect()
    {
        ...
        if ($collect == null) {
            ...
            JjdgGoods::where('code',$data['code'])->setInc('custom_collect');
            $collect->save();
            return json(['status' => ApiConfig::COLLECT_IN, 'info' => '收藏成功']);
        }
        $collect->delete();
        JjdgGoods::where('code',$data['code'])->setDec('custom_collect');
