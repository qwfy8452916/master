...
class CollectLogic extends Model
{
    ...
    public function getCollectNumByObj($goods_obj)
    {
        if ($goods_obj->has_custom_collect == 1) {
            if (array_key_exists('collect_num', $goods_obj->data)) {
                return (int)$goods_obj->getAttr('collect_num');
            }
            if (array_key_exists('collection_count', $goods_obj->data)) {
                return (int)$goods_obj->getAttr('collection_count');
            }
        }
