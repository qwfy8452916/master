<template>
    <div class="resourcemanage">
        ...
        <div v-if="authzData['F:BH_BOOK_RESOURCE_ADD']"><el-button class="addbutton" @click="bookResourceAdd">新增房源</el-button></div>
        <el-table :data="ResourceDataList" border stripe style="width:100%;" >
            ...
            <el-table-column prop="livePeople" label="入住人数" min-width="80px" align=center></el-table-column>
            <el-table-column prop="isCancellable" label="是否可取消" min-width="120px">
                <template slot-scope="scope">
                    <span v-if="scope.row.isCancellable == 0">不可取消</span>
                    <span v-if="scope.row.isCancellable == 1">{{scope.row.cancelDeadline}}{{scope.row.cancelDeadline == null?'':'前'}}可取消</span>
                </template>
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" min-width="100px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BH_BOOK_RESOURCE_EDIT']" type="text" size="small" @click="bookResourceModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BH_BOOK_RESOURCE_DELETE']" type="text" size="small" @click="bookResourceDelete(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        bookResourceList(){
            ...
            // console.log(params);
            this.$api.bookResourceList(params)
                .then(response => {
        ...
        EnsureDetail(){
            const params = {};
            const id = this.brId;
             this.$api.bookResourceDelete(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
