    ...
    <div class="featuredetailadd">
        ...
        <el-form :model="FeatureDetailData" :rules="rules" ref="FeatureDetailData" label-width="120px" class="hotelfeatureform">
            ...
            <el-form-item label="描述" prop="detailDesc">
                <el-input v-model="FeatureDetailData.detailDesc" maxlength="255" type="textarea" :rows="5"></el-input>
            </el-form-item>
            <!-- <el-form-item label="文字注释"> -->
                <div class="annotationadd">
                    <span class="annspan">标签</span>
                    <div class="addbtn"><el-button v-if="authzlist['F:BH_HOTEL_FEATURETYPE_DETAILALTER_ADDLABEL']" type="primary" size="small" @click="annotationAddLine">添加</el-button></div>
                    <el-table
                        ...
                        <el-table-column width="30px">
                            <template slot-scope="scope">
                                <el-button v-if="authzlist['F:BH_HOTEL_FEATURETYPE_DETAILALTER__DELETELABEL']" type="text" size="small" @click="annotationDeleteLine(scope.$index)">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                </div>
            ...
            <el-form-item>
                ...
                <el-button v-if="authzlist['F:BH_HOTEL_FEATURETYPE_DETAILALTERSUBMIT']" type="primary" :disabled="isSubmit" @click="submitForm('FeatureDetailData')">确定</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
            ...
            this.$api.getHotelFeatureDetail(params)
                .then(response => {
                    ...
                    if(result.code == '0'){
                        ...
                        this.$message.error('酒店特色明细列表获取失败！');
                    }
                })
                .catch(error => {
                    this.$alert(error,"警告",{
                ...
                if(valid){
                    ...
                    this.$api.HotelFeatureDetailModify(params, id)
                        .then(response => {
                            ...
                            if(result.code == 0){
                                if(result.data){
                                    this.$message.success('修改酒店特色成功！');
                                    const id = this.hfId;
                                    this.$router.push({name: 'HotelFeatureTypeDetail', query: {id}});
                                }else{
                                    this.$message.error('修改酒店特色失败！');
                                    this.isSubmit = false;
                                }
                            }else{
                                this.$message.error('修改酒店特色失败！');
