<template>
    <div class="replenishlist">
        ...
        <div><el-button v-if="authzlist['M:BH_REPL_REPLENISHLISTEXPORT']" class="addbutton" @click="outExe">导&nbsp;&nbsp;出</el-button></div>
        <div class="prodtotal">
            <span>总计：</span>
            <ul>
                <li v-for="item in HotelReplenishTotal" :key="item.prodId">&nbsp;&nbsp;&nbsp;{{item.prodName}}*{{item.allSum}}</li>
            </ul>
        </div>
        <el-table
...
export default {
    ...
    methods: {
        ...
        replenishTotal(){
            ...
            // console.log(params);
            // return
            this.$api.replenishTotal(params)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        outExe(){
            ...
            window.location.href ="http://hotel.kefangbao.com.cn/longan/api/repl/export/download?opFlag="+this.inquireType+"&orgAs=3&hotelId="+this.hotelId+"&prodName="+this.inquireCommodityName+"&roomFloor="+this.inquireFloor +"&token="+ this.token;
