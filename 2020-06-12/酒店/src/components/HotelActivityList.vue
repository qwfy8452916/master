...
export default {
    ...
    methods: {
        ...
        addNewSetting(){
            this.$router.push({name:'HotelActivityAdd'});
        },
        //修改
        Cabinetglchange(index,row){
            let guiId=row[index].id
            this.$router.push({name:'HotelActivityChange',query:{modifyid: guiId}});
        },
        //获取活动列表
        getActList(){
        ...
        viewDetail(index,row){
            let guiId=row[index].id
            this.$router.push({name:'HotelActivityDetail',query:{modifyid: guiId}});
        },
        //活动明细
        viewActivityDef(index,row){
            ...
            if(actType == 1){
                this.$router.push({name:'HotelActivityDef',query:{modifyid: guiId}});
            }else if(actType == 2){
                this.$router.push({name:'HotelActivityDef',query:{modifyid: guiId}});
            }else if(actType == 3){
                this.$router.push({name:'HotelActShareDef',query:{modifyid: guiId}});
            }else if(actType == 4){
                this.$router.push({name:'HotelActRedpackDef',query:{modifyid: guiId,hotelId}});
            }
        },
        //活动参与记录
        viewActivityRecords(index,row){
            let guiId=row[index].id
            this.$router.push({name:'HotelActivityRecord',query:{modifyid: guiId}});
        },
        //修改状态
