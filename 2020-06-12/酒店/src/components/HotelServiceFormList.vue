<template>
    <div class="servicetypelist">
        <div v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_FORM_INT_EDIT']" class="modifybtn"><el-button type="primary" @click="formIntroduceModify">修改动态表单介绍信息</el-button></div>
        ...
        <div v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_FORM_ADD']"><el-button class="addbutton" @click="serviceFormAdd">新增表单明细</el-button></div>
        <el-table :data="ServiceFormDataList" border style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" min-width="100px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_FORM_EDIT']" type="text" size="small" @click="serviceFormModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_PART_FORM_DELETE']" type="text" size="small" @click="serviceFormDelete(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        serviceFormList(){
            ...
            // console.log(params);
            const hsId = this.hsId;
            this.$api.serviceFormList(params, hsId)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        formIntroduceModify(){
            const hsId = this.hsId;
            this.$router.push({name:'HotelServiceFormIntroduce', query: {hsId}});
        },
        //新增
        serviceFormAdd(){
            const hsId = this.hsId;
            this.$router.push({name:'HotelServiceFormAdd', query: {hsId}});
        },
        //修改
        serviceFormModify(id){
            const hsId = this.hsId;
            this.$router.push({name:'HotelServiceFormModify', query: {hsId, id}});
        },
        //删除
        ...
        EnsureDetail(){
            ...
            const params = {};
            this.$api.serviceFormDelete(params, hsId, sfId)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
