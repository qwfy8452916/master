    ...
    <div class="functionlist">
        ...
        <el-table :data="hotelFunctionDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" width="260px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BH_FUNC_FUNCTION_DETAIL']" type="text" size="small" @click="hotelFunctionDetail(scope.row.id)">详情</el-button>
                    <el-button v-if="authzData['F:BH_FUNC_FUNCTION_EDIT']" type="text" size="small" @click="hotelFunctionModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BH_FUNC_FUNCTION_DELETE']" type="text" size="small" @click="hotelFunctionDelete(scope.row.id)">删除</el-button>
                    <el-button v-if="authzData['F:BH_FUNC_FUNCTION_CLASSIFY']" type="text" size="small" @click="hotelFunctionClassify(scope.row.hotelId, scope.row.id, scope.row.funcCnName)">分类管理</el-button>
                    <el-button v-if="authzData['F:BH_FUNC_FUNCTION_PROD']" type="text" size="small" @click="hotelFunctionProd(scope.row.id)">商品管理</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        hotelFunctionClassify(hotelId, funcId, funcName){
            this.$router.push({name:'HotelFunctionClassify', query: {hotelId, funcId, funcName}});
        },
        //商品管理
