<template>
    <div class="ExpressTemplate">
        <div v-if="authzData['F:BH_PROD_EXPRESSTPL_ADD']" style="display:flex;"><el-button class="addbutton" @click="addNewExpress">新增运费模板</el-button></div>
        <div class="module"
        ...
        :key="index">
            <div class="header">
                ...
                <div class="operate">
                    <span v-if="authzData['F:BH_PROD_EXPRESSTPL_EDIT']" @click="changeOne(item.id)">修改</span>
                    <span v-if="authzData['F:BH_PROD_EXPRESSTPL_DELETE']" @click="cancelOne(item.id)">删除</span>
                </div>
            </div>
...
export default {
    ...
    methods: {
        addNewExpress(){
            this.$router.push({name:'HotelExpressAdd'});
        },
        cancelOne(itemId){
            console.log(itemId);
            let guiId=itemId;
        ...
        changeOne(itemId){
            this.$router.push({name:'HotelExpressChange',query:{modifyid:itemId}})
        },
        getData(){
