<template>
    <div class="prodspecs">
        <p class="title">功能区商品规格详情</p>
        <el-form v-model="ProdSpecsData" :model="ProdSpecsData" :rules="rules" ref="ProdSpecsData" label-width="100px" class="specsform">
            <el-form-item label="规格" prop="hotelProdSpecId">
                <el-select :disabled="true" v-model="ProdSpecsData.hotelProdSpecId" placeholder="请选择" @change="selectSpecs">
                    <el-option 
...
export default {
    ...
    mounted(){
        ...
        this.funcProdId  = this.$route.query.funcProdId ;
        this.fpsId  = this.$route.query.id ;
        this.funcProdSpecsDetail();
    },
    methods: {
        //获取酒店商品规格详情
        funcProdSpecsDetail(){
            const params = {};
            this.$api.funcProdSpecsDetail(params, this.fpsId)
                .then(response => {
        ...
        //未被使用的规格列表
        unusedFuncProdSpecsList(){
           const params = {
                funcProdId : this.funcProdId 
            };
            // console.log(params);
            this.$api.unusedFuncProdSpecsList(params)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
                        ...
                        const specsAdd = {
                            id: this.ProdSpecsData.hotelProdSpecId,
        ...
        selectSpecs(value){
            ...
            this.ProdSpecsData.showName = specsInfo.showName;
            ...
            this.ProdSpecsData.availableSaleQty = specsInfo.availableSaleQty;
        ...
        submitForm(ProdSpecsData){
            ...
            const params = {
                funcProdId: this.funcProdId,
                hotelProdSpecId: this.ProdSpecsData.hotelProdSpecId,
            ...
            this.$refs[ProdSpecsData].validate((valid) => {
                if (valid) {
                    ...
                    this.isSubmit = true;
                    this.$api.funcProdSpecsModify(params, this.fpsId)
                        .then(response => {
                            // console.log(response);
                            const result = response.data;
                            if(result.code == '0'){
                                this.$message.success('功能区商品规格修改成功！');
                                const id = this.funcProdId;
                                this.$router.push({name: 'HotelFunctionSpecsList', query: {id}});
                            }else{
                                this.$message.error(result.msg);
                                this.isSubmit = false;
                            }
