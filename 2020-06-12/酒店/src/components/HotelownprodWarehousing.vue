<template>
    <div class="godownentrylist">
        ...
        <el-table :data="HotelGodownEntryDataList" border stripe style="width:100%;" >
            <el-table-column fixed prop="invInCode" label="入库单编号" width="80px" align=center></el-table-column>
            ...
            <el-table-column fixed="right" label="操作" align=center>
                <template slot-scope="scope">
                   <el-button v-if="scope.row.reviewStatus== 2 && authzlist['F:BH_INV_OWNPRODWAREHOUSING_SCHEDULE']" type="text" size="small" @click="examineDetail(scope.row.wfId)">审核进度</el-button>
                    <el-button v-else-if="authzlist['F:BH_INV_OWNPRODWAREHOUSING_DETAIL']" type="text" size="small" @click="lookDetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        //查看商品明细
        lookDetail(id){
            this.$router.push({name: 'HotelownprodWarehousingcheck', query: {id}});
        },
