   ...
   <div class="CardCouponList">
       ...
       <el-table :data="cardData" border stripe>
          ...
          <el-table-column fixed="right" prop="id" label="操作" align="center" width="240px">
              <template slot-scope="scope">
                 <el-button v-if="authzData['F:BH_VOU_CARDCOUPON_DETAIL']" type="text" @click="detailCard(scope.row.id)">详情</el-button>
                 <el-button v-if="authzData['F:BH_VOU_CARDCOUPON_DELAY']" type="text" @click="yancDate(scope.row.id)">延长有效期</el-button>
                 <el-button v-if="scope.row.vouUseScene==2 && scope.row.isUsed==1" type="text" @click="checkOrder(scope.row.id)">查看订单</el-button>
                 <el-button v-if="authzData['F:BH_VOU_CARDCOUPON_CHECKDETAIL']" type="text" @click="checkcard(scope.row.id)">查看使用明细</el-button>
              </template>
          </el-table-column>
       </el-table>
       ...
       <el-dialog title="延长有效期" :visible.sync="datchdialog" width="30%">
            ...
            <span slot="footer">
                ...
                <el-button v-if="authzData['F:BH_VOU_CARDCOUPON_DELAY_SUBMIT']" type="primary" @click="sureExtendTimeBtn('diayancData')">确 定</el-button>
            </span>
        </el-dialog>
   </div>
</template>
       ...
       getUseCardticketRecord(vouId){
        ...
        this.$api.getUseCardticketRecord({params}).then(response=>{
           ...
           if(result.code==0){
               ...
               that.$router.push({name:"HotelOrderCouponDetails",query:{id}})
           }else{
             that.$message.error(result.msg)
           }
        }).catch(error=>{
