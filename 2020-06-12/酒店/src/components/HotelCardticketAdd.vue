   ...
   <div class="CardticketAdd">
      <el-form align="left" :model="carddata" label-width="140px" ref="carddata" :rules="rules">
          ...
          <el-form-item label="卡券名称" prop="vouName">
               <el-input v-model="carddata.vouName" maxlength="20"></el-input>
          </el-form-item>
          <el-form-item label="基础价格" prop="vouBasicPrice">
               <el-input v-model="carddata.vouBasicPrice" maxlength="16" oninput ="value=value.replace(/[^0-9.]/g,'')"></el-input>元
          </el-form-item>
          <el-form-item label="允许转赠" prop="canGive">
               <el-switch v-model="carddata.canGive" :active-value="1" :inactive-value="0"></el-switch>
          </el-form-item>
          ...
          <el-form-item label="卡券图片" prop="vouImagePath" class="cardpicbox">
            <el-upload
                    :action="uploadUrl"
                    list-type="picture"
                    :limit="1"
                    :file-list="carddata.vouImagePath"
                    :headers="headers"
                    name="fileContent"
          ...
          <el-form-item label="使用有效期" prop="vouTermType">
               <div>
                  <el-radio @change="changecouponTermType" v-model="carddata.vouTermType" label="0">领取后天数</el-radio>
                    <el-form-item class="vouTermType" v-if="carddata.vouTermType==='0'" prop="vouTermDays">
                         <el-input v-model.number="carddata.vouTermDays"></el-input>天
                    </el-form-item>
               </div>
            ...
            <el-form-item label="核销次数" prop="vouVerifiedTotalType" v-if="carddata.vouUseScene==1">
               <el-radio v-model="carddata.vouVerifiedTotalType" :label="1" @change="vouVerifiedTotalType">一次</el-radio>
               <el-radio v-model="carddata.vouVerifiedTotalType" :label="2" @change="vouVerifiedTotalType">多次</el-radio>
            ...
            <el-form-item label="抵扣设置" prop="vouDeductibleType" v-if="carddata.vouUseScene==2">
               ...
               金额:<el-input v-model="carddata.vouDeductibleMoney" oninput ="value=value.replace(/[^0-9.]/g,'')" class="vouDeductibleType"></el-input>元</el-form-item>
               <div>
                  <el-radio v-model="carddata.vouDeductibleType" :label="1" @change="setdikou">商品</el-radio>
                  <span v-if="carddata.vouDeductibleType==1">
                    <!-- <span style="margin-right:5px">{{carddata.vouDeductibleHotelProd.prodName}}</span> -->

                    <el-form-item prop="vouDeductibleHotelProdId" style="display:inline-block;">
                      <el-select v-model="carddata.vouDeductibleHotelProd" class="vouDeductibleHotelProd" @change="vouDeductibleHotelProd" value-key="id">
                          <el-option :label="item.prodName" :value="item" v-for="item in prodData" :key="item.id"></el-option>
                      </el-select>
                    </el-form-item>

                    <el-form-item v-if="carddata.vouDeductibleHotelProd.isSupportSpec>0" prop="vouDeductibleProdSpecId" style="display:inline-block;">
                      <el-select v-model="carddata.vouDeductibleProdSpecId" class="specguige" @change="specchange" value-key="id">
                          <el-option :label="item.specName" :value="item.id" v-for="item in specDara" :key="item.id"></el-option>
                      </el-select>
                    </el-form-item>
            ...
            <el-form-item>
                ...
                <el-button v-if="authzData['F:BH_VOU_CARDTICKET_ADD_SUBMIT']" type="primary" @click="sureBtn('carddata')">确定</el-button>
            </el-form-item>
      </el-form>
   </div>
</template>
...
export default {
  ...
  data(){
      ...
      return {
       ...
       carddata:{
         ...
         vouUseScene:'',
         vouVerifiedAddress:'',
         vouVerifiedTotalType:1,
         vouVerifiedTotal:'',
         vouDeductibleType:'',
         vouDeductibleMoney:0,
         vouDeductibleProdSpecId:'', //规格id
         vouDeductibleHotelProdId:'', //卡券抵扣商品id
      ...
      rules:{
        ...
        vouImagePath:{required:true,message:"请上传卡券图片",trigger:["blur","change"]},
        vouTermType:{required:true,message:"请选择使用有效期",trigger:"change"},
        vouTermDays:[
          {required:true,message:"请填写使用的天数",trigger:"blur"},
          {required:true,type:'number',min:1,message:"请填写正确的天数",trigger:"blur"},
          ],
        cardTermDate:{required:true,message:"请选择固定的日期",trigger:"change"},
        vouUseScene:{required:true,message:"请选择使用场景",trigger:"change"},
        vouVerifiedAddress:{required:true,message:"请填写核销地点",trigger:"blur"},
        vouVerifiedTotalType:{required:true,message:"请选择核销次数",trigger:"change"},
        vouVerifiedTotal:[
          {required:true,message:"请填写核销次数",trigger:"blur"},
          {min:2,type:"number",max:99999,message:"核销次数最小1最大99999",trigger:"blur"}
        ],
        vouDeductibleType:{required:true,message:"请选择抵扣设置",trigger:["blur","change"]},
        vouDeductibleMoney:[
            {required:true,message:"请填写现金金额",trigger:"blur",},
          ],
        vouDeductibleHotelProdId:{required:true,validator: validateProd,message:"请选择商品",trigger:["blur","change"]},
        vouDeductibleProdSpecId:{required:true,message:"请选择商品规格",trigger:"change"},
  ...
  methods:{
        ...
        vouDeductibleHotelProd(e){
          let that=this;
          that.carddata.vouDeductibleProdSpecId=""
          that.specDara=[];
        ...
        getCardticketProdspec(){
          ...
          this.$api.getCardticketProdspec({params}).then(response=>{
        ...
        useScene(e){
          if(e==1){
            this.carddata.vouVerifiedTotalType=1;
        ...
        sureBtn(carddata){
           ...
           this.$refs[carddata].validate((valid,model)=>{
            ...
            let params={
                ...
                vouDeductibleMoney:parseFloat(that.carddata.vouDeductibleMoney).toFixed(2),
                vouDeductibleHotelProdId:that.carddata.vouDeductibleHotelProdId,
                vouDeductibleHotelProdSpecId:that.carddata.vouDeductibleProdSpecId,
                vouOwnerOrgKind:3,
            };
                that.$api.CardticketAdd(params).then(response=>{
                   let result=response.data;
                   if(result.code==0){
        ...
        getCardticketProd(){
          ...
          this.$api.getCardticketProd({params}).then(response=>{
             let result=response.data;
             if(result.code==0){
        ...
        //选择使用有效期
            ...
            hotelDetail(){
              let that=this;
              this.$api.hotelDetail(that.hotelId).then(response=>{
                 let result=response.data;
                 if(result.code==0){
                    that.carddata.vouVerifiedAddress=result.data.hotelAddress
                 }
              }).catch(error=>{
                that.$alert(error,"警告",{
                  confirmButtonText:"确定"
           ...
           //图片上传成功
            handleSuccess(res, file, fileList) {
                  ...
                  this.carddata.vouImagePath.push(image);
                }
                console.log(this.carddata.vouImagePath)
            },
            //图片移除
            handleRemove(file, fileList) {
                if(fileList.length>0){
                   this.carddata.vouImagePath=fileList.map((item,index)=>{
                    return {
                        name:item.name,
                        url:item.url,
...
<style lang="less" scope>
  .CardticketAdd{
    ...
    .vouDeductibleHotelProd.el-select{
      width: 230px;
      .el-input{
        width: 230px;
      }
