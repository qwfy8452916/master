<template>
    <div class="plateeliverylist" >
        ...
        <el-table :data="DeliveryDataList" border stripe style="width:100%;" >
            ...
            <el-table-column prop="delivWay" label="配送方式" min-width="80px">
            ...
            <el-table-column prop="lgcName" label="外部物流" min-width="80px" align=center></el-table-column>
            ...
            <el-table-column prop="lgcDeductFee" label="运费" min-width="80px" align=center></el-table-column>
            ...
            <el-table-column fixed="right" label="操作" min-width="200px" align=center>
                <template slot-scope="scope">
                    <el-button type="text" size="small" v-if="scope.row.status == 0 && authzlist['F:BH_DELIV_OWNDELIVERYLISTSUBMIT']" @click="ensurePlatDelivery(scope.row.id)">确认</el-button>
                    <el-button type="text" size="small" v-if="scope.row.delivWay == 2 && scope.row.status == 1 && authzlist['F:BH_DELIV_OWNDELIVERYLISTSHIP']" @click="shipmentsPlatDelivery(scope.row.id, scope.row.lgcHotelId)">发货</el-button>
                    <el-button type="text" size="small" v-if="scope.row.status == 2 && scope.row.lgcStatus == 5" @click="againShipmentsDelivery(scope.row.delivCode)">重新发单</el-button>
                    <el-button type="text" size="small" v-if="scope.row.lgcHotelId != 0 && scope.row.lgcStatus != 4 && scope.row.status != 0 && scope.row.status != 1" @click="updateLgcStatus(scope.row.delivCode)">更新状态</el-button>
                    <el-button v-if="authzlist['F:BH_DELIV_OWNDELIVERYLIST_DETAIL']" type="text" size="small" @click="deliveryDetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog title="发货提示" :visible.sync="dialogVisibleLgcShipments" width="30%">
            <span>是否确认发货？</span>
            <span slot="footer">
                <el-button @click="dialogVisibleLgcShipments=false">取消</el-button>
                <el-button type="primary" :disabled="isLgcSubmit" @click="ensureLgcShipments">确认</el-button>
            </span>
        </el-dialog>
...
export default {
    ...
    methods: {
        ...
        platDeliveryList(){
            ...
            if(that.flag==true){
                that.falg=false;
                that.$api.platDeliveryList(params)
                    .then(response=>{
                        that.flag=true;
                        const result = response.data;
                        if(response.data.code==0){
                            ...
                            if(that.loadjudge==true){
                                ...
                                if(that.totaldate<response.data.data.total && countnum>0){
                                    ...
                                    localStorage.setItem('OwnDeliverytotal', response.data.data.total);
                                }
        ...
        //发货
        shipmentsPlatDelivery(id, lgcHotelId){
        ...
        againShipmentsDelivery(delivCode){
            ...
            // const id = this.pdId;
            // console.log(params);
            this.$api.againShipmentsDelivery(params, delivCode)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        updateLgcStatus(delivCode){
            ...
            this.$api.updateLgcStatus(params)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
