    ...
    <div class="hoteladd">
            ...
            <el-form-item label="板块类型">
                ...
                <el-table :header-cell-style="{background:'rgba(64,158,255,0.8)',border:'1px rgb(235,238,245) solid !important'}" :data="modelData">
                    ...
                    <el-table-column label="金额" align="center">
                        <el-table-column align="center" prop="redPacketAmountFrom" label="来源"></el-table-column>
                        ...
                        <el-table-column align="center" prop="bonusBaselineType" label="计算基准"></el-table-column>
        ...
        width="900px">
            <el-form :model="modelTypeObj" :disabled='this.modeTitleType == 3' :rules="rulesM" ref="modelTypeObj" label-width="140px" style="width:80%;" class="hotelform">
                ...
                <el-form-item label="分享奖励类型" prop="bonusType">
        ...
        :before-close="cancelLink"
         ...
         width="400px">
            <div>
            <el-form :model="modelLink" ref="modelLink" label-width="80px" style="width:80%;" class="hotelform">
                    ...
                    <el-form-item
                     ...
                     :prop="item.parameterName"
                      :rules="item.isNecessary?[{ required: true, message: '请填写参数',trigger:'blur'}]:[]">
                        <el-input maxlength="50" v-model="modelLink[item.parameterName]" placeholder="请输入参数"></el-input>
                    </el-form-item>
                </div>
            </el-form>
            </div>
    ...
    methods: {
        ...
        getAlllinks(){ 
            ...
            this.$api.selNewLink({params}).then(response => {
                if(response.data.code == 0){
                    that.allLinkList = response.data.data.records;
                }else{
                ...
                if(response.data.code == 0){
                    ...
                    if(this.adAfterPageID == this.currentLinkId){
                        ...
                        this.paramData.forEach(item => {
                            this.$set(this.modelLink,item.parameterName.toString(),item.defaultValue)
                        })
                        let obj = {}
                        for(var item in this.modelLink){
                            obj[item] = this.modelLink[item]
        ...
        deleteRedModel(index,row){
            ...
            this.$confirm('是否确认删除该板块类型?', '提示', {
                ...
                this.$api.delRedpackModel(guiId).then(response => {
                    if(response.data.code == 0){
                        this.$message.success("操作成功");
        ...
        changeRedModel(index,row){
            this.chooseModelID = row[index].id;
            this.dialogVisible1= true
            this.modeTitleType = 2
            this.$api.selOneRedpackModel(this.chooseModelID).then(response => {
                if(response.data.code==0){
                    ...
                    if(this.modelTypeObj.modelType == 1){
                        ...
                        if(this.modelTypeObj.sort == 1){
                            this.typeChooseList1 = resdata.categoryScopeDetailList
                        ...
                        if(this.modelTypeObj.goods == 1){
                            this.goodsChooseList1 = resdata.prodScopeDetailList
                        }else if(this.modelTypeObj.goods == 2){
                            this.goodsChooseList2 = resdata.prodScopeDetailList
                        }
                        ...
                        if(this.modelTypeObj.rooms == 1){
                            this.roomsChooseList1 = resdata.prodScopeDetailList
                        }else if(this.modelTypeObj.rooms == 2){
                            this.roomsChooseList2 = resdata.prodScopeDetailList
                        }
                    }
                ...
                if(response.data.code==0){
                    ...
                    if(this.modelTypeObj.modelType == 1){
                        ...
                        if(this.modelTypeObj.sort == 1){
                            ...
                            this.typeChooseList2 = resdata.categoryScopeDetailList
                            this.sortBoxs[1] = resdata.categoryScopeList
                        }
                        if(this.modelTypeObj.goods == 1){
                            this.goodsChooseList1 = resdata.prodScopeDetailList
        ...
        getModelDataList(){
            this.$api.selRedpackModel({settinId:this.detailId}).then(response => {
                if(response.data.code==0){
                    this.modelData = response.data.data;
                    this.modelData.forEach(item => {
                        item.redPacketAmountFrom = this.changeAFName(item.redPacketAmountFrom).label
                        item.bonusBaselineType = !item.bonusBaselineType?'-':this.changeBSName(item.bonusBaselineType).label
                    })
                }else{
                    this.$alert(response.data.msg,"警告",{
                        confirmButtonText: "确定"
            ...
            this.$refs[modelTypeObj].validate((valid) => {
                if (valid) {
                    if(this.modeTitleType == 1){
                        this.$api.addRedpackModel(params)
                            .then(response => {
                                if(response.data.code==0){
                                   this.$message.success("操作成功")
                                   this.cancelModel()
                        ...
                        this.$api.changeRedpackModel(params,this.chooseModelID)
                            .then(response => {
                                if(response.data.code==0){
                                   this.$message.success("操作成功")
                                   this.cancelModel()
