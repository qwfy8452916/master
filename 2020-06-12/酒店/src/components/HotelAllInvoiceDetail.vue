<template>
    <div class="bookorderdetail">
        ...
        <el-table :data="orderDataDetail.prodList" border style="width:88%;">
            <el-table-column prop="orderCode" label="订单号" width="180px" align=center></el-table-column>
            ...
            <el-table-column prop="createdAt" label="支付时间" width="160px" align=center></el-table-column>
        </el-table>
        <br/><br/>
        <el-button @click="returnList">返回</el-button>
        <el-button v-if="orderStatus == 1 && authzlist['F:BH_FIN_INVOICE_DEAL_SUBMIT']" type="primary" @click="allInvoiceProdDeal">处理</el-button>
...
export default {
    ...
    mounted(){
        ...
        this.allInvoiceProdDetail();
    },
    methods: {
        //获取订单详情
        allInvoiceProdDetail(){
            const params = {};
            const id = this.ipId;
            this.$api.allInvoiceProdDetail(params, id)
                .then(response => {
        ...
        ensureDeal(dealForm){
            ...
            this.$refs[dealForm].validate((valid) => {
                if(valid){
                    this.$api.allInvoiceProdDeal(params, id)
                        .then(response => {
                            // console.log(response);
                            const result = response.data;
                            if(result.code == '0'){
