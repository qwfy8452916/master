<template>
    <div class="godownentrylist">
        ...
        <div><el-button v-if="authzlist['F:BH_INV_GODOWNENTRYLIST_ADD']" class="addbutton" @click="godownEntryAdd">新&nbsp;&nbsp;增</el-button></div>
        <el-table :data="HotelGodownEntryDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" align=center>
                <template slot-scope="scope">
                     <el-button v-if="scope.row.reviewStatus== 2 && authzlist['F:BH_INV_GODOWNENTRYLIST_SCHEDULE']" type="text" size="small" @click="examineDetail(scope.row.wfId)">审核进度</el-button>
                    <el-button v-if="scope.row.reviewStatus == 0 && authzlist['F:BH_INV_GODOWNENTRYLIST_ALTER']" type="text" size="small" @click="modifyInfo(scope.row.id)">修改</el-button>
                    <el-button v-else-if="authzlist['F:BH_INV_GODOWNENTRYLIST_DETAIL']" type="text" size="small" @click="lookDetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
