<template>
    <div class="godownentrylist">
        ...
        <el-table :data="HotelGodownEntryDataList" border style="width:100%;" >
            ...
            <el-table-column prop="supplName" label="类型" align=center></el-table-column>
            ...
            <el-table-column prop="isApproved" label="状态" align=center>
                <template slot-scope="scope">
                    <span v-if="scope.row.isApproved == '0'">驳回</span>
                    <span v-else>{{scope.row.isApproved == '2' ?'通过':'待审核'}}</span>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        //新增
        godownEntryAdd(){
            this.$router.push({name: 'Hotelwarehouseadd'});
        },
        //查看商品明细
        lookDetail(id){
