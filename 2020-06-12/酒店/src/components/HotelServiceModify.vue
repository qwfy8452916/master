    ...
    <div class="hotelserviceadd">
        ...
        <el-form :model="HotelServiceTypeData" :rules="rules" ref="HotelServiceTypeData" label-width="120px" class="hotelservicetypeform">
            ...
            <el-form-item>
                ...
                <el-button v-if="authzData['F:BH_RMSVC_SERVICELIST_EDIT_SUBMIT']" type="primary" :disabled="isSubmit" @click="submitForm('HotelServiceTypeData')">确定</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
                ...
                if(valid){
                    ...
                    this.$api.HotelServiceTypeModify(params, id)
                        .then(response => {
                            ...
                            if(result.code == 0){
                                if(result.data == true){
                                    this.$message.success('修改服务类型成功！');
                                    this.$router.push({name: 'HotelServiceList'});
                                }else{
                                    this.$message.error('修改服务类型失败！');
                                    this.isSubmit = false;
                                }
                            }else{
                                this.$message.error(result.msg);
