<template>
    <div class="hotellist">
        ...
        <div><el-button class="addbutton" @click="hotelADAdd">新&nbsp;&nbsp;增</el-button></div>
        <el-table :data="AdDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" min-width="280px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="scope.row.status == 2 && scope.row.adLevel == 1" type="text" size="small" @click="ADModifyCancel(scope.row.id)">撤销修改</el-button>
                    <el-button type="text" size="small" @click="hotelADDetail(scope.row.id)">详情</el-button>
                    <el-button v-if="scope.row.adLevel == 1" type="text" size="small" @click="hotelADModify(scope.row.id)">修改</el-button>
                    <el-button v-if="scope.row.adLevel == 1" type="text" size="small" @click="hotelADDelete(scope.row.id)">删除</el-button>
                    <el-button v-if="scope.row.status == 1" type="text" size="small" @click="ADDetailQuote(scope.row.id)">引用详情</el-button>
                    <el-button type="text" size="small" @click="hotelADPreview(scope.row.id)">预览</el-button>
                    <el-button v-if="scope.row.status != 1 && scope.row.adLevel == 1" type="text" size="small" @click="hotelADIssue(scope.row.id)">发布</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog title="发布" :visible.sync="dialogVisibleIssue" width="48%">
            ...
            <span slot="footer">
                ...
                <el-button type="primary" @click="EnsureIssue">确定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        //新增
        hotelADAdd(){
            this.$router.push({name:'HotelADAdd'});
        },
        //撤销修改
        ADModifyCancel(id){
            this.ADId = id;
        ...
        EnsureCancel(){
            const id = this.ADId;
            const params = {};
            this.$api.ADModifyCancel(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        //详情
        hotelADDetail(id){
            this.$router.push({name:'HotelADDetail', query: {id}});
        },
        //修改
        hotelADModify(id){
            this.$router.push({name:'HotelADModify', query: {id}});
        },
        //删除
        hotelADDelete(id){
            this.ADId = id;
        ...
        EnsureDelete(){
            const id = this.ADId;
            const params = {};
            this.$api.hotelADDelete(params,id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        //引用详情
        ADDetailQuote(id){
            this.$router.push({name:'HotelADQuoteDetail', query: {id}});
        },
        //预览
        hotelADPreview(id){
            this.ADId = id;
            const params = {};
            this.$api.hotelADDetail(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        EnsureIssue(){
            const id = this.ADId;
            const params = {};
            this.$api.hotelADIssue(params,id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
