<template>
    <div class="functionlist">
        ...
        <div v-if="authzData['F:BO_PROD_MINIPROD_ADD']"><el-button class="addbutton" @click="miniDeployAdd">新增配置商品</el-button></div>
        <el-table :data="miniDeployDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" width="240px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_PROD_MINIPROD_EDIT'] && scope.row.isDefault != 1" type="text" size="small" @click="miniDeployModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BO_PROD_MINIPROD_DELETE'] && scope.row.isDefault != 1" type="text" size="small" @click="miniDeployDelete(scope.row.id)">删除</el-button>
                    <el-button v-if="authzData['F:BO_PROD_MINIPROD_PROD']" type="text" size="small" @click="deployProdSelect(scope.row.hotelId, scope.row.id, scope.row.isDefault)">选择商品</el-button>
                    <el-button v-if="authzData['F:BO_PROD_MINIPROD_ROOM'] && scope.row.isDefault != 1" type="text" size="small" @click="deployRoomSelect(scope.row.hotelId, scope.row.id)">选择房间</el-button>
                </template>
            </el-table-column>
        </el-table>
        ...
        <el-dialog title="管理房间" :visible.sync="dislogVisibleRoom" width="42%">
            <el-transfer
                filterable
                :data = "roomDataList"
...
export default {
    ...
    mounted(){
        ...
        if(this.$route.query.hId != undefined){
            this.inquireHotelName = this.$route.query.hId;
            this.miniDeployList();
        }else{
    ...
    methods: {
        ...
        //新增
        miniDeployAdd(){
            this.$router.push({name:'LonganMinibarProdAdd'});
        },
        //修改
        miniDeployModify(id){
            this.$router.push({name:'LonganMinibarProdModify', query: {id}});
        },
        //删除
        miniDeployDelete(id){
            this.mpId = id;
