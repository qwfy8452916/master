        ...
        <el-form :inline="true" align=left class="searchform">
            ...
            <el-form-item label="柜子类型">
             ...
             <el-form-item label="奖励类型">
                <el-select 
                    v-model="inquirebonusType"
                    :loading="loadingC"
                    placeholder="请选择">
                    <el-option v-for="item in bonusTypeList" :key="item.id" :label="item.typeName" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
        ...
        <el-table :data="PartnerDataList" border stripe style="width:100%;" >
            ...
            <el-table-column label="柜子类型" align=center>
                <template slot-scope="scope">
                    {{scope.row.cabTypeName?scope.row.cabTypeName:"-"}}
                </template>
            </el-table-column>
            <el-table-column label="柜子数量" align=center>
                <template slot-scope="scope">
                    {{scope.row.cabCount?scope.row.cabCount:"-"}}
                </template>
            </el-table-column>
            <el-table-column label="实付金额" align=center>
                <template slot-scope="scope">
                    {{scope.row.orderPayAmount?scope.row.orderPayAmount:"-"}}
                </template>
            </el-table-column>
            <el-table-column label="奖励类型" align=center>
                <template slot-scope="scope">
                    {{scope.row.detailType==3?"推广奖":scope.row.detailType==4?"团队奖":""}}
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        channelPartnerList(){
            ...
            this.$api.getMemberBonus({params})
                .then(response => {
