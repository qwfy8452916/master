    ...
    <div class="plateeliverylist">
        ...
        <el-table :data="DeliveryDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" width="140px" align=center>
                <template slot-scope="scope">
                    <el-button type="text" size="small" v-if="authzData['F:BO_ORDER_OPRDELIV_SUBMIT'] && scope.row.status == 0" @click="ensurePlatDelivery(scope.row.id)">确认</el-button>
                    <el-button type="text" size="small" v-if="authzData['F:BO_ORDER_OPRDELIV_DELIV'] && scope.row.delivWay == 2 && scope.row.status == 1" @click="shipmentsPlatDelivery(scope.row.id)">发货</el-button>
                    <el-button type="text" size="small" v-if="authzData['F:BO_ORDER_OPRDELIV_VIEW']" @click="deliveryDetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
