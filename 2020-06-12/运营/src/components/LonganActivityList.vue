        ...
        <el-form :inline="true" align=left class="searchform">
            ...
            <el-form-item label="活动类型">
                <el-select 
                    ...
                    :loading="loadingH"
                    placeholder="请选择">
                    <el-option v-for="item in actTypeList" :key="item.id" :label="item.label" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="级别">
                <el-select v-model="grade" placeholder="请选择级别">
        ...
        <el-table :data="CabinetList" border stripe style="width:100%;" >
            ...
            <el-table-column label="级别" align=center>
                <template slot-scope="scope">
                    {{scope.row.actScopeLevel?'单店':'平台'}}
                </template>
            </el-table-column>
            <el-table-column prop="actHotelDTOS[0].hotelName" label="适用范围" align=center></el-table-column>
            ...
            <el-table-column label="参与次数" align=center>
                <template slot-scope="scope">
                    {{scope.row.actPartInCount?scope.row.actPartInCount:'不限制'}}
                </template>
            </el-table-column>
            <el-table-column label="禁/启用状态" align=center>
                <template slot-scope="scope">
                    <el-switch @change="changeActivityStatus(scope.row.status,scope.row.id,scope.$index)" v-model="scope.row.status">
                    </el-switch>
                </template>
            </el-table-column>
            ...
            <el-table-column fixed="right" label="操作" align=center>
                <template slot-scope="scope">
                    ...
                    <el-button type="text" size="small" @click="viewDetail(scope.$index, CabinetList)">查看详情</el-button>
                    <el-button type="text" size="small" @click="viewActivityDef(scope.$index, CabinetList)">活动明细</el-button>
                    <!-- <el-button v-if="scope.row.actType == 1" type="text" size="small" @click="viewActivityRecords(scope.$index, CabinetList)">参与记录</el-button> -->
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        addNewSetting(){
            this.$router.push({name:'LonganActivityAdd'});
        },
        //修改
        Cabinetglchange(index,row){
            let guiId=row[index].id
            this.$router.push({name:'LonganActivityChange',query:{modifyid: guiId}});
        },
        remoteCabType(val){
        ...
        viewDetail(index,row){
            let guiId=row[index].id
            this.$router.push({name:'LonganActivityDetail',query:{modifyid: guiId}});
        },
        //活动明细
        viewActivityDef(index,row){
            ...
            if(actType == 1){
                this.$router.push({name:'LonganActivityDef',query:{modifyid: guiId}});
            }else if(actType == 2){
                this.$router.push({name:'LonganActivityDef',query:{modifyid: guiId}});
            }else if(actType == 3){
                this.$router.push({name:'LonganActShareDef',query:{modifyid: guiId}});
            }else if(actType == 4){
                this.$router.push({name:'LonganActRedpackDef',query:{modifyid: guiId,hotelId}});
            }
        },
        //活动参与记录
        viewActivityRecords(index,row){
            let guiId=row[index].id
            this.$router.push({name:'LonganActivityRecord',query:{modifyid: guiId}});
        },
        //修改状态
        changeActivityStatus(value,id,index){
            ...
            this.$confirm(msg, '提示', {
                ...
                this.$api.changeActivityStatus(status,id)
                    .then(response => {
                        if(response.data.code==0){
                            if(value){
        ...
        Cabinetglcancel(index,row){
            ...
            this.$confirm('是否确认删除该活动?', '提示', {
                ...
                this.$api.deleteActivityOne(guiId).then(response => {
                    if(response.data.code == 0){
        ...
        Getdata(){
            ...
            this.$api.selectActivity({params}).then(response => {
                if(response.data.code == 0){
