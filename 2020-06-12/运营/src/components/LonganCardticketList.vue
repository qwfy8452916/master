   ...
   <div class="CardticketList">
       ...
       <el-table :data="cardData" border stripe>
          ...
          <el-table-column prop="isActive" label="状态" align="center">
             <template slot-scope="scope">
                <el-switch :disabled="!authzData['F:BO_VOU_CARDTICKET_SWITCH']" v-model="scope.row.isActive" :active-value="1" :inactive-value="0" @change="updateStatus(scope.row.id)"></el-switch>
             </template>
          </el-table-column>
          ...
          <el-table-column prop="id" label="操作" align="center" width="200px">
              <template slot-scope="scope">
                 <el-button v-if="authzData['F:BO_VOU_CARDTICKET_DETAIL']" type="text" @click="detailCard(scope.row.id)">详情</el-button>
              </template>
          </el-table-column>
       </el-table>
  ...
  export default{
    ...
    methods:{
      resetFunc(){
        ...
        updateStatus(id){
          let that=this;
           this.$api.getCardticketisActive(id).then(response=>{
