<template>
    <div class="commoditymanage">
        ...
        <div><el-button v-if="authzData['F:BO_PROD_OPRPROD_ADD']" class="addbutton" @click="platformCommodityAdd">新&nbsp;&nbsp;增</el-button></div>
        <el-table :data="CommodityDataList" border stripe style="width:100%;" >
            ...
            <el-table-column label="商品形式" min-width="100px" align=center>
                <template slot-scope="scope">
            ...
            <el-table-column fixed="right" label="操作" min-width="340px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_PROD_OPRPROD_REVIEWPROGRESS'] && scope.row.reviewStatus == 2" type="text" size="small" @click="lookReviewProcess(scope.row.wfId)">审核进度</el-button>
                    <el-button type="text" size="small" @click="platformCommodityDetail(scope.row.id)">详情</el-button>
                    <el-button v-if="authzData['F:BO_PROD_OPRPROD_EDIT'] && scope.row.reviewStatus != 2" type="text" size="small" @click="platformCommodityModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BO_PROD_OPRPROD_DELETE']" type="text" size="small" @click="platformCommodityDelete(scope.row.id)">删除</el-button>
                    <el-button type="text" size="small" @click="prodSpecsManage(scope.row.id)">规格管理</el-button>
                    <el-button v-if="authzData['F:BO_PROD_OPRPROD_HOTEL'] && scope.row.isActive == 1" type="text" size="small" @click="platCommHotelManage(scope.row.prodCode)">酒店平台商品管理</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        platformCommodityDetail(id){
            this.$router.push({name: 'LonganPlatformCommodityDetail', query: {id}});
        },
        //修改
        platformCommodityModify(id){
            this.$router.push({name: 'LonganPlatformCommodityModify', query: {id}});
        },
        //删除
        ...
        EnsureDetail(){
            const id = this.pcId;
            const params = {};
            this.$api.platformCommodityDelete(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        platCommHotelManage(pId){
            this.$router.push({name: 'LonganHotelPlatCommodityList', query: {pId}});
        },
        //查看审核进度
