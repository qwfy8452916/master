        ...
        <el-table :data="CabinetList" border stripe style="width:100%;" >
            ...
            <el-table-column label="状态" align=center>
                <template slot-scope="scope">
                    {{scope.row.status==0?'待处理':scope.row.status==1?'已同意':scope.row.status==2?"已拒绝":"已取消"}}
                </template>
            </el-table-column>
            ...
            <el-table-column label="处理时间" align=center>
                <template slot-scope="scope">
                    {{scope.row.transTime == '1970-01-01 00:00:00'? '-' : scope.row.transTime}}
                </template>
            </el-table-column>
            <el-table-column fixed="right" label="操作" align=center>
                <template slot-scope="scope">
                    <el-button type="text" v-if="authzData['F:BO_FS_MEMBERWITHDRAW_DEAL'] && !scope.row.status" size="samll" @click="handleWithdraw(scope.row.id)">处理</el-button>
                    <span v-else>-</span>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        handlresult(params){
            this.$api.handlewithdraw(params,this.currentIndex).then(response => {
        ...
        Getdata(){ 
            ...
            this.$api.getwithdrawAmount({params}).then(response => {
                if(response.data.code == 0){
