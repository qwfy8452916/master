    ...
    <div>
        ...
        <el-table :data="CabinetList" border style="width:100%;" >
            ...
            <el-table-column fixed="right" prop="" width="200px" label="操作" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_CAB_LOG_CAB'] && scope.row.replaceStatus===2" type="text" size="small" @click="Cabinetglchange(scope.$index, CabinetList)">更换柜子</el-button>
                    <el-button v-if="authzData['F:BO_CAB_LOG_CAB'] && scope.row.replaceStatus===0" type="text" size="small" @click="Cabinetglchange(scope.$index, CabinetList)">待处理</el-button>
                    <el-button v-if="authzData['F:BO_CAB_LOG_CAB'] && scope.row.replaceStatus===1" type="text" size="small">待取货</el-button>
                    <el-button v-if="authzData['F:BO_CAB_LOG_CAB'] && scope.row.replaceStatus===3" type="text" size="small" >已更换</el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    methods: {
      ...
      statusselect(e){
        ...
        Confirmdel(){
            ...
            this.$api.launchreplacecabinet(that.modifyid,params).then(response=>{
        ...
        //得到酒店数据
         gethotel(){
            ...
            let params={
              ...
              this.$api.queryhotel(params).then(response=>{
                if(response.data.code==0){
