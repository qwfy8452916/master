   ...
   <div class="AllCouponList">
       ...
       <el-table :data="allCouponList" border stripe style="width:100%">
          ...
          <el-table-column prop="isActive" label="是否有效" min-width="80px" align="center">
              <template slot-scope="scope">
                    <el-switch v-if="authzData['F:BO_COUPON_ALLCOUPON_SWITCH']" v-model="scope.row.isActive" :active-value="1" :inactive-value="0" @change="updateStatus(scope.row.id)"></el-switch>
              </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" prop="id" label="操作" align="center" min-width="180px">
              <template slot-scope="scope">
                  <el-button type="text" v-if="authzData['F:BO_COUPON_ALLCOUPON_EXTEND']" @click="extendTime(scope.row.id,scope.row.couponBatchName,scope.row.couponName)">延长有效期</el-button>
                  <el-button type="text" v-if="scope.row.orderId!='0' && authzData['F:BO_COUPON_ALLCOUPON_ORDER']" @click="checkOrder(scope.row.orderId)">查看订单</el-button>
              </template>
          </el-table-column>
       </el-table>
       <div class="pagination">
        ...
        <el-dialog title="延长有效期" :visible.sync="datchdialog" width="30%">
            ...
            <span slot="footer">
                ...
                <el-button v-if="authzData['F:BO_COUPON_ALLCOUPON_SUBMIT']" type="primary" @click="sureExtendTimeBtn">确 定</el-button>
            </span>
        </el-dialog>
   </div>
</template>
