<template>
    <div class="protocollist">
        ...
        <div v-if="authzData['F:BO_FIN_DIVIDE_ADD']"><el-button class="addbutton" @click="hotelProtocolAdd">新增分成协议</el-button></div>
        <el-table
            :data="hotelProtocolDataList"
            border
            stripe
            style="width:100%;">
            ...
            <el-table-column fixed="right" label="操作" width="180px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="scope.row.isActive == '0' && authzData['F:BO_FIN_DIVIDE_USING']" type="text" size="small" @click="disableProtocol(scope.row.id, scope.row.isActive)">启用</el-button>
                    <el-button v-if="scope.row.isActive == '1' && authzData['F:BO_FIN_DIVIDE_USING']" type="text" size="small" @click="disableProtocol(scope.row.id, scope.row.isActive)">禁用</el-button>
                    <el-button v-if="authzData['F:BO_FIN_DIVIDE_DELETE']" type="text" size="small" @click="hotelProtocolDelete(scope.row.id)">删除</el-button>
                    <el-button v-if="authzData['F:BO_FIN_DIVIDE_VIEW']" type="text" size="small" @click="hotelProtocolDetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        hotelProtocolList(page){
            ...
            this.$api.hotelProtocolList(params)
                .then(response => {
        ...
        inquire(){
            this.pageNum = 1;
            this.hotelProtocolList(this.pageNum);
        ...
        disableEnsure(){
            ...
            this.$api.disableProtocol(params, id)
                .then(response => {
        ...
        EnsureDetail(){
            ...
            const id = this.hpId;
            // console.log(params);
            this.$api.hotelProtocolDelete(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
