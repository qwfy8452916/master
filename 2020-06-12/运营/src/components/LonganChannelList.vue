<template>
    <div class="channellist">
        ...
        <div v-if="authzData['F:BO_FS_CHANNEL_ADD']"><el-button class="addbutton" @click="channelAdd">新增渠道</el-button></div>
        <el-table :data="ChannelDataList" border stripe style="width:100%;" >
            ...
            <el-table-column v-if="authzData['F:BO_FS_CHANNEL_USING']" prop="statusVal" label="禁/启用状态" align=center>
                <template slot-scope="scope">
                    <el-switch v-model="scope.row.statusVal" @change="updateStatus(scope.row.id, scope.row.statusVal)"></el-switch>
                </template>
            </el-table-column>
            <el-table-column fixed="right" label="操作" width="320px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_FS_CHANNEL_EDIT']" type="text" size="small" @click="channelModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BO_FS_CHANNEL_DELETE']" type="text" size="small" @click="channelDelete(scope.row.id)">删除</el-button>
                    <el-button v-if="authzData['F:BO_FS_CHANNEL_RESETPWD']" type="text" size="small" @click="resetPWD(scope.row.userName)">重置密码</el-button>
                    <el-button v-if="authzData['F:BO_FS_CHANNEL_PARTNER']" type="text" size="small" @click="wealthPartner(scope.row.id)">财富合伙人</el-button>
                    <el-button v-if="authzData['F:BO_FS_CHANNEL_LINK']" type="text" size="small" @click="shareLink(scope.row.id)">分享链接</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        updateStatus(id,value){
            ...
            const params = {};
            let status;
            if(value){ status = 1; }else{ status = 0; }
            this.$api.channelStatus(params, id, status)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        EnsureDetail(){
            ...
            this.$api.channelDelete(params,id)
                .then(response => {
                    // console.log(response);
        ...
        EnsureReset(resetForm){
            ...
            this.$refs[resetForm].validate((valid) => {
                if (valid) {
                    this.$api.channelResetPWD(params)
                        .then(response => {
                            // console.log(response);
                            if(response.data.code == 0){
        ...
        wealthPartner(channelId){
            this.$router.push({name: 'LonganChannelPartner', params: {channelId}});
        },
        //分享链接
        shareLink(channelId){
            this.$router.push({name: 'LonganChannelShareLink', params: {channelId}});
        }
    }
}
</script>
