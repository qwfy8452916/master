    ...
    <div>
        ...
        <el-table :data="CabinetList" border stripe style="width:100%;" >
            ...
            <el-table-column label="退款状态" align=center>
                <template slot-scope="scope">
                    {{scope.row.refoundStatus==0?'正常':'已退款'}}
                </template>
            </el-table-column>
            <el-table-column fixed="right" label="退款时间" align=center>
                <template slot-scope="scope">
                    {{scope.row.refoundTime=='1970-01-01 00:00:00'?'-':scope.row.refoundTime}}
                </template>
            </el-table-column>
        </el-table>
    ...
    methods: {
        ...
        changeHotel(){
            if(this.hotelID === ''){
                ...
                this.$api.FsCabinetHotelAll({params}).then(response => {
        ...
        Getdata(){ 
            ...
            this.$api.FsCabinetInvestor({params}).then(response => {
                if(response.data.code == 0){
