    ...
    <div>
        ...
        <el-table :data="CabinetList" border stripe style="width:100%;" >
            ...
            <el-table-column label="终端" align=center>
                <template slot-scope="scope">
                    <span v-if="scope.row.linkTerminal == 1">购物小程序</span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="状态" align=center>
                <template slot-scope="scope">
                    <el-switch v-model="scope.row.isEnable" @change="swichTab(scope.row.isEnable,scope.row.id,scope.$index)"></el-switch>
                </template>
            </el-table-column>
            ...
            <el-table-column fixed="right" width="200" label="操作" align=center>
                <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.isNeedParameter" type="text" size="small" @click="linkParamManager(scope.$index, CabinetList)">参数管理</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        linkChange(index,row){
            ...
            this.$router.push({name:'LonganFuncLinkChange',query:{modifyid:guiId}});
        },
        ...
        linkDelete(index,row){
            ...
            this.$confirm('是否确认删除?', '提示', {
                ...
                this.$api.delNewLink(guiId).then(response => {
                    if(response.data.code == 0){
        ...
        linkDetail(index,row){
            let guiId=row[index].id
            this.$router.push({name:'LonganFuncLinkDetail',query:{modifyid:guiId}});
        },
        //参数管理
        linkParamManager(index,row){
            let guiId=row[index].id
            this.$router.push({name:'LonganFuncLinkParams',query:{modifyid:guiId}});
        },
        //当前页码
        current(){
