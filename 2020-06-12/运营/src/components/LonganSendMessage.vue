  ...
  <div class="SendMessage">
      ...
      <el-table :data="sendMsgData" stripe style="width:100%">
          ...
          <el-table-column prop="content" label="消息内容" align="center">
              <template slot-scope="scope">
                  <el-button v-if="authzData['F:BO_MSG_SENDMSG_MSGCONTENT']" @click="checkMessage(scope.row.content)" type="text" size="small" >查看</el-button>
              </template>
          </el-table-column>
          <el-table-column prop="status" label="消息状态" align="center">
              <template slot-scope="scope">
                 ...
                 <span v-if="scope.row.status=='WAIT'">待确认</span>
              </template>
          </el-table-column>
   ...
   export default {
     ...
     methods:{
       ...
       SendMsg(){
         ...
         this.$api.SendMsg({params}).then(response=>{
            if(response.data.code=='0'){
              that.sendMsgData=response.data.data.records;
              that.pageTotal=response.data.data.total
              console.log(that.sendMsgData)
            }else{
