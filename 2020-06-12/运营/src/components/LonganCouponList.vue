   ...
   <div class="CouponList">
       ...
       <el-table :data="CouponList" border stripe style="width:100%">
          ...
          <el-table-column prop="isActive" label="是否有效" min-width="80px" align="center">
              <template slot-scope="scope">
                    <el-switch v-if="authzData['F:BO_COUPON_OPRCOUPON_SWITCH']" v-model="scope.row.isActive" :active-value="1" :inactive-value="0" @change="updateStatus(scope.row.id)"></el-switch>
              </template>
          </el-table-column>
          ...
          <el-table-column prop="id" label="操作" align="center" fixed="right" min-width="180px">
              <template slot-scope="scope">
                  <el-button v-if="authzData['F:BO_COUPON_OPRCOUPON_EXTEND']" type="text" @click="extendTime(scope.row.id,scope.row.couponBatchName,scope.row.couponName)">延长有效期</el-button>
                  <el-button type="text" v-if="scope.row.orderId!='0' && authzData['F:BO_COUPON_OPRCOUPON_ORDER']" @click="checkOrder(scope.row.orderId)">优惠券订单</el-button>
              </template>
          </el-table-column>
       </el-table>
       <div class="pagination">
        ...
        <el-dialog title="延长有效期" :visible.sync="datchdialog" width="30%">
            ...
            <span slot="footer">
                ...
                <el-button v-if="authzData['F:BO_COUPON_OPRCOUPON_SUBMIT']" type="primary" @click="sureExtendTimeBtn">确 定</el-button>
            </span>
        </el-dialog>
   </div>
</template>
