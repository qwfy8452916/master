        ...
        <el-dialog :before-close="clearData1" width="30%" :visible.sync="dialogVisible" title="新增参数">
          <el-form align="left" :rules="rules" :model="addParmas" ref="addParmas">
            ...
            <el-form-item label="参数说明" label-width="120px" prop="parameterInstructions">
              <el-input v-model="addParmas.parameterInstructions" maxlength="200" type="textarea" :rows="8"></el-input>
            </el-form-item>
  ...
  export default{
    ...
    data(){
      return {
          ...
          rules:{
            parameterName:{required:true,message:"请填写参数名称",trigger:"blur"},
          }
      }
    },
    mounted(){
    ...
    methods:{
      ...
      paramsChange(index,row){
        ...
        this.$api.selOneNewParams(guiId).then(response => {
            if(response.data.code == 0){
                this.addParmas.parameterName = response.data.data.parameterName;
                this.addParmas.defaultValue = response.data.data.defaultValue;
                this.addParmas.parameterInstructions = response.data.data.parameterInstructions;
                this.addParmas.isNecessary = response.data.data.isNecessary? 1:0;
            }else{
                that.$alert(response.data.data.msg,"警告",{
                    confirmButtonText: "确定"
                })
      ...
      paramsDelete(index,row){
          ...
          this.$confirm('是否确认删除?', '提示', {
              ...
              this.$api.delNewParams(guiId).then(response => {
                  if(response.data.code == 0){
