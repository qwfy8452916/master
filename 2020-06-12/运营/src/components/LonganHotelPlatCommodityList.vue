<template>
    <div class="commoditymanage">
        ...
        <div v-if="authzData['F:BO_PROD_HOTELPROD_ADD']"><el-button class="addbutton" @click="hotelPlatCommodityAdd">添&nbsp;&nbsp;加</el-button></div>
        <el-table :data="CommodityDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" width="260px" align=center>
                <template slot-scope="scope">
                    ...
                    <el-button v-if="authzData['F:BO_PROD_HOTELPROD_REVIEWPROGRESS'] && scope.row.reviewStatus == 2" type="text" size="small" @click="lookReviewProcess(scope.row.wfId)">审核进度</el-button>
                    <el-button v-if="authzData['F:BO_PROD_HOTELPROD_EDIT']" type="text" size="small" @click="hotelPlatCommodityModify(scope.row.id, scope.row.hotelId)">修改</el-button>
                    <el-button type="text" size="small" @click="hotelPlatProdDelete(scope.row.id, scope.row.hotelId)">移除</el-button>
                    <el-button v-if="scope.row.prodProductDTO.isSupportSpec == 1" type="text" size="small" @click="hotelPlatProdSpecs(scope.row.id)">规格管理</el-button>
...
export default {
    ...
    methods: {
        ...
        hotelPlatCommodityModify(id, hotelId){
            this.$router.push({name: 'LonganHotelPlatCommodityModify', query: {id}, params: {hotelId}});
        },
        //详情
        hotelPlatCommodityDetail(id){
          this.$router.push({name: 'LonganHotelPlatCommodityDetail', query: {id}});
        },
        //移除
