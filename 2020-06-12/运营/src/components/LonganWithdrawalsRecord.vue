<template>
    <div class="LonganWithdrawalsRecord">
        ...
        <el-table :data="LonganWithdrawalsRecordDataList" border stripe style="width:100%;" >
            ...
            <el-table-column prop="withdrawalStatus" fixed="right" label="操作" width="200px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_FIN_WITHDRAW_DEAL'] && scope.row.withdrawalStatus == '3'" type="text" size="small" @click="HandleDetail(scope.row.id,scope.row.hotelId)">处理</el-button>
                    <el-button v-if="authzData['F:BO_FIN_WITHDRAW_VIEW']" type="text" size="small" @click="lookDetail(scope.row.id)">查看详情</el-button>
                </template>  
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        HotelNameList(){
            ...
            this.$api.HotelNameList(params).then(response=>{
