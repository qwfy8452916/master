        ...
        <el-table :data="AccountlistDataList" border stripe >
            ...
            <el-table-column label="操作" align=center fixed="right" width="240px">
               <template slot-scope="scope">
                    ...
                    <el-button v-if="authzData['F:BO_FIN_ORGACCOUNTLIST_WAITINCOM']" type="text" size="small" @click="waitDivide">待入账分成</el-button>
                    <el-button v-if="authzData['F:BO_FIN_ORGACCOUNTLIST_DIVIDE']" type="text" size="small" @click="DivideRecord">分成记录</el-button>
                    <el-button v-if="authzData['F:BO_FIN_ACCOUNT_CHECKGETCASH']" type="text" size="small" @click="carrydetail(scope.row.id,scope.row.orgId)">提现记录</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        Accountlist(){
            ...
            this.$api.getOrgaccount({params}).then(response=>{
