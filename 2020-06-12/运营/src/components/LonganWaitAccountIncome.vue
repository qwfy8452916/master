<template>
    <div class="WaitAccountIncome">
        ...
        <el-table :data="WaitAccountIncomeDataList" border stripe style="width:100%;" >
            ...
            <el-table-column label="操作" align=center fixed="right">
               <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_FIN_EMPWAITINCOME_DETAIL']" type="text" size="small" @click="lookdetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        resetFunc(){
          ...
          //查看详情
         lookdetail(id){
            this.$router.push({name:'LonganWaitAccountIncomeDetail',query:{id}})
         },
         ...
         WaitAccountIncome(){
            ...
            this.$api.waitInlist({params},1).then(response=>{
                if(response.data.code==0){
                    this.WaitAccountIncomeDataList = response.data.data.records;
                    this.pageTotal = response.data.data.total
                    console.log(this.WaitAccountIncomeDataList)
                }else{
