<template>
   <div class="ProdCouponBatch">
       ...
       <div class="alignleft"><el-button class="addbutton" v-if="authzData['F:BO_COUPON_OPERBATCH_ADD']" @click="addbatch">新增优惠券批次</el-button></div>
       <el-table :data="batchData" border stripe style="width:100%">
          ...
          <el-table-column prop="isActive" label="禁/启用状态"  min-width="100px" align="center">
              <template slot-scope="scope">
                    <el-switch v-if="authzData['F:BO_COUPON_OPERBATCH_SWITCH']" v-model="scope.row.isActive" :active-value="1" :inactive-value="0" @change="updateStatus(scope.row.id,scope.row.isActive)"></el-switch>
              </template>
          </el-table-column>
          <el-table-column fixed="right" prop="id" label="操作" align="center" min-width="300px">
              <template slot-scope="scope">
                  <el-button v-if="authzData['F:BO_COUPON_OPERBATCH_MODIFY']" type="text" @click="editBatch(scope.row.id)">修改</el-button>
                  <el-button type="text" v-if="authzData['F:BO_COUPON_OPERBATCH_DELETE']" @click="delBatch(scope.row.id)">删除</el-button>
                  <el-button v-if="authzData['F:BO_COUPON_OPERBATCH_DETAIL']" type="text" @click="checkBtn(scope.row.id)">查看详情</el-button>
                  <el-button type="text" v-if="scope.row.canGive=='1' && authzData['F:BO_COUPON_OPERBATCH_GRANT']" :disabled="scope.row.isActive=='0'" @click="grant(scope.row.id,scope.row.couponBatchName)">发放</el-button>
                  <el-button v-if="authzData['F:BO_COUPON_OPERBATCH_ORDER']" type="text" @click="checkOrder(scope.row.id)">查看订单</el-button>
              </template>
          </el-table-column>
       </el-table>
       <div class="pagination">
        ...
        <el-dialog title="发放" :visible.sync="datchdialog" width="30%">
            ...
            <span slot="footer">
                ...
                <el-button v-if="authzData['F:BO_COUPON_OPERBATCH_GRANTSUBMIT']" type="primary" @click="sureGrantBtn('diaGrantData')">确 定</el-button>
            </span>
        </el-dialog>
   </div>
</template>
   ...
   export default {
     ...
     methods:{
       ...
       resetFunc(){
        ...
        //新增
        addbatch(){
            this.$router.push({name:"LonganProdCouponBatchAdd"})
        },

        //修改
        editBatch(id){
          this.$router.push({name:"LonganProdCouponBatchEdit",query:{id}})
        },
        //查看详情
        checkBtn(id){
          this.$router.push({name:"LonganProdCouponBatchCheck",query:{id}})
        },
