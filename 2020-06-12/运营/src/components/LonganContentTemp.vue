<template>
  <div class="ContentTemp">
      <el-form align="left" :inline="true">
         <el-form-item label="消息类型">
               <el-select v-model="messageStatus">
                   ...
                   <el-option label="模板消息" value="WX_MP_T"></el-option>
                   <el-option label="订阅消息" value="WX_MINI_D"></el-option>
                   <el-option label="短信" value="PHONE_M"></el-option>
               </el-select>
         </el-form-item>
      ...
      <div class="addranchisee"><el-button class="addbutton" v-if="authzData['F:BO_MSG_CONTENTTEMP_ADD']" type="primary" @click="addranchisee">+创建</el-button></div>
      <el-table :data="contentTempData" stripe style="width:100%">
          <el-table-column prop="ctpType" label="消息类型" align="center">
              <template slot-scope="scope">
                  ...
                  <span v-if="scope.row.ctpType==='PHONE_M'">短消息</span>
              </template>
          </el-table-column>
          <el-table-column prop="ctpCode" label="模板id" align="center"></el-table-column>
          <el-table-column prop="platformTemplateContent" label="内容/说明" align="center">

          </el-table-column>
          <el-table-column prop="ctpCode" fixed="right" label="操作" align="center">
              <template slot-scope="scope">
                  <el-button v-if="authzData['F:BO_MSG_CONTENTTEMP_MODIFY']" @click="editMsgContent(scope.row.ctpCode)" type="text" size="small">修改</el-button>
                  <el-button v-if="authzData['F:BO_MSG_CONTENTTEMP_DELETE']" @click="deleteMsgContent(scope.row.ctpCode)" type="text" size="small">删除</el-button>
              </template>
          </el-table-column>
      </el-table>
      ...
      <!-- 内容模板弹窗 -->
          <el-dialog class="createTemp" :title="contentTempTitle" :visible.sync='showhidemsgone' center width="30%">
             <el-form :model="msgContentTemplateDTO" :rules="rules" ref="msgContentTemplateDTO">
                <el-form-item label="选择类型" label-width="108px" prop="ctpType">
                   <el-select v-model="msgContentTemplateDTO.ctpType" @change="selectType">
                       <el-option label="模板消息" value="WX_MP_T"></el-option>
                       <el-option label="订阅消息" value="WX_MINI_D"></el-option>
                       <el-option label="短消息" value="PHONE_M"></el-option>
                   </el-select>
                </el-form-item>
                <el-form-item label="标题" label-width="108px" prop="ctpTitle">
                  <el-input v-model="msgContentTemplateDTO.ctpTitle"></el-input>
                </el-form-item>
                <el-form-item v-if="msgContentTemplateDTO.ctpType!='PHONE_M'" label="Appid" prop
                ="platformAppId" label-width="108px">
                ...
                <el-form-item label="参数转换规则" label-width="108px" prop="platformParamsTransRule">
                  <el-input v-model="msgContentTemplateDTO.platformParamsTransRule" type="textarea" rows="2"></el-input>
                </el-form-item>
                <el-form-item label="内容" label-width="108px" prop="platformTemplateContent">
                  <el-input @keyup.native="wordStatic(this);" v-model="msgContentTemplateDTO.platformTemplateContent" type="textarea" rows="3" maxlength="255"></el-input>
                  <div class="weui_textarea_counter"><span id="num">{{msgContentTemplateDTO.platformTemplateContent.length}}</span>/255</div>
                </el-form-item>
              </el-form>
              <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="showhidemsgone=false">取 消</el-button>
                <el-button type="primary" v-if="authzData['F:BO_MSG_CONTENTTEMP_SUBMIT']" @click="createSure('msgContentTemplateDTO')">确 定</el-button>
              </div>
          </el-dialog>
        <!-- 内容模板弹窗 -->
   ...
   export default {
     ...
     data(){
       return {
          ...
          rules:{
            ...
            platformParamsTransRule:{required:true,message:"请填写参数转换规则",trigger:'blur'},
            platformTemplateContent:{required:true,message:"请填写内容",trigger:'blur'},
            platformAppId:{required:true,message:"请填写APPid",trigger:'blur'}
          },
     ...
     methods:{
       ...
       editMsgContent(e){
         ...
         let params="";
         this.$api.contentTempDetail(params,e).then(response=>{
       ...
       Confirmdel(){
          let that=this;
          let params="";
          this.$api.delContentTemp(params,that.contentTempcode).then(response=>{
             if(response.data.code=='0'){
       ...
       createSure(msgContentTemplateDTO){
        ...
        let params={
              ...
              platformParamsTransRule:that.msgContentTemplateDTO.platformParamsTransRule,
              platformTemplateContent:that.msgContentTemplateDTO.platformTemplateContent,
       ...
       createContentTemp(params){
         let that=this;
         this.$api.createContentTemp(params).then(response=>{
                   if(response.data.code=='0'){
       ...
       editContentTemp(params){
         let that=this;
         this.$api.editContentTemp(params,that.contentTempcode).then(response=>{
            if(response.data.code=='0'){
               that.$message.success("操作成功")
               that.showhidemsgone=false;
               that.contentTempList()
       ...
       contentTempList(){
         ...
         this.$api.contentTempList({params}).then(response=>{
           if(response.data.code=='0'){
...
.ContentTemp{
    ...
    .messageone{
       ...
       .weui_textarea_counter{text-align: right;}
       .addranchisee{
        float: left;
        margin-bottom: 10px;
       }
