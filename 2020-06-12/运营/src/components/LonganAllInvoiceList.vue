    ...
    <div class="commoditymanage">
        ...
        <el-table :data="InvoiceDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" width="100px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="scope.row.status == 1 && scope.row.invOrgId == 2 && authzData['F:BO_FIN_INVOICE_DEAL']" type="text" size="small" @click="waitInvoiceProdDetail(scope.row.id)">处理</el-button>
                    <el-button v-else-if="authzData['F:BO_FIN_INVOICE_VIEW']" type="text" size="small" @click="waitInvoiceProdDetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
