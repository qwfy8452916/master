<template>
    <div class="DefEvaluate">
        ...
        <div><el-button v-if="authzData['F:BO_REM_DEFAULTEVALUATEADD']" class="addbutton" @click="addevaluate()">新增评价</el-button></div>
        <el-table :data="DefEvaluateDataList" border stripe style="width:100%;" >
            ...
            <el-table-column label="操作" align=center fixed="right">
               <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_REM_DEFAULTEVALUATEEDIT']" type="text" size="small" @click="DefEvaluateedit(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BO_REM_DEFAULTEVALUATEDELETE']" type="text" size="small" @click="DefEvaluatedel(scope.row.id)">删除</el-button>
                    <el-button v-if="authzData['F:BO_REM_DEFAULTEVALUATEDETAIL']" type="text" size="small" @click="DefEvaluatedetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    methods: {
        resetFunc(){
         ...
         Confirmdel(){
          let that=this;
          let params={};
          this.$api.DeleEvaluate(params,this.defevaluateid).then(response=>{
        ...
        DefEvaluate(){
            ...
            this.$api.prodEvaluatelist({params}).then(response=>{
                if(response.data.code==0){
