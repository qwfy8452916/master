<template>
    <div class="OrganDivideRecord">
        ...
        <el-table :data="OrganDivideRecordDataList" border stripe style="width:100%;" >
            ...
            <el-table-column prop="orderCode" label="订单号" align=center>
               <template slot-scope="scope">
                   <a class="astyle" href="#" @click="orderList(scope.row.funcId)">{{scope.row.orderCode}}</a>
               </template>
            </el-table-column>
            <el-table-column prop="delivCode" label="配送单号" align=center>
               <template slot-scope="scope">
                   <a class="astyle" href="#" @click="deliveList">{{scope.row.delivCode}}</a>
               </template>
            </el-table-column>
            ...
            <el-table-column prop="revenueAmount" label="分成金额" align=center></el-table-column>
            ...
            <el-table-column label="操作" align=center fixed="right">
               <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_FIN_DIVIDE_DETAIL']" type="text" size="small" @click="lookdetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        resetFunc(){
          ...
          //查看详情
         lookdetail(id){
            this.$router.push({name:'LonganOrganDivideRecordDetail',query:{id}})
         },
        ...
        OrganDivideRecord(){
            ...
            this.$api.orgDivideRecord({params}).then(response=>{
                if(response.data.code==0){
                    this.OrganDivideRecordDataList = response.data.data.records;
