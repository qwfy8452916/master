        ...
        <el-table :data="CabinetList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_FS_INVESTOR_VISIT']" type="text" size="small" @click="lookRecords(scope.$index, CabinetList)">访问记录</el-button>
                    <el-button v-if="authzData['F:BO_FS_INVESTOR_COUPON']" type="text" size="small" @click="bounceRecords(scope.$index, CabinetList)">优惠券记录</el-button>
                    <el-button v-if="authzData['F:BO_FS_INVESTOR_ORDER']" type="text" size="small" @click="investorOrder(scope.$index, CabinetList)">投资订单</el-button>
                    <el-button v-if="authzData['F:BO_FS_INVESTOR_CAB']" type="text" size="small" @click="investorCabinet(scope.$index, CabinetList)">投资柜子</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        lookRecords(index,row){
            ...
            this.$router.push({name:'LauncherlookRecords',params:{modifyid: guiId}});
        },
        bounceRecords(index,row){
            let guiId=row[index].openId
            this.$router.push({name:'LauncherbounceRecords',params:{modifyid: guiId}});
        },
        investorOrder(index,row){
            let guiId=row[index].openId
            this.$router.push({name:'LauncherinvestorOrder',params:{modifyid: guiId}});
        },
        investorCabinet(index,row){
            let guiId=row[index].openId
            this.$router.push({name:'LauncherinvestorCabinet',params:{modifyid: guiId}});
        },
        ...
        Getdata(){ 
            ...
            this.$api.FsPersonSearch({params}).then(response => {
