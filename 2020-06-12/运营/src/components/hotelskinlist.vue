<template>
    <div class="hotellist">
        ...
        <div v-if="authzData['F:BO_HOTEL_THEME_ADD']"><el-button class="addbutton" @click="addproduct()">新增主题</el-button></div>
        <el-table :data="Skindatalist" border stripe style="width:100%;" >
            ...
            <el-table-column prop="" label="操作" width="120px" align=center is-center>
                <template slot-scope="scope">
                    <el-button v-if="authzData['F:BO_HOTEL_THEME_EDIT']" type="text" size="small" @click="Modifyproduct(scope.$index, Skindatalist)">修改</el-button>
                    <el-button v-if="authzData['F:BO_HOTEL_THEME_DELETE']" type="text" size="small" @click="Deleteproduct(scope.$index, Skindatalist)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        Modifyproduct(index,rows){
            ...
            this.$router.push({name:'hotelskinmodify',query:{productid: changeid}});
        },
        //删除
        ...
        Confirmdel(){
            let that=this;
            let params="";
            this.$api.deletehotelskin({params},that.delindexid).then(response=>{
        ...
        Getdata(){
            ...
            this.$api.gethotelskinlist({params}).then(response=>{
                if(response.data.code==0){
