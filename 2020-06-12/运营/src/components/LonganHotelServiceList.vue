<template>
    <div class="servicetypelist">
        ...
        <div v-if="authzData['F:BO_RMSVC_RMSVC_ADD']"><el-button class="addbutton" @click="serviceTypeAdd">添加酒店服务类型</el-button></div>
        <el-table :data="ServiceTypeDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作" width="240px" align=center>
                <template slot-scope="scope">
                    ...
                    <el-button v-if="authzData['F:BO_RMSVC_RMSVC_DELETE']" type="text" size="small" @click="DeleteServiceType(scope.row.id)">移除</el-button>
                    ...
                    <el-button v-if="scope.row.style == 'LIST_ORDER' || scope.row.style == 'ICON_ORDER'" type="text" size="small" @click="ManageCatalogue(scope.row.id)">管理目录</el-button>
                    <el-button type="text" size="small" @click="ManageParticulars(scope.row.id, scope.row.style)">管理明细</el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    methods: {
        ...
        ManageParticulars(id, style){
            if(style == 'PIC_ONLY'){   //纯图片
                ...
                this.$router.push({name:'LonganHotelServiceSelectList', query: {id}});
            }else if(style == 'ICON_ORDER'){   //图标
                this.$router.push({name:'LonganHotelServiceIconList', query: {id}});
