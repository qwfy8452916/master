<template>
    <div class="servicetypelist">
        <div><el-button class="addbutton" @click="serviceCatalogueAdd">新增服务类型目录</el-button></div>
        <el-table :data="ServiceCatalogueDataList" border style="width:100%;" >
            ...
            <el-table-column prop="iconUrl" label="图标" min-width="80px" align=center>
                <template slot-scope="scope">
                    <img :src="scope.row.iconUrl" alt="" style="width:45px;height:35px">
                </template>
            </el-table-column>
            <el-table-column prop="name" label="目录名称" min-width="160px"></el-table-column>
            <el-table-column fixed="right" label="操作" width="100px" align=center>
                <template slot-scope="scope">
                    <el-button type="text" size="small" @click="serviceCatalogueModify(scope.row.id)">修改</el-button>
                    <el-button type="text" size="small" @click="serviceCatalogueDelete(scope.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <div class="returnbtn">
...
export default {
    ...
    methods: {
        ...
        serviceCatalogueAdd(){
            const hsId = this.hsId;
            this.$router.push({name:'LonganHotelServiceCatalogueAdd', query: {hsId}});
        },
        //修改
        serviceCatalogueModify(id){
            const hsId = this.hsId;
            this.$router.push({name:'LonganHotelServiceCatalogueModify', query: {hsId, id}});
        },
        //删除
