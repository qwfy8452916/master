    ...
    <div class="LonganMerchantadd">
        ...
        <el-form :model="Merchantadd" :rules="rules" ref="Merchantadd" label-width="160px" align=left class="Merchantform">
            ...
            <el-form-item label="社会信用代码"  v-if="Merchantadd.merchantType=='c'">
                <el-input v-model.trim="Merchantadd.socialCreditCode" :disabled="true" @blur="isnumber"></el-input>
            </el-form-item>
            <el-form-item label="身份证号码"  v-if="Merchantadd.merchantType=='p'">
                <el-input v-model.trim="Merchantadd.IDnumber" :disabled="true" @blur="isnumber"></el-input>
            </el-form-item>
            ...
            <el-form-item>
                ...
                <el-select v-model="Merchantadd.selectDistrict" placeholder="区级地区" @change="selectArea" style="width: 28%;">
                    <el-option v-for="item in area" :key="item.id" :label="item.dictName" :value="item.dictValue"></el-option>
                </el-select>
            </el-form-item>
            ...
            <el-form-item>
                ...
                <el-button v-if="authzData['F:BO_MER_MER_EDIT_SUBMIT']" type="primary" :disabled="isSubmit" @click="subbtn('Merchantadd')">确定</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
export default {
    ...
    methods: {
        getMerchaninfo(){
            this.$api.getMerchaninfo(this.id).then(response => {
                ...
                if(result.code == 0){
                    ...
                    this.Merchantadd.selectCity = resultdata.merchantCity,//市
                    this.Merchantadd.selectDistrict = resultdata.merchantArea,//区
        ...
        subbtn(Merchantadd){
            ...
            this.$refs[Merchantadd].validate((valid) => {
                if (valid) {
                    ...
                    // console.log(params);
                    // return
                    this.$api.changemerchant(this.id,params).then(response => {
                        const result = response.data;
                        if(result.code == 0){
                            if(result.data){
                                this.$message.success("修改入驻商成功！");
                                this.$router.push({name:'LonganMerchant'});
                            }
                        }else{
                            this.$alert(response.data.msg,"警告",{
