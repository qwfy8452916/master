        ...
        <el-table :data="PacketDataList" border stripe style="width:100%;" >
            ...
            <el-table-column label="过期时间" width="160px" align=center>
                <template slot-scope="scope">
                    {{scope.row.deadLine=='1970-01-01 00:00:00'?'-':scope.row.deadLine}}
                </template>
            </el-table-column>
            <el-table-column fixed="right" label="操作" width="100px" align=center>
                <template slot-scope="scope">
                    ...
                    <el-button v-if="authzData['F:BO_FS_RED_RECORD']" type="text" size="small" @click="packetGetRecord(scope.row.id)">领取记录</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        packetGetRecord(id){
            const params = {};
            // console.log(id);
            this.$api.packetGetRecord(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
