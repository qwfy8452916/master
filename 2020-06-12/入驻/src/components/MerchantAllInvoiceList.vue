<template>
    <div class="commoditymanage">
        ...
        <el-table :data="InvoiceDataList" border stripe style="width:100%;" >
            ...
            <el-table-column prop="invCategory" label="发票类别" width="140px">
                <template slot-scope="scope">
                    <span v-if="scope.row.invCategory == 1">酒店房费发票</span>
                    <span v-else-if="scope.row.invCategory == 2">商品销售发票</span>
                </template>
            </el-table-column>
            <el-table-column prop="invType" label="发票类型" width="140px">
                <template slot-scope="scope">
            ...
            <el-table-column fixed="right" label="操作" width="100px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="scope.row.status == 1 && authzData['F:BM_FIN_INVOICE_DEAL']" type="text" size="small" @click="waitInvoiceProdDetail(scope.row.id)">处理</el-button>
                    <el-button v-if="scope.row.status != 1 && authzData['F:BM_FIN_INVOICE_VIEW']" type="text" size="small" @click="waitInvoiceProdDetail(scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        waitInvoiceProdList(){
            ...
            // console.log(params);
            this.$api.waitInvoiceProdList(params)
                .then(response => {
        ...
        waitInvoiceProdDetail(id){
            this.$router.push({name: 'MerchantAllInvoiceDetail', query: {id}});
        }
    }
}
</script>
