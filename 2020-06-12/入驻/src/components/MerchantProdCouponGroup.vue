<template>
   <div class="ProdCouponGroup">
       ...
       <div class="alignleft"><el-button class="addbutton" v-if="authzData['F:BM_COUPON_MERGROUP_ADD']" @click="addBtn">新增优惠券分组</el-button></div>
       <el-table :data="groupData" border stripe style="width:100%">
          ...
          <el-table-column prop="id" label="操作" align="center">
              <template slot-scope="scope">
                  <el-button v-if="authzData['F:BM_COUPON_MERGROUP_MODIFY']" type="text" @click="editGroup(scope.row.id)">修改</el-button>
                  <el-button v-if="authzData['F:BM_COUPON_MERGROUP_DETELE']" type="text" @click="deleGroup(scope.row.id)">删除</el-button>
              </template>
          </el-table-column>
       </el-table>
...
import resetButton from './resetButton'
   export default {
     ...
     methods:{
        ...
        //新增优惠券分组
        addBtn(){
          this.$router.push({name:"MerchantProdCouponGroupAdd"})
        },

        //修改分组
        editGroup(e){
          let id=e;
          this.$router.push({name:"MerchantProdCouponGroupEdit",query:{id}})
        },
