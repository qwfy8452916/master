<template>
  <div class="godownentrylist">
    ...
    <el-table :data="stockOutDataList" border stripe style="width:100%;">
      <el-table-column fixed prop="invOutCode" label="出库单id" width="120px" align=center></el-table-column>
      ...
      <el-table-column prop="invOutRemark" label="说明"></el-table-column>
      ...
      <el-table-column fixed="right" label="操作" align=center>
        <template slot-scope="scope">
          <el-button v-if="scope.row.reviewStatus==2 && authzData['F:BM_INV_OUTSTOCKTRIAL']" type="text" size="small" @click="examineDetail(scope.row.wfId)">审核进度</el-button>
          <el-button v-if="authzData['F:BM_INV_OUTSTOCKCHECK']" type="text" size="small" @click="stockOutDetail(scope.row.id)">查看详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    methods: {
        ...
        stockOutList() {
            ...
            const params = {
                ...
                outTimeStart: that.outTime[0],
                outTimeEnd: that.outTime[1],
                pageNo: that.pageNum,
                pageSize: that.pageSize
            };
            this.$api.stockOutList(params)
                .then(response => {
                    const result = response.data;
                    // console.log(result);
                    if (response.data.code == '0') {
                        that.pageTotal = result.data.total;
                        that.stockOutDataList = result.data.records;
                    } else {
        ...
        //新增
        stockOutAdd(){
            this.$router.push({name: 'MerchantGodownEntryDetail'});
        },
        //查看详情
        stockOutDetail(id) {
            this.$router.push({name: 'MerchantStockOutDetail', query: {id}});
        },
        //审核详情
        examineDetail(id) {
