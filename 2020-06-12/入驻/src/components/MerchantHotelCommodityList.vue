<template>
    <div class="commoditymanage">
        ...
        <el-table :data="CommodityDataList" border stripe style="width:100%;" >
            ...
            <el-table-column prop="prodType" label="商品形式" min-width="100px" align=center>
               <template slot-scope="scope">
            ...
            <el-table-column fixed="right" label="操作" min-width="260px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="scope.row.reviewStatus == 2 && authzData['F:BM_PROD_HOTELPRODUCTPROGRESS_SUBMIT']" type="text" size="small" @click="lookReviewProcess(scope.row.wfId)">审核进度</el-button>
                    <el-button type="text" size="small" @click="hotelCommodityDetail(scope.row.id, scope.row.hotelId)">详情</el-button>
                    <el-button v-if="authzData['F:BM_PROD_HOTELPRODUCTEDIT']" type="text" size="small" @click="hotelCommodityModify(scope.row.id, scope.row.hotelId)">修改</el-button>
                    <el-button type="text" size="small" @click="hotelCommodityDelete(scope.row.id)">移除</el-button>
                    <el-button v-if="scope.row.prodProductDTO.isSupportSpec == 1" type="text" size="small" @click="hotelCommoditySpecs(scope.row.id)">规格管理</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        updateStatus(id,value){
            // console.log(value);
            const params = {};
            this.$api.hotelCommodityStatus(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
        ...
        hotelCommodityDetail(id, hotelId){
            this.$router.push({name: 'MerchantHotelCommodityDetail', query: {id}, params: {hotelId}});
        },
        //修改
        hotelCommodityModify(id, hotelId){
            this.$router.push({name: 'MerchantHotelCommodityModify', query: {id}, params: {hotelId}});
        },
        //移除
        ...
        EnsureDetail(){
            const id = this.hcId;
            const params = {};
            this.$api.hotelCommodityDelete(params, id)
                .then(response => {
                    // console.log(response);
                    const result = response.data;
                    if(result.code == '0'){
