<template>
    <div class="commoditymanage">
        ...
        <div><el-button v-if="authzData['F:BM_PROD_PRODUCTADD']" class="addbutton" @click="ownCommodityAdd">新&nbsp;&nbsp;增</el-button></div>
        <el-table :data="CommodityDataList" border stripe style="width:100%;" >
            ...
            <el-table-column fixed="right" label="操作"  min-width="320px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="scope.row.reviewStatus == 2 && authzData['F:BM_PROD_PRODUCTPROGRESS_SUBMIT']" type="text" size="small" @click="lookReviewProcess(scope.row.wfId)">审核进度</el-button>
                    <el-button type="text" size="small" @click="ownCommodityDetail(scope.row.id)">详情</el-button>
                    <el-button v-if="scope.row.reviewStatus != 2 && authzData['F:BM_PROD_PRODUCTEDIT']" type="text" size="small" @click="ownCommodityModify(scope.row.id)">修改</el-button>
                    <el-button v-if="authzData['F:BM_PROD_PRODUCTDELETE_SUBMIT']" type="text" size="small" @click="ownCommodityDelete(scope.row.id)">删除</el-button>
                    <el-button type="text" size="small" @click="ownCommoditySpecs(scope.row.id)">规格管理</el-button>
                    <el-button v-if="scope.row.isActive == 1 && authzData['F:BM_PROD_PRODUCTENTER_SUBMIT']" type="text" size="small" @click="ownHotelManage(scope.row.prodCode)">酒店商品管理</el-button>
                </template>
            </el-table-column>
        </el-table>
