<template>
    <div class="ProcessDetailsbox">
        ...
        <ul class="ProcessDetailsul">
            <li>
                <span>审核状态</span>
                <span v-if="PendingReviewDetailsData.isComplete == 0">待审核</span>
                <span v-else-if="PendingReviewDetailsData.isComplete == 1">已审核</span>
            </li>
            <li>
                <span>业务类型</span><span>{{PendingReviewDetailsData.bizType}}</span>
            </li>
            <li>
                <span>业务id</span><span>{{PendingReviewDetailsData.bizId}}</span>
            </li>
            <li>
                <span>提交时间</span><span>{{PendingReviewDetailsData.createdAt}}</span>
            </li>
            <li v-if="PendingReviewDetailsData.isComplete == 1">
                <span>审核结果</span>
                <span v-if="PendingReviewDetailsData.reviewResult == 0">驳回</span>
                <span v-else-if="PendingReviewDetailsData.reviewResult == 1">通过</span>
            </li>
            <li v-if="PendingReviewDetailsData.isComplete == 1">
                <span>备注</span><span>{{PendingReviewDetailsData.callbackContent}}</span>
            </li>
        </ul>
        ...
        <el-table :data="PendingReviewDetailsDataList" border style="width:100%;margin-bottom: 30px;" >
            <el-table-column prop="name" label="任务名称" align=center></el-table-column>
            <el-table-column prop="startTime" label="开始时间" align=center></el-table-column>
            <el-table-column prop="endTime" label="完成时间" align=center></el-table-column>
            <el-table-column prop="handler" label="审核人" align=center>
                <template slot-scope="scope">
                    <span v-if="scope.row.handler != undefined">{{scope.row.handler.value}}</span>
                </template> 
            </el-table-column>
            <el-table-column prop="reviewResult" label="审核结果" align=center>
                <template slot-scope="scope">
                    <span v-if="scope.row.reviewResult != undefined">
            ...
            <el-table-column prop="comments" label="备注" align=center>
                <template slot-scope="scope">
                    <span v-if="scope.row.comments != undefined">{{scope.row.comments.value}}</span>
                </template> 
            </el-table-column>
        </el-table>
        ...
        <el-form :model="form" :rules="rules" ref="form" class="chengebox">
            ...
            <el-form-item label="备注" :label-width="formLabelWidth">
                <el-input type="textarea" :rows="3" v-model.trim="form.comments" style="width: 100%"></el-input>
            </el-form-item>
            
        </el-form>

        <div class="btnbox">
            <el-button v-if="authzlist['F:CM_REV_UNREV_DETAIL_SUBMIT']" type="primary" @click="review()">确定</el-button>
...
export default {
    ...
    data(){
        return{
            ...
            id: '',
            reviewid: '',
            PendingReviewDetailsData: {},
            PendingReviewDetailsDataList: [],
            BizFormData: {},
            form: {
               reviewResult: '',
               comments: ''
    ...
    mounted(){
        ...
        this.reviewid = this.$route.query.id.reviewid
        this.PendingClaimDetailsDetails();
    },
    methods: {
        //详情信息
        PendingClaimDetailsDetails(){
            privilegeApi.getReviewDetails(this.id).then(response=>{
                if(response.data.code==0){
                    this.PendingReviewDetailsData = response.data.data;
                    this.PendingReviewDetailsDataList = response.data.data.taskData.data.map(item=>{
        ...
        review(){
            ...
            privilegeApi.postreview(this.reviewid,params).then(response=>{
