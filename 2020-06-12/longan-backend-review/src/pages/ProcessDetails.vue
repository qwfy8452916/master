<template>
    <div class="ProcessDetailsbox">
        ...
        <ul class="ProcessDetailsul">
            <li>
                <span>审核状态</span>
                <span v-if="ProcessDetailsData.isComplete == 0">待审核</span>
                <span v-else-if="ProcessDetailsData.isComplete == 1">已审核</span>
            </li>
            <li>
                <span>业务类型</span><span>{{ProcessDetailsData.bizType}}</span>
            </li>
            <li>
                <span>业务id</span><span>{{ProcessDetailsData.bizId}}</span>
            </li>
            <li>
                <span>提交时间</span><span>{{ProcessDetailsData.createdAt}}</span>
            </li>
            <li v-if="ProcessDetailsData.isComplete == 1">
                <span>审核结果</span>
                <span v-if="ProcessDetailsData.reviewResult == 0">驳回</span>
                <span v-else-if="ProcessDetailsData.reviewResult == 1">通过</span>
            </li>
            <li v-if="ProcessDetailsData.isComplete == 1">
                <span>备注</span><span>{{ProcessDetailsData.callbackContent}}</span>
            </li>
        </ul>
        ...
        <el-table :data="ProcessDataList" border style="width:100%;margin-bottom: 30px;" >
        ...
        ProcessDetails(){
            privilegeApi.getReviewDetails(this.id).then(response=>{
                if(response.data.code==0){
                    this.ProcessDetailsData = response.data.data;
                    this.ProcessDataList = response.data.data.taskData.data.map(item=>{
