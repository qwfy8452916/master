<template>
    <div class="PendingClaimList">
        ...
        <el-table :data="PendingClaimListDataList" border stripe style="width:100%;" >
            ...
            <el-table-column label="操作" width="160px" align=center>
                <template slot-scope="scope">
                    <el-button v-if="authzlist['F:CM_REV_CLAIM']" type="text" size="small" @click="claim(scope.row.id)">认领</el-button>
                    <el-button v-if="authzlist['F:CM_REV_CLAIM_VIEW']" type="text" size="small" @click="lookDetail(scope.row.processInstanceId,scope.row.id)">查看详情</el-button>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        PendingClaimList(){
            ...
            privilegeApi.getPendingClaimList(params).then(response=>{
                let result = response.data;
                if(result.code==0){
                    this.PendingClaimListDataList = result.data.data.map(item => {
        ...
        //查看详情
        lookDetail(id,claimid){
            // this.$router.push({name: 'HotelRevenueDetail', query: {id}});
            this.$emit('claim-details', {id,claimid});
        },
        //认领
        claim(id){
            const params = {
            ...
            privilegeApi.postclaim(id,params).then(response=>{
                let result = response.data;
                if(result.code==0){
                    if(result.data){
                        this.PendingClaimList();
