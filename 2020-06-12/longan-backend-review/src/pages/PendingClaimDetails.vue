<template>
    <div class="ProcessDetailsbox">
        ...
        <ul class="ProcessDetailsul">
            <li>
                <span>审核状态</span>
                <span v-if="PendingClaimDetailsData.isComplete == 0">待审核</span>
                <span v-else-if="PendingClaimDetailsData.isComplete == 1">已审核</span>
            </li>
            <li>
                <span>业务类型</span><span>{{PendingClaimDetailsData.bizType}}</span>
            </li>
            <li>
                <span>业务id</span><span>{{PendingClaimDetailsData.bizId}}</span>
            </li>
            <li>
                <span>提交时间</span><span>{{PendingClaimDetailsData.createdAt}}</span>
            </li>
            <li v-if="PendingClaimDetailsData.isComplete == 1">
                <span>审核结果</span>
                <span v-if="PendingClaimDetailsData.reviewResult == 0">驳回</span>
                <span v-else-if="PendingClaimDetailsData.reviewResult == 1">通过</span>
            </li>
            <li v-if="PendingClaimDetailsData.isComplete == 1">
                <span>备注</span><span>{{PendingClaimDetailsData.callbackContent}}</span>
            </li>
        </ul>
        ...
        <el-table :data="PendingClaimDetailsDataList" border style="width:100%;margin-bottom: 30px;" >
        ...
        <div class="btnbox">
            <el-button v-if="authzlist['F:CM_REV_CLAIM_DETAIL_SUBMIT']" type="primary" @click="claimfun">认领</el-button>
...
export default {
    ...
    data(){
        return{
            ...
            id: '',
            claimid: '',
            PendingClaimDetailsData: {},
            PendingClaimDetailsDataList: [],
            BizFormData: {},
            stylehtml: '',
            bthtml1: '',
            bthtml2: '',
    ...
    mounted(){
        ...
        this.claimid = this.$route.query.id.claimid
    ...
    methods: {
        ...
        PendingClaimDetailsDetails(){
            privilegeApi.getReviewDetails(this.id).then(response=>{
                if(response.data.code==0){
                    this.PendingClaimDetailsData = response.data.data;
                    this.PendingClaimDetailsDataList = response.data.data.taskData.data.map(item=>{
        ...
        claimfun(){
            ...
            privilegeApi.postclaim(this.claimid,params).then(response=>{
